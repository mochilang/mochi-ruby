...
function test_example_1()
    if not (__eq(intToRoman(3), "III")) then
        error("expect failed")
    end
end

function test_example_2()
    if not (__eq(intToRoman(58), "LVIII")) then
        error("expect failed")
    end
end

function test_example_3()
    if not (__eq(intToRoman(1994), "MCMXCIV")) then
        error("expect failed")
    end
end

function test_small_numbers()
    if not (__eq(intToRoman(4), "IV")) then
        error("expect failed")
    end
    if not (__eq(intToRoman(9), "IX")) then
        error("expect failed")
    end
end
...
local __tests = {
    ...
    {name = "small numbers", fn = test_small_numbers}
}
__run_tests(__tests)
