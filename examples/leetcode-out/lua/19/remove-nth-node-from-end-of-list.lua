...
function test_example_1()
    if not (__eq(removeNthFromEnd({1, 2, 3, 4, 5}, 2), {1, 2, 3, 5})) then
        error("expect failed")
    end
end

function test_example_2()
    if not (__eq(removeNthFromEnd({1}, 1), {})) then
        error("expect failed")
    end
end

function test_example_3()
    if not (__eq(removeNthFromEnd({1, 2}, 1), {1})) then
        error("expect failed")
    end
end

function test_remove_first()
    if not (__eq(removeNthFromEnd({7, 8, 9}, 3), {8, 9})) then
        error("expect failed")
    end
end

function test_remove_last()
    if not (__eq(removeNthFromEnd({7, 8, 9}, 1), {7, 8})) then
        error("expect failed")
    end
end
...
local __tests = {
    ...
    {name = "remove first", fn = test_remove_first},
    {name = "remove last", fn = test_remove_last}
}
__run_tests(__tests)
