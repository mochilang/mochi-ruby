    ...
    for i = 0, (n) - 1 do
        ...
        for j = __add(i, 1), (n) - 1 do
            ...
            while (left < right) do
                local sum =
                    __add(
                    __add(__add(__index(sorted, i), __index(sorted, j)), __index(sorted, left)),
                    __index(sorted, right)
                )
                if __eq(sum, target) then
                    result =
                        __add(
                        result,
                        {{__index(sorted, i), __index(sorted, j), __index(sorted, left), __index(sorted, right)}}
                    )
                    left = __add(left, 1)
                    right = (right - 1)
...
function test_example_1()
    if not (__eq(fourSum({1, 0, -1, 0, -2, 2}, 0), {{-2, -1, 1, 2}, {-2, 0, 0, 2}, {-1, 0, 0, 1}})) then
        error("expect failed")
    end
end

function test_example_2()
    if not (__eq(fourSum({2, 2, 2, 2, 2}, 8), {{2, 2, 2, 2}})) then
        error("expect failed")
    end
end
