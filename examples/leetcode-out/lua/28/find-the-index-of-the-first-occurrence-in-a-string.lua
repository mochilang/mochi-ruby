...
function test_example_1()
    if not (__eq(strStr("sadbutsad", "sad"), 0)) then
        error("expect failed")
    end
end

function test_example_2()
    if not (__eq(strStr("leetcode", "leeto"), (-1))) then
        error("expect failed")
    end
end

function test_empty_needle()
    if not (__eq(strStr("abc", ""), 0)) then
        error("expect failed")
    end
end

function test_needle_at_end()
    if not (__eq(strStr("hello", "lo"), 3)) then
        error("expect failed")
    end
end
...
local __tests = {
    ...
    {name = "empty needle", fn = test_empty_needle},
    {name = "needle at end", fn = test_needle_at_end}
}
__run_tests(__tests)
