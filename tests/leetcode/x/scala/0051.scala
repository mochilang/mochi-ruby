object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (lines.nonEmpty && lines(0).nonEmpty) {
      ...
      for (tc <- 0 until t) { val n = lines(idx).toInt; idx += 1; val sols = solve(n); out += sols.length.toString; for (si <- sols.indices) { out ++= sols(si); if (si + 1 < sols.length) out += "-" }; if (tc + 1 < t) out += "=" }
      print(out.mkString("\n"))
    }
  }
}
