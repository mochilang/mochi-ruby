      ...
      let rec loop tc xs =
        if tc = t then []
          ...
          let body =
            List.concat
              (List.mapi
                 (fun i sol -> sol @ if i + 1 < List.length sols then [ "-" ] else [])
                 sols)
          in
          let block = [ string_of_int (List.length sols) ] @ body in
          let sep = if tc + 1 < t then [ "=" ] else [] in
          block @ sep @ loop (tc + 1) (List.tl xs)
      in
