...
for tc = 1, t do local n = tonumber(lines[idx]); idx = idx + 1; local sols = solve(n); table.insert(out, tostring(#sols)); for si, sol in ipairs(sols) do for _, row in ipairs(sol) do table.insert(out, row) end if si < #sols then table.insert(out, '-') end end if tc < t then table.insert(out, '=') end end
