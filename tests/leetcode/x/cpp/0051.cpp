...
int main(){ ios::sync_with_stdio(false); cin.tie(nullptr); int t; if(!(cin>>t)) return 0; for(int tc=0; tc<t; tc++){ int n; cin>>n; auto sols=solve(n); if(tc) cout<<"=\n"; cout<<sols.size()<<'\n'; for(size_t si=0; si<sols.size(); si++){ for(auto &row: sols[si]) cout<<row<<'\n'; if(si+1<sols.size()) cout<<"-\n"; } } return 0; }
