...
fn fizz_buzz(number_param: i64, iterations_param: i64) []const u8 {
    ...
    var out: []const u8 = "";
    out = out;
    var n: i64 = number_var;
    n = n;
    while (n <= iterations_var) {
        if (@mod(n, 3) == 0) {
            out = _concat_string(out, "Fizz");
        }
        if (@mod(n, 5) == 0) {
            out = _concat_string(out, "Buzz");
        }
        if (@mod(n, 3) != 0 and @mod(n, 5) != 0) {
            out = _concat_string(out, _str(n));
        }
