...
fn minimum_squares_to_represent_a_number(number_param: i64) i64 {
    ...
    var answers: []i64 = make_list(number_var + 1, 0 - 1);
    answers = answers;
    answers[_idx(answers.len, 0)] = 0;
    var i: i64 = 1;
    i = i;
    while (i <= number_var) {
        var answer: i64 = i;
        answer = answer;
        const root: i64 = int_sqrt(i);
        var j: i64 = 1;
        j = j;
        while (j <= root) {
            const current_answer: i64 = 1 + answers[_idx(answers.len, i - j * j)];
            if (current_answer < answer) {
                answer = current_answer;
            }
            j = j + 1;
        }
        answers[_idx(answers.len, i)] = answer;
        i = i + 1;
    }
    return answers[_idx(answers.len, number_var)];
}
...
pub fn main() void {
    {
        ...
        std.debug.print("{s}\n", .{_str(minimum_squares_to_represent_a_number(25))});
        std.debug.print("{s}\n", .{_str(minimum_squares_to_represent_a_number(21))});
        const __end = _now();
