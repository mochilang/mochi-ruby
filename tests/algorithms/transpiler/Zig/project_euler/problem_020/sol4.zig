...
fn factorial_digit_sum(num: i64) i64 {
    var factorial: i64 = 1;
    factorial = factorial;
    var i: i64 = 1;
    i = i;
    ...
    const s: []const u8 = _str(factorial);
    var result: i64 = 0;
    result = result;
    var j: i64 = 0;
    j = j;
    while (j < @as(i64, @intCast(s.len))) {
        result = result +% (std.fmt.parseInt(i64, s[_idx(s.len, j).._idx(s.len, j) + 1], 10) catch 0);
        j = j +% 1;
    }
    return result;
}
...
fn mochi_main() void {
    std.debug.print("{s}\n", .{"Enter the Number: "});
    const n: i64 = (std.fmt.parseInt(i64, _input(), 10) catch 0);
    std.debug.print("{s}\n", .{_str(factorial_digit_sum(n))});
}
