...
fn solution(n: []const u8) i64 {
    ...
    while (i <= @as(i64, @intCast(n.len)) -% 13) {
        ...
        j = j;
        while (j < 13) {
            product = product *% (std.fmt.parseInt(i64, n[@as(usize, @intCast(i +% j))..@min(@as(usize, @intCast(i +% j +% 1)), @as(usize, @intCast(n.len)))], 10) catch 0);
            j = j +% 1;
        }
...
pub fn main() void {
    {
        ...
        std.debug.print("{s}\n", .{_concat_string("solution() = ", _str(solution(N_var)))});
        const __end = _now();
