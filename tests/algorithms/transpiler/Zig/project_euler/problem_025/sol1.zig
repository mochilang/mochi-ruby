...
fn fibonacci_digits_index(n_1: i64) i64 {
    var digits: i64 = 0;
    digits = digits;
    var index: i64 = 2;
    index = index;
    while (digits < n_1) {
        index = index +% 1;
        const fib: i64 = fibonacci(index);
        digits = @as(i64, @intCast(_str(fib).len));
    }
    return index;
}

fn solution(n_2: i64) i64 {
    return fibonacci_digits_index(n_2);
}

fn mochi_main() void {
    const n_3: i64 = (std.fmt.parseInt(i64, _input(), 10) catch 0);
    std.debug.print("{s}\n", .{_str(solution(n_3))});
}
