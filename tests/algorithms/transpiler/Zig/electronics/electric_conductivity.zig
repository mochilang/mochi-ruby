...
fn electric_conductivity(conductivity: f64, electron_conc: f64, mobility: f64) Result {
    ...
    if (conductivity == 0.0) {
        return .{ .kind = "conductivity", .value = mobility * electron_conc * ELECTRON_CHARGE_var };
    }
    if (electron_conc == 0.0) {
        return .{ .kind = "electron_conc", .value = conductivity / (mobility * ELECTRON_CHARGE_var) };
    }
    return .{ .kind = "mobility", .value = conductivity / (electron_conc * ELECTRON_CHARGE_var) };
}
