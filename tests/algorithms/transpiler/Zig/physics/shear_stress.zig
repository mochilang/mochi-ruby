...
fn shear_stress(stress: f64, tangential_force: f64, area: f64) Result {
    var zeros: i64 = 0;
    zeros = zeros;
    if (stress == 0.0) {
        zeros = zeros +% 1;
...
pub fn main() void {
    {
        ...
        r1_var = shear_stress(25.0, 100.0, 0.0);
        r2_var = shear_stress(0.0, 1600.0, 200.0);
        r3_var = shear_stress(1000.0, 0.0, 1200.0);
        std.debug.print("{s}\n", .{str_result(r1_var)});
        std.debug.print("{s}\n", .{str_result(r2_var)});
        std.debug.print("{s}\n", .{str_result(r3_var)});
        const __end = _now();
