...
fn relu(vector_param: []f64) []f64 {
    ...
    while (i < @as(i64, @intCast(vector_var.len))) {
        ...
        if (v > 0.0) {
            ...
            result = blk: { var _tmp = std.ArrayList(f64).init(std.heap.page_allocator); _tmp.appendSlice(@as([]const f64, result)) catch |err| handleError(err); _tmp.append(0.0) catch |err| handleError(err); break :blk (_tmp.toOwnedSlice() catch |err| handleError(err)); };
        }
        i = i + 1;
    }
    return result;
...
pub fn main() void {
    {
        ...
        std.debug.print("{s}\n", .{_str(relu(@constCast(&([3]f64{@as(f64, @floatFromInt(0)) - 1.0, 0.0, 5.0}))[0..]))});
        const __end = _now();
