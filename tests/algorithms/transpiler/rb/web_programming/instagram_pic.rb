...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def download_image(html)
    ...
    if _eq(idx_content, (-1))
      return "Image URL not found in meta tag."
    end
    start = _add(idx_content, _len(key))
    end_ = start
    while end_ < _len(html) && !_eq(html[end_..._add(end_, 1)], "\"")
      end_ = _add(end_, 1)
    end
    if end_ >= _len(html)
      return "Image URL not found in meta tag."
    end
    image_url = html[start...end_]
    return _add("Image URL: ", image_url)
  end
