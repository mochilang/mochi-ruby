...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def max_product_subarray(numbers)
    if _eq(_len(numbers), 0)
      return 0
    end
    max_till_now = (__tmp1 = numbers; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0))
    min_till_now = (__tmp2 = numbers; __tmp2.is_a?(Hash) ? __tmp2[0] : _idx(__tmp2, 0))
    max_prod = (__tmp3 = numbers; __tmp3.is_a?(Hash) ? __tmp3[0] : _idx(__tmp3, 0))
    i = 1
    while i < _len(numbers)
      number = (__tmp4 = numbers; __tmp4.is_a?(Hash) ? __tmp4[i] : _idx(__tmp4, i))
      if number < 0
        temp = max_till_now
