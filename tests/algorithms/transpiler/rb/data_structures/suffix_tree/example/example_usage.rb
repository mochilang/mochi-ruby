...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def search(tree, pattern)
    ...
    i = 0
    while i <= n - m
      if _eq(tree.text[i..._add(i, m)], pattern)
        return true
      end
      i = _add(i, 1)
    end
  ...
  def main()
    ...
    while i < _len(patterns)
      pattern = (__tmp1 = patterns; __tmp1.is_a?(Hash) ? __tmp1[i] : _idx(__tmp1, i))
      found = search(suffix_tree, pattern)
      puts(_add(_add(_add("Pattern '", pattern), "' found: "), _str(found)))
      i = _add(i, 1)
    end
  end
  Object.send(:remove_const, :SuffixTree) if Object.const_defined?(:SuffixTree)
  Object.const_set(:SuffixTree, Struct.new(:text, keyword_init: true))
  main()
