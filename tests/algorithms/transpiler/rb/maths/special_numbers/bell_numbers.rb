...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def binomial_coefficient(total_elements, elements_to_choose)
    if _eq(elements_to_choose, 0) || _eq(elements_to_choose, total_elements)
      return 1
    end
    ...
    while i < k
      ...
      coefficient = coefficient / (_add(i, 1))
      i = _add(i, 1)
    end
    return coefficient
  end
  def bell_numbers(max_set_length)
    ...
    while i <= max_set_length
      j = 0
      while j < i
        bell[i] = _add((__tmp1 = bell; __tmp1.is_a?(Hash) ? __tmp1[i] : _idx(__tmp1, i)), binomial_coefficient(i - 1, j) * (__tmp2 = bell; __tmp2.is_a?(Hash) ? __tmp2[j] : _idx(__tmp2, j)))
        j = _add(j, 1)
      end
      i = _add(i, 1)
    end
