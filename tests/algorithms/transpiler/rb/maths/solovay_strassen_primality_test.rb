...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def jacobi_symbol(random_a, number)
    if _eq(random_a, 0) || _eq(random_a, 1)
      return random_a
    end
    random_a = random_a % number
    t = 1
    while !_eq(random_a, 0)
      while _eq(random_a % 2, 0)
        random_a = random_a / 2
        r = number % 8
        if _eq(r, 3) || _eq(r, 5)
          t = -t
      ...
      random_a = number
      number = temp
      if _eq(random_a % 4, 3) && _eq(number % 4, 3)
        t = -t
      end
      random_a = random_a % number
    end
  ...
  def solovay_strassen(number, iterations)
    ...
    while i < iterations
      ...
      mod_x = x % number
      if mod_x < 0
        mod_x = _add(mod_x, number)
      end
      if _eq(x, 0) || !_eq(y, mod_x)
        return false
      end
      i = _add(i, 1)
    end
