...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def simpson_rule(boundary, steps)
    ...
    a = (__tmp1 = boundary; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0))
    b = (__tmp2 = boundary; __tmp2.is_a?(Hash) ? __tmp2[1] : _idx(__tmp2, 1))
    h = (b - a) / ((steps).to_f)
    ...
    while i < _len(pts)
      ...
      y = _add(y, (h / 3.0) * coeff * f((__tmp3 = pts; __tmp3.is_a?(Hash) ? __tmp3[i] : _idx(__tmp3, i))))
      cnt = _add(cnt, 1)
      i = _add(i, 1)
    end
    y = _add(y, (h / 3.0) * f(b))
