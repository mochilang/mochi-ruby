...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def is_safe(grid, visited, row, col)
    rows = _len(grid)
    cols = _len((__tmp1 = grid; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0)))
    ...
    return not_visited && _eq((__tmp4 = (__tmp5 = grid; __tmp5.is_a?(Hash) ? __tmp5[row] : _idx(__tmp5, row)); __tmp4.is_a?(Hash) ? __tmp4[col] : _idx(__tmp4, col)), 1)
  end
  ...
  def count_islands(grid)
    rows = _len(grid)
    cols = _len((__tmp9 = grid; __tmp9.is_a?(Hash) ? __tmp9[0] : _idx(__tmp9, 0)))
    visited = []
    i = 0
    while i < rows
      row_list = []
