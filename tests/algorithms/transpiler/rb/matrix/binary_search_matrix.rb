...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def mat_bin_search(value, matrix)
    ...
    while index < _len(matrix) && (__tmp5 = (__tmp6 = matrix; __tmp6.is_a?(Hash) ? __tmp6[index] : _idx(__tmp6, index)); __tmp5.is_a?(Hash) ? __tmp5[0] : _idx(__tmp5, 0)) < value
      r = binary_search((__tmp7 = matrix; __tmp7.is_a?(Hash) ? __tmp7[index] : _idx(__tmp7, index)), 0, _len((__tmp8 = matrix; __tmp8.is_a?(Hash) ? __tmp8[index] : _idx(__tmp8, index))) - 1, value)
      if !_eq(r, (-1))
        return [index, r]
      end
      index = _add(index, 1)
  ...
  def main()
    row = [1, 4, 7, 11, 15]
    puts(_str(binary_search(row, 0, _len(row) - 1, 1)))
    puts(_str(binary_search(row, 0, _len(row) - 1, 23)))
