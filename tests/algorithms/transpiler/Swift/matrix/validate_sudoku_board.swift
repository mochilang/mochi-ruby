// Generated by Mochi transpiler v0.10.73 on 2025-08-25 22:33:46 GMT+7
import Foundation
import Dispatch
    ...
    func is_valid_sudoku_board(_ board: [[String]]) -> Bool {
        ...
        for r in 0..<NUM_SQUARES {
            for c in 0..<NUM_SQUARES {
                ...
                let box: Int = ((_int(_int(Int((Double(r) / Double(3)).rounded(.down)))) &* 3) &+ _int(_int(Int((Double(c) / Double(3)).rounded(.down)))))
                if (((((_idx(rows, r) as! [String]).contains(value)) || ((_idx(cols, c) as! [String]).contains(value))) || ((_idx(boxes, box) as! [String]).contains(value)))) {
                    return false
                }
                rows = _set(rows, r, { let _tmp0 = (_append((_idx(rows, r) as! [String]), value) as! [String]); return _tmp0 }())
                cols = _set(cols, c, { let _tmp1 = (_append((_idx(cols, c) as! [String]), value) as! [String]); return _tmp1 }())
                boxes = _set(boxes, box, { let _tmp2 = (_append((_idx(boxes, box) as! [String]), value) as! [String]); return _tmp2 }())
            }
        }
        return true
    }
