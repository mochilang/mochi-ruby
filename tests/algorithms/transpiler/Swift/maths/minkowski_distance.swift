// Generated by Mochi transpiler v0.10.67 on 2025-08-17 09:21:02 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func nth_root(_ value: Double, _ n: Int) -> Double {
        ...
        var i = 0
        while (i < 20) {
            let num = ((_num((n &- 1)) * x) + (value / _num(pow_float(_num(x), (n &- 1)))))
            x = _num((num / _num(n)))
            i = _int((i &+ 1))
        }
        return x
    ...
    func minkowski_distance(_ point_a: [Double], _ point_b: [Double], _ order: Int) -> Double {
        ...
        while (idx < _int(((point_a).count))) {
            let diff = _num(abs_val(_num((_num(_idx(point_a, idx)) - _num(_idx(point_b, idx))))))
            total = _num((total + _num(pow_float(_num(diff), order))))
            idx = _int((idx &+ 1))
        }
        return _num(nth_root(_num(total), order))
    }
    func test_minkowski() {
        if ({ let _tmp3 = _num(abs_val(_num({ let _tmp1 = _num(minkowski_distance(({
            var _arr: [Double] = []
            ...
            _arr = _append(_arr, 2.0)
            return _arr
        }() as! [Double]), 1)); let _tmp2 = 2.0; return _tmp1 - _tmp2}()))); let _tmp4 = 0.0001; return _tmp3 > _tmp4}()) {
            _ = fatalError("minkowski_distance test1 failed")
        }
        if ({ let _tmp7 = _num(abs_val(_num({ let _tmp5 = _num(minkowski_distance(({
            var _arr: [Double] = []
            _arr = _append(_arr, 1.0)
            _arr = _append(_arr, 2.0)
            ...
            return _arr
        }() as! [Double]), 2)); let _tmp6 = 8.0; return _tmp5 - _tmp6}()))); let _tmp8 = 0.0001; return _tmp7 > _tmp8}()) {
            _ = fatalError("minkowski_distance test2 failed")
        }
    }
    func main() {
        _ = test_minkowski()
        print(_p(_num(minkowski_distance(({
            var _arr: [Double] = []
            _arr = _append(_arr, 5.0)
            return _arr
        }() as! [Double]), ({
