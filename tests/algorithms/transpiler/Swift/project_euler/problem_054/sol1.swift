// Generated by Mochi transpiler v0.10.66 on 2025-08-14 18:40:15 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func card_value(_ c: String) -> Int {
        if (c == "2") {
            return 2
        }
        if (c == "3") {
    ...
    func filter_not(_ xs: [Int], _ v: Int) -> [Int] {
        var res: [Int] = ([] as! [Int])
        for x in xs {
            if (x != v) {
    ...
    func hand_rank(_ hand: [String]) -> [Int] {
        var ranks: [Int] = ([] as! [Int])
        var suits: [String] = ([] as! [String])
        for card in hand {
            ranks = (_append(ranks, _int(card_value((String(Array(card)[0]) as! String)))) as! [Int])
            suits = (_append(suits, (String(Array(card)[1]) as! String)) as! [String])
        }
        ranks = (sort_desc((ranks as! [Int])) as! [Int])
        var flush = true
        var i = 1
        while (i < _int(((suits).count))) {
            if ((_idx(suits, i) as! String) != (_idx(suits, 0) as! String)) {
                flush = false
            }
            i = _int((i &+ 1))
        }
        ...
        i = 0
        while (i < 4) {
            if (((_idx(ranks, i) as? Int ?? 0) &- (_idx(ranks, (i &+ 1)) as? Int ?? 0)) != 1) {
                straight = false
            }
            i = _int((i &+ 1))
        }
        if ((((((!straight) && ((_idx(ranks, 0) as? Int ?? 0) == 14)) && ((_idx(ranks, 1) as? Int ?? 0) == 5)) && ((_idx(ranks, 2) as? Int ?? 0) == 4)) && ((_idx(ranks, 3) as? Int ?? 0) == 3)) && ((_idx(ranks, 4) as? Int ?? 0) == 2)) {
            straight = true
            ranks = ({
                var _arr: [Int] = []
                _arr = _append(_arr, 5)
        ...
        for r in ranks {
            if (counts[r] != nil) {
                counts = _set(counts, r, _int((((counts[r] ?? 0) as? Int ?? 0) &+ 1)))
            } else {
                counts = _set(counts, r, 1)
            }
        }
        var uniq: [Int] = ([] as! [Int])
        for r in ranks {
            ...
            if (!exists) {
                uniq = (_append(uniq, r) as! [Int])
            }
        }
        var count_vals: [[Int]] = ([] as! [[Int]])
        for u in uniq {
            count_vals = (_append(count_vals, ({
                var _arr: [Int] = []
                _arr = _append(_arr, ((counts[u] ?? 0) as? Int ?? 0))
                _arr = _append(_arr, u)
                return _arr
            }() as! [Int])) as! [[Int]])
        }
        var n = _int(((count_vals).count))
        var i2 = 0
        while (i2 < n) {
            var j2 = 0
            while (j2 < (n &- 1)) {
                let a: [Int] = (_idx(count_vals, j2) as! [Int])
                let b: [Int] = (_idx(count_vals, (j2 &+ 1)) as! [Int])
                if (((_idx(a, 0) as? Int ?? 0) < (_idx(b, 0) as? Int ?? 0)) || (((_idx(a, 0) as? Int ?? 0) == (_idx(b, 0) as? Int ?? 0)) && ((_idx(a, 1) as? Int ?? 0) < (_idx(b, 1) as? Int ?? 0)))) {
                    let tmp: [Int] = (_idx(count_vals, j2) as! [Int])
                    count_vals = _set(count_vals, j2, (_idx(count_vals, (j2 &+ 1)) as! [Int]))
                    count_vals = _set(count_vals, (j2 &+ 1), (tmp as! [Int]))
                }
                j2 = _int((j2 &+ 1))
            }
            i2 = _int((i2 &+ 1))
        }
        let c1 = (_idx(_idx(count_vals, 0)!, 0) as? Int ?? 0)
        let v1 = (_idx(_idx(count_vals, 0)!, 1) as? Int ?? 0)
        var rank: [Int] = ([] as! [Int])
        if (straight && flush) {
            if ((_idx(ranks, 0) as? Int ?? 0) == 14) {
                rank = (_append(rank, 9) as! [Int])
                rank = (_append(rank, 14) as! [Int])
                return rank
            }
            rank = (_append(rank, 8) as! [Int])
            rank = (_append(rank, (_idx(ranks, 0) as? Int ?? 0)) as! [Int])
            return rank
        }
        if (c1 == 4) {
            var kicker = 0
            ...
            rank = (_append(rank, 7) as! [Int])
            rank = (_append(rank, v1) as! [Int])
            rank = (_append(rank, kicker) as! [Int])
            return rank
        }
        if (c1 == 3) {
            let c2 = (_idx(_idx(count_vals, 1)!, 0) as? Int ?? 0)
            let v2 = (_idx(_idx(count_vals, 1)!, 1) as? Int ?? 0)
            if (c2 == 2) {
                rank = (_append(rank, 6) as! [Int])
                rank = (_append(rank, v1) as! [Int])
                rank = (_append(rank, v2) as! [Int])
                return rank
            }
            rank = (_append(rank, 3) as! [Int])
            rank = (_append(rank, v1) as! [Int])
            for r in ranks {
                if (r != v1) {
                    rank = (_append(rank, r) as! [Int])
                }
            }
            return rank
        }
        if (c1 == 2) {
            ...
            if (c2 == 2) {
                ...
                rank = (_append(rank, 2) as! [Int])
                rank = (_append(rank, high_pair) as! [Int])
                rank = (_append(rank, low_pair) as! [Int])
                rank = (_append(rank, kicker) as! [Int])
                return rank
            }
            rank = (_append(rank, 1) as! [Int])
            rank = (_append(rank, v1) as! [Int])
            for r in ranks {
                if (r != v1) {
                    rank = (_append(rank, r) as! [Int])
    ...
    func compare_hands(_ h1: [String], _ h2: [String]) -> Int {
        let r1: [Int] = (hand_rank((h1 as! [String])) as! [Int])
        let r2: [Int] = (hand_rank((h2 as! [String])) as! [Int])
        var i = 0
        while ((i < _int(((r1).count))) && (i < _int(((r2).count)))) {
            if ((_idx(r1, i) as? Int ?? 0) > (_idx(r2, i) as? Int ?? 0)) {
                return 1
            }
            if ((_idx(r1, i) as? Int ?? 0) < (_idx(r2, i) as? Int ?? 0)) {
                return -1
