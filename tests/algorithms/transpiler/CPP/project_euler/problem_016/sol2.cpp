// Generated by Mochi transpiler v0.10.71 on 2025-08-23 15:46:54 GMT+7
...
cpp_int solution(const cpp_int& power) {
    ...
    while ((i < power)) {
        ...
        cpp_int j = int64_t(0);
        while ((j < digits.size())) {
            cpp_int v = ((digits[static_cast<size_t>(j)] * int64_t(2)) + carry);
            digits[static_cast<size_t>(j)] = (v % int64_t(10));
            carry = (v / int64_t(10));
            j = (j + int64_t(1));
        }
        if ((carry > int64_t(0))) {
    ...
    cpp_int k = int64_t(0);
    while ((k < digits.size())) {
        sum = (sum + digits[static_cast<size_t>(k)]);
        k = (k + int64_t(1));
    }
    return sum;
}
