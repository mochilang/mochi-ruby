// Generated by Mochi transpiler v0.10.71 on 2025-08-23 15:47:53 GMT+7
...
cpp_int factorial_digit_sum(const cpp_int& num);
int main();

cpp_int factorial_digit_sum(const cpp_int& num) {
    cpp_int factorial = int64_t(1);
    cpp_int i = int64_t(1);
    while ((i <= num)) {
    ...
    cpp_int result = int64_t(0);
    cpp_int j = int64_t(0);
    while ((j < s.size())) {
        result = (result + cpp_int(([&](const auto& __s){ auto __i = j; if (__i < 0) __i += __s.size(); return __s[static_cast<size_t>(__i)]; })(s) - '0'));
        j = (j + int64_t(1));
    }
    return result;
}
