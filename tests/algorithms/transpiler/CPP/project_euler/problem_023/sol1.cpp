// Generated by Mochi transpiler v0.10.71 on 2025-08-23 15:48:33 GMT+7
...
cpp_int solution(const cpp_int& limit) {
    std::vector<cpp_int> sum_divs = {};
    cpp_int i = int64_t(0);
    while ((i <= limit)) {
        sum_divs = ([&]{ auto __tmp = sum_divs; __tmp.push_back(int64_t(1)); return __tmp; }());
        i = (i + int64_t(1));
    }
    cpp_int sqrt_limit = int_sqrt(limit);
    i = int64_t(2);
    while ((i <= sqrt_limit)) {
        sum_divs[static_cast<size_t>((i * i))] = (sum_divs[static_cast<size_t>((i * i))] + i);
        cpp_int k = (i + int64_t(1));
        while ((k <= (limit / i))) {
            sum_divs[static_cast<size_t>((k * i))] = ((sum_divs[static_cast<size_t>((k * i))] + k) + i);
            k = (k + int64_t(1));
        }
        i = (i + int64_t(1));
    }
    ...
    std::vector<cpp_int> abundants = {};
    cpp_int res = int64_t(0);
    cpp_int n = int64_t(1);
    while ((n <= limit)) {
        if ((sum_divs[static_cast<size_t>(n)] > n)) {
            abundants = ([&]{ auto __tmp = abundants; __tmp.push_back(n); return __tmp; }());
            is_abundant[static_cast<size_t>(n)] = true;
        }
        bool has_pair = false;
        cpp_int j = int64_t(0);
        while ((j < abundants.size())) {
            cpp_int a = abundants[static_cast<size_t>(j)];
            if ((a > n)) {
                break;
            }
            cpp_int b = (n - a);
            if (((b <= limit) && is_abundant[static_cast<size_t>(b)])) {
                has_pair = true;
                break;
            }
            j = (j + int64_t(1));
