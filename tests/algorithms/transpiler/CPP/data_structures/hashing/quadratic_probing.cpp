// Generated by Mochi transpiler v0.10.72 on 2025-08-24 23:14:47 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
double balanced_factor(const HashTable& table) {
    ...
    while ((i < table.values.size())) {
        if ((_index(table.values, i) != 0)) {
            count = (count + int64_t(1));
        }
        i = (i + int64_t(1));
    }
    return ((double)((double)(count)) / (double)((double)(table.size_table)));
}

int64_t collision_resolution(const HashTable& table, int64_t key) {
    int64_t i = int64_t(1);
    int64_t new_key = hash_function(table, (key + (i * i)));
    while (((_index(table.values, new_key) != 0) && (_index(table.values, new_key) != key))) {
        i = (i + int64_t(1));
...
void insert_data(HashTable table, int64_t data) {
    ...
    if ((_index(vals, key) == 0)) {
        _index(vals, key) = data;
    } else     if ((_index(vals, key) == data)) {
        table.values = vals;
        return;
    } else {
        int64_t new_key = collision_resolution(table, key);
        if (((new_key < vals.size()) && (_index(vals, new_key) == 0))) {
            _index(vals, new_key) = data;
        }
    }
...
std::string int_to_string(int64_t n) {
    ...
    while ((num > int64_t(0))) {
        ...
        std::string ch = ([&]{ std::string __s = std::string("0123456789"); long long __start = digit; long long __end = (digit + int64_t(1)); if(__start < 0) __start = 0; if(__end < __start) __end = __start; if(__start > static_cast<long long>(__s.size())) __start = __s.size(); if(__end > static_cast<long long>(__s.size())) __end = __s.size(); return __s.substr(static_cast<size_t>(__start), static_cast<size_t>(__end - __start)); })();
        res = (ch + res);
        num = (num / int64_t(10));
    }
    if (neg) {
...
std::string keys_to_string(const HashTable& table) {
    ...
    int64_t i = int64_t(0);
    while ((i < table.values.size())) {
        int64_t v = _index(table.values, i);
        if ((v != 0)) {
            if (!first) {
...
int main() {
    {
        ...
        qp = create_hash_table(int64_t(8));
