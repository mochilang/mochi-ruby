// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:22:48 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<int64_t> spans = std::vector<int64_t>{};

std::vector<int64_t> calculation_span(const std::vector<int64_t>& price) {
    int64_t n = price.size();
    ...
    for (int64_t i = int64_t(1); i < n; i++ ) {
        while (((st.size() > int64_t(0)) && (_index(price, _index(st, (st.size() - int64_t(1)))) <= _index(price, i)))) {
            st = _slice(st, int64_t(0), (st.size() - int64_t(1)));
        }
        int64_t s = ((st.size() <= int64_t(0)) ? (i + int64_t(1)) : (i - _index(st, (st.size() - int64_t(1)))));
...
void print_array(const std::vector<int64_t>& arr) {
    for (int64_t i = int64_t(0); i < arr.size(); i++ ) {
        std::cout << _index(arr, i);
        std::cout << std::endl;
    }
}
