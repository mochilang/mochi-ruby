// Generated by Mochi transpiler v0.10.73 on 2025-08-25 21:48:56 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <iomanip>
...
void send_file(std::string content, int64_t chunk_size) {
    ...
    while ((start < n)) {
        ...
        std::string chunk = ([&]{ std::string __s = content; long long __start = start; long long __end = end; if(__start < 0) __start = 0; if(__end < __start) __end = __start; if(__start > static_cast<long long>(__s.size())) __start = __s.size(); if(__end > static_cast<long long>(__s.size())) __end = __s.size(); return __s.substr(static_cast<size_t>(__start), static_cast<size_t>(__end - __start)); })();
        std::cout << chunk;
        std::cout << std::endl;
        start = end;
    }
