// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:23:52 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<double>> gabor_filter_kernel(int64_t ksize, double sigma, double theta, double lambd, double gamma, double psi) {
    ...
    while ((y < size)) {
        ...
        while ((x < size)) {
            ...
            double exponent = ((double)(-((x_rot * x_rot) + (((gamma * gamma) * y_rot) * y_rot))) / (double)(((2.0 * sigma) * sigma)));
            double value = (exp_taylor(exponent) * cos_taylor((((double)(((2.0 * PI) * x_rot)) / (double)(lambd)) + psi)));
...
int main() {
    {
        ...
        { auto __tmp1 = kernel; std::cout << "["; for(size_t i=0;i<__tmp1.size();++i){ if(i>0) std::cout << ' '; { std::ostringstream __ss;         { auto __tmp2 = __tmp1[static_cast<size_t>(i)]; __ss << "["; for(size_t i=0;i<__tmp2.size();++i){ if(i>0) __ss << ' '; __ss << __tmp2[i]; } __ss << "]"; }
