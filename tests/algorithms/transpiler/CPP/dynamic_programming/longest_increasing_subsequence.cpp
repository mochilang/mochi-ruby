// Generated by Mochi transpiler v0.10.72 on 2025-08-25 08:44:07 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<int64_t> longest_subsequence(const std::vector<int64_t>& xs) {
    ...
    int64_t pivot = _index(xs, int64_t(0));
    bool is_found = false;
    int64_t i = int64_t(1);
    ...
    while ((j < n)) {
        if ((_index(xs, j) >= pivot)) {
            filtered = ([&]{ auto __tmp = filtered; __tmp.push_back(_index(xs, j)); return __tmp; }());
        }
        j = (j + int64_t(1));
    }
