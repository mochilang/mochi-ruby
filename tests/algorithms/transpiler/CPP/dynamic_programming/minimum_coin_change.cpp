// Generated by Mochi transpiler v0.10.72 on 2025-08-25 08:44:51 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
int64_t dp_count(const std::vector<int64_t>& s, int64_t n) {
    ...
    _index(table, int64_t(0)) = int64_t(1);
    int64_t idx = int64_t(0);
    while ((idx < s.size())) {
        int64_t coin_val = _index(s, idx);
        int64_t j = coin_val;
        while ((j <= n)) {
            _index(table, j) = (_index(table, j) + _index(table, (j - coin_val)));
            j = (j + int64_t(1));
        }
        idx = (idx + int64_t(1));
    }
