// Generated by Mochi transpiler v0.10.63 on 2025-08-11 16:59:38 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::string to_lower_char(std::string c);
...
std::string to_lower_char(std::string c) {
    int64_t code = ord(c);
    if (((code >= int64_t(65)) && (code <= int64_t(90)))) {
...
bool is_alpha(std::string c) {
    int64_t code = ord(c);
    return (((code >= int64_t(65)) && (code <= int64_t(90))) || ((code >= int64_t(97)) && (code <= int64_t(122))));
}
...
bool is_isogram(std::string s) {
    ...
    while ((i < s.size())) {
        ...
        if (!is_alpha(ch)) {
            throw std::runtime_error(std::string("String must only contain alphabetic characters."));
        }
        std::string lower = to_lower_char(ch);
        if ((index_of(seen, lower) >= int64_t(0))) {
            return false;
        }
        seen = (seen + lower);
        i = (i + int64_t(1));
...
int main() {
    {
        ...
        std::cout << ([&]{ std::ostringstream ss; ss << std::boolalpha << is_isogram(std::string("Uncopyrightable")); return ss.str(); }());
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; ss << std::boolalpha << is_isogram(std::string("allowance")); return ss.str(); }());
        std::cout << std::endl;
    }
    return 0;
}
