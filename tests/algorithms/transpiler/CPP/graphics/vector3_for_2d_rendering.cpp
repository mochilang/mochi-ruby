// Generated by Mochi transpiler v0.10.73 on 2025-08-25 21:52:11 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
double modf(double x, double m) {
    return (x - (std::floor(((double)(x) / (double)(m))) * m));
}
...
std::vector<double> convert_to_2d(double x, double y, double z, double scale, double distance) {
    double projected_x = (((double)((x * distance)) / (double)((z + distance))) * scale);
    double projected_y = (((double)((y * distance)) / (double)((z + distance))) * scale);
    return std::vector<double>{projected_x, projected_y};
}

std::vector<double> rotate(double x, double y, double z, std::string axis, double angle) {
    angle = ((double)((((double)(modf(angle, 360.0)) / (double)(450.0)) * 180.0)) / (double)(PI));
...
int main() {
    {
        ...
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp1 = convert_to_2d(1.0, 2.0, 3.0, 10.0, 10.0); ss << "["; for(size_t i=0;i<__tmp1.size();++i){ if(i>0) ss << ' '; ss << __tmp1[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp2 = rotate(1.0, 2.0, 3.0, std::string("y"), 90.0); ss << "["; for(size_t i=0;i<__tmp2.size();++i){ if(i>0) ss << ' '; ss << __tmp2[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
    }
    return 0;
