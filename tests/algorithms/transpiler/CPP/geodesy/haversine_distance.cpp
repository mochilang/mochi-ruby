// Generated by Mochi transpiler v0.10.73 on 2025-08-25 21:51:38 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
double tan_approx(double x) {
    return ((double)(sin_taylor(x)) / (double)(cos_taylor(x)));
}
...
double atan2Approx(double y, double x) {
    if ((x > 0.0)) {
        double val = atanApprox(((double)(y) / (double)(x)));
        return val;
    }
    if ((x < 0.0)) {
        if ((y >= 0.0)) {
...
double haversine_distance(double lat1, double lon1, double lat2, double lon2) {
    double flattening = ((double)((AXIS_A - AXIS_B)) / (double)(AXIS_A));
    ...
    double sin_sq_phi = sin_taylor(((double)((phi_2 - phi_1)) / (double)(2.0)));
    double sin_sq_lambda = sin_taylor(((double)((lambda_2 - lambda_1)) / (double)(2.0)));
