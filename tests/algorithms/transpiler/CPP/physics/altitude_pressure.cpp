// Generated by Mochi transpiler v0.10.73 on 2025-08-26 08:46:34 GMT+7
#include <iostream>
#include <string>
#include <cmath>
#include <sstream>
...
double _ln(double x) {
    ...
    double y = ((double)((x - 1.0)) / (double)((x + 1.0)));
    double y2 = (y * y);
    double term = y;
    double sum = 0.0;
    int64_t k = int64_t(0);
    while ((k < int64_t(10))) {
        ...
        sum = (sum + ((double)(term) / (double)(denom)));
        term = (term * y2);
        k = (k + int64_t(1));
    }
    return (2.0 * sum);
...
double pow_float(double base, double exponent) {
    return std::exp((exponent * _ln(base)));
}
...
double get_altitude_at_pressure(double pressure) {
    ...
    double ratio = ((double)(pressure) / (double)(101325.0));
    return (44330.0 * (1.0 - pow_float(ratio, ((double)(1.0) / (double)(5.5255)))));
}
