...
let rec calculate_pi (limit: int) =
    let mutable __ret : string = Unchecked.defaultof<string>
    let mutable limit = limit
    try
        if (limit < 0) || (limit > (String.length (PI_DIGITS))) then
            failwith ("limit out of range")
        __ret <- "3." + (_substring PI_DIGITS 0 limit)
        raise Return
        __ret
    with
        | Return -> __ret
and test_pi_generator () =
    ...
    try
        if (calculate_pi (15)) <> "3.141592653589793" then
