...
let rec lower (word: string) =
    ...
    try
        ...
        while i < (String.length (word)) do
            ...
            let idx: int = index_of (upper) (c)
            if idx >= 0 then
                result <- result + (_substring lower_chars (idx) (idx + 1))
            else
                result <- result + c
            i <- i + 1
        __ret <- result
    ...
    with
        | Return -> __ret
printfn "%s" (lower ("wow"))
printfn "%s" (lower ("HellZo"))
printfn "%s" (lower ("WHAT"))
printfn "%s" (lower ("wh[]32"))
printfn "%s" (lower ("whAT"))
