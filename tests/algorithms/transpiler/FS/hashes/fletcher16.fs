...
let rec ord (ch: string) =
    ...
    with
        | Return -> __ret
let rec fletcher16 (text: string) =
    let mutable __ret : int = Unchecked.defaultof<int>
    let mutable text = text
    try
        let mutable sum1: int = 0
        let mutable sum2: int = 0
        let mutable i: int = 0
        while i < (String.length (text)) do
            let code: int = ord (string (text.[i]))
            sum1 <- (((sum1 + code) % 255 + 255) % 255)
            sum2 <- (((sum1 + sum2) % 255 + 255) % 255)
            i <- i + 1
        __ret <- (sum2 * 256) + sum1
        raise Return
        __ret
