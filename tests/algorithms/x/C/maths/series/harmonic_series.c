...
const char* * harmonic_series(double n_term);
int main(void);
...
const char* * harmonic_series(double n_term) {
    if (n_term <= 0.0) {
        {
            ...
            harmonic_series_len = 0;
            return __tmp1_data;
        }
    }
    long long limit = (int)(n_term);
    ...
    while (i < limit) {
        if (i == 0LL) {
            ...
            series = list_append_str(series, &series_len, str_concat("1/", str_int(i + 1LL)));
        }
        i = i + 1LL;
    }
    return harmonic_series_len = series_len, series;
}

int main(void) {
    {
        long long __start = _now();
        puts(({const char **__tmp2 = harmonic_series(5.0); char *__res = str_list_str(__tmp2, harmonic_series_len); __res;}));
