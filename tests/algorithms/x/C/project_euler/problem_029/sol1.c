...
long long solution(long long n) {
    ...
    size_t powers_len = 0;
    long long limit = n + 1LL;
    for (int a = 2LL; a < limit; a++) {
        for (int b = 2LL; b < limit; b++) {
            long long p = int_pow(a, b);
            if (!(contains_int(powers, powers_len, p))) {
                powers = list_append_long_long(powers, &powers_len, p);
            }
        }
    }
    return powers_len;
