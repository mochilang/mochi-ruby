...
-export([main/1, factorial/1, digit_sum/1, solution/1, main/0]).
...
digit_sum(N_2) ->
    try
        Total = 0,
        M = N_2,
        Fun_2 = fun Fun_2_loop(M, N_2, Total) ->
    case (M > 0) of
        true ->
            Total_2 = (Total + mochi_mod(M, 10)),
            M_2 = (M div 10),
            Fun_2_loop(M_2, N_2, Total_2);
        _ -> {M, N_2, Total}
    end
end,
{M_2, N_2, Total_2} = Fun_2(M, N_2, Total),
        Total_2
    catch {return, Ret} -> Ret end.
...
main() ->
    try
        ...
        io:format("~ts~n", [mochi_repr(solution(3))]),
