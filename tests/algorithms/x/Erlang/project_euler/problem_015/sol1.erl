...
-export([main/1, factorial/1, solution/1]).
...
solution(N_2) ->
    try
        Total = (2 * N_2),
        K = (Total div 2),
        (factorial(Total) div (factorial(K) * factorial((Total - K))))
    catch {return, Ret} -> Ret end.
...
main(_) ->
    ...
    io:format("~ts~n", [mochi_str(solution(25))]),
    io:format("~ts~n", [mochi_str(solution(23))]),
