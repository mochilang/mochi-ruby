...
NULL = 0 - 1
def empty_list():
    return LinkedList(next_=[], head=NULL)
def add_node(list_, value):
    nexts = list_.next_
    new_index = len(nexts)
    nexts = _append(nexts, NULL)
    if list_.head == NULL:
        return LinkedList(next_=nexts, head=new_index)
    last = list_.head
    ...
    while i < len(nexts):
        ...
        i = i + 1
    return LinkedList(next_=new_nexts, head=list_.head)
def set_next(list_, index, next_index):
    nexts = list_.next_
    new_nexts = []
    i = 0
    while i < len(nexts):
