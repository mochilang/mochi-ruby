...
defmodule Main do
  ...
  def create_state_space_tree(sequence, current, index, used) do
    try do
      if index == _len(sequence) do
        IO.puts(Kernel.to_string(current))
        throw {:return, nil}
      end
      i = 0
      while_fun_4 = fn while_fun_4, i ->
        if i < _len(sequence) do
          if !Enum.at(used, i) do
            next_current = (current ++ [Enum.at(sequence, i)])
  ...
  def generate_all_permutations(sequence) do
    try do
      used = repeat_bool(_len(sequence))
      create_state_space_tree(sequence, [], 0, used)
    catch
      {:return, val} -> val
    end
  end
  Process.put(:sequence, [3, 1, 2, 4])
  Process.put(:sequence_2, ["A", "B", "C"])
  def main() do
    generate_all_permutations(Process.get(:sequence))
    generate_all_permutations(Process.get(:sequence_2))
  end
end
Main.main()
