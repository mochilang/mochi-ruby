...
defmodule Main do
  ...
  def find_unique_number(arr) do
    try do
      if _len(arr) == 0 do
        raise("input list must not be empty")
      end
      result = 0
      {result} = Enum.reduce(arr, {result}, fn num, {result} ->
  ...
  def main() do
    ...
    mem_start = _mem()
    t_start = _bench_now()
    IO.puts(Kernel.inspect(find_unique_number([1, 1, 2, 2, 3])))
    IO.puts(Kernel.inspect(find_unique_number([4, 5, 4, 6, 6])))
    IO.puts(Kernel.inspect(find_unique_number([7])))
    mem_end = _mem()
