...
defmodule Main do
  ...
  def dencrypt(s, n) do
    try do
      ...
      while_fun_2 = fn while_fun_2, i, out ->
        if i < _len(s) do
          ch = _slice(s, i, i + 1 - (i))
          idx_u = index_of(Process.get(:uppercase), ch)
          {out} = if idx_u >= 0 do
            new_idx = rem((idx_u + n), 26)
            out = (out <> _slice(Process.get(:uppercase), new_idx, new_idx + 1 - (new_idx)))
            {out}
          else
            idx_l = index_of(Process.get(:lowercase), ch)
            {out} = if idx_l >= 0 do
              new_idx = rem((idx_l + n), 26)
              out = (out <> _slice(Process.get(:lowercase), new_idx, new_idx + 1 - (new_idx)))
              {out}
            else
              out = (out <> ch)
              {out}
  ...
  def main() do
    try do
      ...
      s = dencrypt(msg, 13)
      IO.puts(s)
      IO.puts(Kernel.inspect(dencrypt(s, 13) == msg))
    catch
      {:return, val} -> val
    end
  end
