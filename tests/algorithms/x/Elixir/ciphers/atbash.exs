...
defmodule Main do
    ...
    try do
      ...
      while_fun_2 = fn while_fun_2, i, result ->
        if i < _len(sequence) do
          ch = String.at(sequence, i)
          idx = index_of(lower, ch)
          {result} = if idx != (-1) do
            result = (result <> String.at(lower_rev, idx))
            {result}
          else
            idx2 = index_of(upper, ch)
            result = (if idx2 != (-1), do: (result <> String.at(upper_rev, idx2)), else: (result <> ch))
            {result}
          end
          i = i + 1
  ...
  def main() do
    ...
    mem_start = _mem()
    t_start = _bench_now()
    IO.puts(Kernel.inspect(atbash("ABCDEFGH")))
    IO.puts(Kernel.inspect(atbash("123GGjj")))
    IO.puts(Kernel.inspect(atbash("testStringtest")))
    IO.puts(Kernel.inspect(atbash("with space")))
    mem_end = _mem()
