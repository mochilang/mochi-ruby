...
defmodule Main do
  ...
  def encrypt(input_string, key, alphabet) do
    try do
      result = ""
      i = 0
      n = _len(alphabet)
      while_fun_2 = fn while_fun_2, i, result ->
        if i < _len(input_string) do
          ch = _slice(input_string, i, i + 1 - (i))
          idx = index_of(alphabet, ch)
          {result} = if idx < 0 do
            result = (result <> ch)
            {result}
          else
            new_key = rem((idx + key), n)
            {new_key} = if new_key < 0 do
              new_key = new_key + n
              {new_key}
            else
              {new_key}
            end
            result = (result <> _slice(alphabet, new_key, new_key + 1 - (new_key)))
            {result}
          end
          i = i + 1
  ...
  def decrypt(input_string, key, alphabet) do
    try do
      result = ""
      i = 0
      n = _len(alphabet)
  ...
  def brute_force(input_string, alphabet) do
    try do
      results = []
      key = 1
      n = _len(alphabet)
      while_fun_4 = fn while_fun_4, key, results ->
        if key <= n do
          message = decrypt(input_string, key, alphabet)
          results = (results ++ [message])
          key = key + 1
          while_fun_4.(while_fun_4, key, results)
        else
          {key, results}
        end
      end
  ...
  def main() do
    try do
      alpha = Process.get(:default_alphabet)
      ...
      IO.puts(Enum.at(brute, 19))
    catch
      {:return, val} -> val
    end
  end
  Process.put(:default_alphabet, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
  def bench_main() do
    Process.put(:default_alphabet, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
