...
defmodule Main do
  ...
  def find_mod_inverse(a, m) do
    try do
      if gcd(a, m) != 1 do
        error((((("mod inverse of " <> Kernel.to_string(a)) <> " and ") <> Kernel.to_string(m)) <> " does not exist"))
      end
      u1 = 1
      u2 = 0
      u3 = a
  ...
  def main() do
    ...
    mem_start = _mem()
    t_start = _bench_now()
    IO.puts(Kernel.inspect(find_mod_inverse(3, 11)))
    IO.puts(Kernel.inspect(find_mod_inverse(7, 26)))
    IO.puts(Kernel.inspect(find_mod_inverse(11, 26)))
    IO.puts(Kernel.inspect(find_mod_inverse(17, 43)))
    mem_end = _mem()
