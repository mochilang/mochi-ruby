...
defmodule Main do
  ...
  def xnor_gate(a, b) do
    try do
      throw {:return, ((if a == b, do: 1, else: 0))}
    catch
      {:return, val} -> val
    end
  end
  def main() do
    IO.puts(Kernel.inspect(xnor_gate(0, 0)))
    IO.puts(Kernel.inspect(xnor_gate(0, 1)))
    IO.puts(Kernel.inspect(xnor_gate(1, 0)))
    IO.puts(Kernel.inspect(xnor_gate(1, 1)))
  end
end
Main.main()
