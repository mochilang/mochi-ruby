...
and find_unique_number arr =
  ...
  (try List.iter (fun num ->
    try
  result := bit_xor (Obj.repr (!result)) (Obj.repr (num));
...
let () =
  ...
  let start = _now () in
  print_endline ((string_of_int (find_unique_number ([1; 1; 2; 2; 3]))));
  print_endline ((string_of_int (find_unique_number ([4; 5; 4; 6; 6]))));
  print_endline ((string_of_int (find_unique_number ([7]))));
  let finish = _now () in
  let mem_end = _mem () in
