...
let rec shear_stress stress tangential_force area =
  ...
  if (stress = 0.0) then (
  __ret := (Obj.magic ([(__str (Obj.repr ("name")), Obj.repr ("stress")); (__str (Obj.repr ("value")), Obj.repr ((tangential_force /. area)))]) : (string * Obj.t) list); raise Return
  ) else (
  if (tangential_force = 0.0) then (
  __ret := (Obj.magic ([(__str (Obj.repr ("name")), Obj.repr ("tangential_force")); (__str (Obj.repr ("value")), Obj.repr ((stress *. area)))]) : (string * Obj.t) list); raise Return
  ) else (
  __ret := (Obj.magic ([(__str (Obj.repr ("name")), Obj.repr ("area")); (__str (Obj.repr ("value")), Obj.repr ((tangential_force /. stress)))]) : (string * Obj.t) list); raise Return
  );
  );
  );
  );
...
and str_result r =
  ...
  __ret := (Obj.magic ((((("Result(name='" ^ (match List.assoc_opt (__str (Obj.repr ("name"))) (r) with Some v -> (Obj.obj (v : Obj.t) : string) | None -> "")) ^ "', value=") ^ (Printf.sprintf "%.16g" (Obj.magic ((match List.assoc_opt (__str (Obj.repr ("value"))) (r) with Some v -> (Obj.obj (v : Obj.t) : float) | None -> 0.)) : float))) ^ ")")) : string); raise Return
  with Return -> !__ret)
