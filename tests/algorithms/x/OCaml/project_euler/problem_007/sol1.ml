...
and is_prime number =
  ...
  let limit = isqrt (Obj.repr (number)) in
  let i = ref (5) in
...
and solution nth =
  ...
  let count = ref (0) in
  let number = ref (1) in
  (try while ((!count <> nth) && (!number < 3)) do
    try
  number := (!number + 1);
...
and main () =
  ...
  (try
  print_endline (("solution() = " ^ (string_of_int (Obj.magic (solution (Obj.repr (10001))) : int))));
    !__ret
  with Return -> !__ret)
