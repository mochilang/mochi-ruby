...
and find_max_iterative nums =
  ...
  let max_num = ref ((let __l = nums in let __i = 0 in if __i < 0 then 0. else match List.nth_opt __l __i with Some v -> (Obj.magic v : float) | None -> 0.)) in
  let i = ref (0) in
  (try while (!i < List.length (nums)) do
    try
  let x = (let __l = nums in let __i = !i in if __i < 0 then 0. else match List.nth_opt __l __i with Some v -> (Obj.magic v : float) | None -> 0.) in
  if (x > !max_num) then (
  max_num := x;
  );
  i := (!i + 1);
