...
let rec triangular_number position =
  let __ret = ref 0 in
  (try
  ...
  __ret := (Obj.magic (((position * (position + 1)) / 2)) : int); raise Return
  with Return -> !__ret)

and test_triangular_number () =
  let __ret = ref (Obj.magic 0) in
  (try
  if (triangular_number (Obj.repr (1)) <> 1) then (
  (failwith ("triangular_number(1) failed"));
  );
  if (triangular_number (Obj.repr (3)) <> 6) then (
...
and main () =
  ...
  print_endline (string_of_int (triangular_number (Obj.repr (10))));
    !__ret
  with Return -> !__ret)
