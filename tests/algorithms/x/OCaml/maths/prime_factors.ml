...
let rec prime_factors n =
  ...
  if (!num > 1) then (
  factors := (Obj.magic (List.append (!factors) [(Obj.magic (!num) : int)]) : int list);
  );
...
and test_prime_factors () =
  ...
  (try
  if not (list_eq (prime_factors (Obj.repr (0))) ((Obj.magic [] : int list))) then (
  (failwith ("prime_factors(0) failed"));
  );
  if not (list_eq (prime_factors (Obj.repr (100))) ([2; 2; 5; 5])) then (
  (failwith ("prime_factors(100) failed"));
  );
  if not (list_eq (prime_factors (Obj.repr (2560))) ([2; 2; 2; 2; 2; 2; 2; 2; 2; 5])) then (
  (failwith ("prime_factors(2560) failed"));
  );
  if not (list_eq (prime_factors (Obj.repr (97))) ([97])) then (
...
and main () =
  ...
  print_endline ((__str (prime_factors (Obj.repr (100)))));
  print_endline ((__str (prime_factors (Obj.repr (2560)))));
  print_endline ((__str (prime_factors (Obj.repr (97)))));
    !__ret
  with Return -> !__ret)
