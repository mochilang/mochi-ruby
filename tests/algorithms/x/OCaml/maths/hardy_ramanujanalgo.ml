...
and main () =
  ...
  print_endline (("The number of distinct prime factors is/are " ^ (string_of_int (Obj.magic (count) : int))));
  let loglog = ln (Obj.repr (ln (Obj.repr (float_of_int (n))))) in
  print_endline (("The value of log(log(n)) is " ^ (Printf.sprintf "%.16g" (Obj.magic (round4 (Obj.repr (loglog))) : float))));
    !__ret
  with Return -> !__ret)
