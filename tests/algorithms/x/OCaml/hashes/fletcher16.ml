...
let rec ord ch =
  ...
  (try while (!i < String.length (ascii_chars)) do
    try
  if ((let __s = ascii_chars in let __i = !i in let __len = String.length __s in String.make 1 (String.get __s (if __i >= 0 then __i else __len + __i))) = ch) then (
...
and fletcher16 text =
  ...
  sum1 := (((!sum1 + code) mod 255 + 255) mod 255);
  sum2 := (((!sum1 + !sum2) mod 255 + 255) mod 255);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  __ret := (Obj.magic (((!sum2 * 256) + !sum1)) : int); raise Return
  with Return -> !__ret)
