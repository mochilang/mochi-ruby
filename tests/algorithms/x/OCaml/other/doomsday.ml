...
let _week_day_names = ref ([(__str (Obj.repr (0)), "Sunday"); (__str (Obj.repr (1)), "Monday"); (__str (Obj.repr (2)), "Tuesday"); (__str (Obj.repr (3)), "Wednesday"); (__str (Obj.repr (4)), "Thursday"); (__str (Obj.repr (5)), "Friday"); (__str (Obj.repr (6)), "Saturday")])
let rec get_week_day year month day =
  ...
  let day_anchor = if ((((year mod 4 + 4) mod 4) <> 0) || ((centurian = 0) && (((year mod 400 + 400) mod 400) <> 0))) then (let __l = !_doomsday_not_leap in let __i = (month - 1) in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) else (let __l = !_doomsday_leap in let __i = (month - 1) in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) in
  let week_day = ref (((((dooms_day + day) - day_anchor) mod 7 + 7) mod 7)) in
  if (!week_day < 0) then (
  week_day := (!week_day + 7);
  );
  __ret := (Obj.magic ((match List.assoc_opt (__str (Obj.repr (!week_day))) (!_week_day_names) with Some v -> (Obj.magic v : string) | None -> "")) : string); raise Return
  with Return -> !__ret)
