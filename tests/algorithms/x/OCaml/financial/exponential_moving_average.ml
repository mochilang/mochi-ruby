...
let rec exponential_moving_average stock_prices window_size =
  ...
  let alpha = (2.0 /. (1.0 +. float_of_int (window_size))) in
  let moving_average = ref (0.0) in
  let result = ref (([] : (float) list)) in
  let i = ref (0) in
  (try while (!i < List.length (stock_prices)) do
    try
  let price = List.nth (stock_prices) (!i) in
  ...
  result := (List.append (!result) [(Obj.magic (!moving_average) : float)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let stock_prices = ref ([2.0; 5.0; 3.0; 8.2; 6.0; 9.0; 10.0])
let window_size = 3
let result = ref (exponential_moving_average (!stock_prices) (Obj.repr (window_size)))
let () =
