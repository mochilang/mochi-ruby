...
and append_value list value =
  let __ret = ref ([] : (string * Obj.t) list) in
  (try
  let value = (Obj.magic value : int) in
  let d = ref ((Obj.obj (List.assoc (__str ("data")) (list) : Obj.t) : int list)) in
  d := (List.append (!d) [(Obj.magic (value) : int)]);
  __ret := (Obj.magic ([(__str ("data"), Obj.repr (!d))]) : (string * Obj.t) list); raise Return
  with Return -> !__ret)

and extend_list list items =
  let __ret = ref ([] : (string * Obj.t) list) in
  (try
  ...
  (try while (!i < List.length (items)) do
    try
  result := append_value ((Obj.magic !result : (string * Obj.t) list)) (Obj.repr (List.nth (items) (!i)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
and make_linked_list items =
  ...
  __ret := (Obj.magic (!ll) : (string * Obj.t) list); raise Return
  with Return -> !__ret)
...
and in_reverse list =
  ...
  let s = ref ((string_of_int (List.nth ((Obj.obj (List.assoc (__str ("data")) (list) : Obj.t) : int list)) (!i)))) in
  i := (!i - 1);
  (try while (!i >= 0) do
    try
  s := ((!s ^ " <- ") ^ (string_of_int (List.nth ((Obj.obj (List.assoc (__str ("data")) (list) : Obj.t) : int list)) (!i))));
...
and main () =
  ...
  (try
  let linked_list = make_linked_list ([14; 52; 14; 12; 43]) in
  print_endline (("Linked List:  " ^ to_string (linked_list)));
  print_endline (("Reverse List: " ^ in_reverse (linked_list)));
    !__ret
  with Return -> !__ret)
