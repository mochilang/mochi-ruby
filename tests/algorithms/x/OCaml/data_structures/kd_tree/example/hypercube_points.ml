...
and hypercube_points num_points hypercube_size num_dimensions =
  ...
  point := (Obj.magic ((List.append (!point) [(Obj.magic (value) : float)])) : float list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  points := (Obj.magic ((List.append (!points) [(Obj.magic (!point) : float list)])) : float list list);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let () =
  ...
  print_endline (__show (Obj.repr !pts));
