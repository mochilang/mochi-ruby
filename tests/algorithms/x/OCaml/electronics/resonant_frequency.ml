...
and resonant_frequency inductance capacitance =
  let __ret = ref 0.0 in
  (try
  ...
  let denom = ((2.0 *. _pi) *. sqrtApprox (Obj.repr ((inductance *. capacitance)))) in
  __ret := (Obj.magic ((1.0 /. denom)) : float); raise Return
  with Return -> !__ret)
...
let () =
  ...
  let start = _now () in
  print_endline (string_of_float (resonant_frequency (Obj.repr (10.0)) (Obj.repr (5.0))));
  let finish = _now () in
  let mem_end = _mem () in
