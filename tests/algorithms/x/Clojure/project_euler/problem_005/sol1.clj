(ns main (:refer-clojure :exclude [gcd lcm solution]))
...
(defn lcm [lcm_a lcm_b]
  (try (throw (ex-info "return" {:v (* (/ lcm_a (gcd lcm_a lcm_b)) lcm_b)})) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e)))))

(defn solution [solution_n]
  (binding [solution_i nil solution_result nil] (try (do (when (<= solution_n 0) (throw (Exception. "Parameter n must be greater than or equal to one."))) (set! solution_result 1) (set! solution_i 2) (while (<= solution_i solution_n) (do (set! solution_result (lcm solution_result solution_i)) (set! solution_i (+ solution_i 1)))) (throw (ex-info "return" {:v solution_result}))) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e))))))
