...
(define (solution limit)
  (let/ec _return (begin
(if (let ([__l limit] [__r 2]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(_return 1)
) (void))
(define result 1)
(define p 2)
(let/ec _break (let loop ()
  (if (let ([__l (* result p)] [__r limit]) (cond [(and (number? __l) (number? __r)) (<= __l __r)] [else (<= (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (is_prime p) (let ()
(set! result (* result p))
) (void))
