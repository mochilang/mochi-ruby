...
(displayln (to-string "started..."))
(define last_lowest lower_)
(define last_highest higher)
(define last_numbers (vector))
(let/ec _break (let loop ()
  (if #t (begin
    (let/ec _cont
      (define number (get_avg last_lowest last_highest))
      (set! last_numbers (appendv (or last_numbers (vector)) (vector number)))
...
(_break)
    ...
    (loop)) (void))))
(displayln (to-string (string-append "guess the number : " (to-string (if last_numbers (if (hash? last_numbers) (hash-ref last_numbers (- (cond [(not last_numbers) 0] [(string? last_numbers) (string-length last_numbers)] [(vector? last_numbers) (vector-length last_numbers)] [(hash? last_numbers) (hash-count last_numbers)] [else (length last_numbers)]) 1) #f) (safe-index last_numbers (int (- (cond [(not last_numbers) 0] [(string? last_numbers) (string-length last_numbers)] [(vector? last_numbers) (vector-length last_numbers)] [(hash? last_numbers) (hash-count last_numbers)] [else (length last_numbers)]) 1)))) #f)))))
(displayln (to-string (string-append "details : " (to-string last_numbers))))
