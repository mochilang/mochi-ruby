...
(set! current (floor (sqrt (to_float current))))
) (let ()
(define r (sqrt (to_float current)))
(set! current (floor (* (* r r) r)))
))
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (format "~a" (juggler_sequence 3)))
(displayln (format "~a" (juggler_sequence 10)))
    (void)
  ))
