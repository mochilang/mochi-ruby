...
(define (sqrt_to_complex d)
  (let/ec _return (begin
(if (let ([__l d] [__r 0.0]) (cond [(and (number? __l) (number? __r)) (>= __l __r)] [else (>= (int __l) (int __r))])) (let ()
...
(if (let ([__l (if r (hash-ref r "im" #f) #f)] [__r 0.0]) (cond [(and (number? __l) (number? __r)) (>= __l __r)] [else (>= (int __l) (int __r))])) (let ()
...
(displayln (string-append (string-append (string-append "The solutions are: " (root_str (if roots (if (hash? roots) (hash-ref roots 0 #f) (safe-index roots (int 0))) #f))) " and ") (root_str (if roots (if (hash? roots) (hash-ref roots 1 #f) (safe-index roots (int 1))) #f))))
) (void))
))
)
