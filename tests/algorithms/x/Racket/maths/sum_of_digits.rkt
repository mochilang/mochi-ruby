...
(define (sum_of_digits_recursion n)
  (let/ec _return (begin
(define m (abs_int n))
(if (let ([__l m] [__r 10]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(_return m)
) (void))
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not s) 0] [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (set! res (let ([__l res] [__r (int (if (hash? s) (hash-ref s i #f) (substring s (int i) (+ (int i) 1))))]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
