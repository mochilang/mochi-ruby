...
(define (gaussian x mu sigma)
  (let/ec _return (begin
(define coeff (/ 1.0 (sqrtApprox (* (* (* 2.0 PI) sigma) sigma))))
(define exponent (/ (- (* (- x mu) (- x mu))) (* (* 2.0 sigma) sigma)))
(_return (* coeff (expApprox exponent)))
))
)
(define (main)
  (let/ec _return (begin
(define result (gaussian 1.0 0.0 1.0))
(displayln result)
))
)
