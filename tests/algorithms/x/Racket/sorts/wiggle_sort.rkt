...
(let/ec _break (let loop ()
  (if (let ([__l k] [__r (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (equal? k i) (let ()
(set! res (appendv (or res (vector)) (vector (if xs (if (hash? xs) (hash-ref xs j #f) (safe-index xs (int j))) #f))))
) (let ()
(if (equal? k j) (let ()
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not res) 0] [(string? res) (string-length res)] [(vector? res) (vector-length res)] [(hash? res) (hash-count res)] [else (length res)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define j (if (equal? i 0) (- (cond [(not res) 0] [(string? res) (string-length res)] [(vector? res) (vector-length res)] [(hash? res) (hash-count res)] [else (length res)]) 1) (- i 1)))
      (define prev (if res (if (hash? res) (hash-ref res j #f) (safe-index res (int j))) #f))
      (define curr (if res (if (hash? res) (hash-ref res i #f) (safe-index res (int i))) #f))
      (if (equal? (equal? (modulo i 2) 1) (let ([__l prev] [__r curr]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))]))) (let ()
(set! res (swap res j i))
) (void))
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (wiggle_sort (vector 3.0 5.0 2.0 1.0 6.0 4.0))))
(displayln (to-string (wiggle_sort (vector 0.0 5.0 3.0 2.0 2.0))))
(displayln (to-string (wiggle_sort (vector))))
(displayln (to-string (wiggle_sort (vector (- 2.0) (- 5.0) (- 45.0)))))
(displayln (to-string (wiggle_sort (vector (- 2.1) (- 5.68) (- 45.11)))))
    (void)
  ))
