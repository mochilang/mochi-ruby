...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define value (if xs (if (hash? xs) (hash-ref xs i #f) (safe-index xs (int i))) #f))
      (define j (- i 1))
      (let/ec _break (let loop ()
  (if (and (let ([__l j] [__r 0]) (cond [(and (number? __l) (number? __r)) (>= __l __r)] [else (>= (int __l) (int __r))])) (let ([__l (if xs (if (hash? xs) (hash-ref xs j #f) (safe-index xs (int j))) #f)] [__r value]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))]))) (begin
    (let/ec _cont
      (vector-set! xs (int (let ([__l j] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))) (if xs (if (hash? xs) (hash-ref xs j #f) (safe-index xs (int j))) #f))
      (set! j (- j 1))
    )
    (loop)) (void))))
      (vector-set! xs (int (let ([__l j] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))) value)
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (insertion_sort (vector 0 5 3 2 2))))
(displayln (to-string (insertion_sort (vector))))
(displayln (to-string (insertion_sort (vector (- 2) (- 5) (- 45)))))
(displayln (to-string (insertion_sort (vector 3))))
    (void)
  ))
