...
(define edges (let ([h (make-hash)]) (hash-set! h "a" (vector "c" "b")) (hash-set! h "b" (vector "d" "e")) (hash-set! h "c" (vector)) (hash-set! h "d" (vector)) (hash-set! h "e" (vector)) h))
(define vertices (vector "a" "b" "c" "d" "e"))
(define (topological_sort start visited sort)
  (let/ec _return (begin
(if (hash? visited) (hash-set! visited start #t) (begin (set! visited (make-hash)) (hash-set! visited start #t)))
(define neighbors (if edges (hash-ref edges start (vector)) #f))
(define i 0)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not neighbors) 0] [(string? neighbors) (string-length neighbors)] [(vector? neighbors) (vector-length neighbors)] [(hash? neighbors) (hash-count neighbors)] [else (length neighbors)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
...
(set! sort (topological_sort neighbor visited sort))
) (void))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
(set! sort (appendv (or sort (vector)) (vector start)))
(if (not (equal? (cond [(not visited) 0] [(string? visited) (string-length visited)] [(vector? visited) (vector-length visited)] [(hash? visited) (hash-count visited)] [else (length visited)]) (cond [(not vertices) 0] [(string? vertices) (string-length vertices)] [(vector? vertices) (vector-length vertices)] [(hash? vertices) (hash-count vertices)] [else (length vertices)]))) (let ()
(define j 0)
(let/ec _break (let loop ()
  (if (let ([__l j] [__r (cond [(not vertices) 0] [(string? vertices) (string-length vertices)] [(vector? vertices) (vector-length vertices)] [(hash? vertices) (hash-count vertices)] [else (length vertices)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define v (if vertices (if (hash? vertices) (hash-ref vertices j #f) (safe-index vertices (int j))) #f))
      (if (not (hash-has-key? visited v)) (let ()
(set! sort (topological_sort v visited sort))
) (void))
...
(define (main)
  (let/ec _return (begin
(define result (topological_sort "a" (let ([h (make-hash)]) h) (vector)))
