...
(let/ec _break (let loop ()
  ...
  (if (let ([__l k] [__r n]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (let ([__l (if arr (if (hash? arr) (hash-ref arr k #f) (safe-index arr (int k))) #f)] [__r (if arr (if (hash? arr) (hash-ref arr min_index #f) (safe-index arr (int min_index))) #f)]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(set! min_index k)
) (void))
      (set! k (let ([__l k] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
      (if (not (equal? min_index i)) (let ()
(define tmp (if arr (if (hash? arr) (hash-ref arr i #f) (safe-index arr (int i))) #f))
(vector-set! arr (int i) (if arr (if (hash? arr) (hash-ref arr min_index #f) (safe-index arr (int min_index))) #f))
(vector-set! arr (int min_index) tmp)
) (void))
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (selection_sort (vector 0 5 3 2 2))))
(displayln (to-string (selection_sort (vector))))
(displayln (to-string (selection_sort (vector (- 2) (- 5) (- 45)))))
    (void)
  ))
