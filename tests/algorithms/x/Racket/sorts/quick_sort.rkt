...
(define (quick_sort items)
  (let/ec _return (begin
(if (let ([__l (cond [(not items) 0] [(string? items) (string-length items)] [(vector? items) (vector-length items)] [(hash? items) (hash-count items)] [else (length items)])] [__r 2]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(_return items)
) (void))
(define pivot (if items (if (hash? items) (hash-ref items 0 #f) (safe-index items (int 0))) #f))
(define lesser (vector))
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not items) 0] [(string? items) (string-length items)] [(vector? items) (vector-length items)] [(hash? items) (hash-count items)] [else (length items)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define item (if items (if (hash? items) (hash-ref items i #f) (safe-index items (int i))) #f))
      (if (let ([__l item] [__r pivot]) (cond [(and (number? __l) (number? __r)) (<= __l __r)] [else (<= (int __l) (int __r))])) (let ()
(set! lesser (appendv (or lesser (vector)) (vector item)))
) (let ()
(set! greater (appendv (or greater (vector)) (vector item)))
))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
(_return (append (append (quick_sort lesser) (vector pivot)) (quick_sort greater)))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (string-join (map (lambda (x) (to-string x)) (list "sorted1:" (quick_sort (vector 0 5 3 2 2)))) " "))
(displayln (string-join (map (lambda (x) (to-string x)) (list "sorted2:" (quick_sort (vector)))) " "))
(displayln (string-join (map (lambda (x) (to-string x)) (list "sorted3:" (quick_sort (vector (- 2) 5 0 (- 45))))) " "))
    (void)
  ))
