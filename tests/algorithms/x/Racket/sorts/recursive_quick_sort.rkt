...
(let ([__seq a]) (let/ec _break (for ([x (if (hash? __seq) (in-hash-keys __seq) __seq)])
  (let/ec _cont
(set! result (appendv (or result (vector)) (vector x)))
  ))))
(let ([__seq b]) (let/ec _break (for ([x (if (hash? __seq) (in-hash-keys __seq) __seq)])
  (let/ec _cont
(set! result (appendv (or result (vector)) (vector x)))
  ))))
(_return result)
...
(define (quick_sort data)
  (let/ec _return (begin
(if (let ([__l (cond [(not data) 0] [(string? data) (string-length data)] [(vector? data) (vector-length data)] [(hash? data) (hash-count data)] [else (length data)])] [__r 1]) (cond [(and (number? __l) (number? __r)) (<= __l __r)] [else (<= (int __l) (int __r))])) (let ()
(_return data)
) (void))
(define pivot (if data (if (hash? data) (hash-ref data 0 #f) (safe-index data (int 0))) #f))
(define left (vector))
(define right (vector))
(define i 1)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not data) 0] [(string? data) (string-length data)] [(vector? data) (vector-length data)] [(hash? data) (hash-count data)] [else (length data)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define e (if data (if (hash? data) (hash-ref data i #f) (safe-index data (int i))) #f))
      (if (let ([__l e] [__r pivot]) (cond [(and (number? __l) (number? __r)) (<= __l __r)] [else (<= (int __l) (int __r))])) (let ()
(set! left (appendv (or left (vector)) (vector e)))
) (let ()
(set! right (appendv (or right (vector)) (vector e)))
))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
(define sorted_left (quick_sort left))
(define sorted_right (quick_sort right))
(define left_pivot (appendv (or sorted_left (vector)) (vector pivot)))
(_return (concat left_pivot sorted_right))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (quick_sort (vector 2 1 0))))
(displayln (to-string (quick_sort (vector 3 5 2 4 1))))
    (void)
  ))
