...
(define i 1)
(define m (if xs (if (hash? xs) (hash-ref xs 0 #f) (safe-index xs (int 0))) #f))
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (let ([__l (if xs (if (hash? xs) (hash-ref xs i #f) (safe-index xs (int i))) #f)] [__r m]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(set! m (if xs (if (hash? xs) (hash-ref xs i #f) (safe-index xs (int i))) #f))
) (void))
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (and (not removed) (equal? (if xs (if (hash? xs) (hash-ref xs i #f) (safe-index xs (int i))) #f) value)) (let ()
(set! removed #t)
) (let ()
...
(define (reverse_list xs)
  (let/ec _return (begin
(define res (vector))
(define i (- (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)]) 1))
(let/ec _break (let loop ()
...
(let/ec _break (let loop ()
  (if (let ([__l (cond [(not coll) 0] [(string? coll) (string-length coll)] [(vector? coll) (vector-length coll)] [(hash? coll) (hash-count coll)] [else (length coll)])] [__r 1]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))])) (begin
    (let/ec _cont
      (define mn (list_min coll))
      (define mx (list_max coll))
      (set! start (appendv (or start (vector)) (vector mn)))
      (set! end (appendv (or end (vector)) (vector mx)))
      (set! coll (remove_once coll mn))
      (set! coll (remove_once coll mx))
    )
    (loop)) (void))))
(set! end (reverse_list end))
(_return (append (append start coll) end))
))
)
(define (test_merge_sort)
  (let/ec _return (begin
(if (not (equal? (merge_sort (vector 0 5 3 2 2)) (vector 0 2 2 3 5))) (let ()
(panic "case1 failed")
) (void))
(if (not (equal? (merge_sort (vector)) (vector))) (let ()
(panic "case2 failed")
) (void))
(if (not (equal? (merge_sort (vector (- 2) (- 5) (- 45))) (vector (- 45) (- 5) (- 2)))) (let ()
(panic "case3 failed")
) (void))
))
)
