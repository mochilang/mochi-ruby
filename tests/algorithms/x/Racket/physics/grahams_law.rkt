...
(define (validate values)
  (let/ec _return (begin
(if (equal? (cond [(not values) 0] [(string? values) (string-length values)] [(vector? values) (vector-length values)] [(hash? values) (hash-count values)] [else (length values)]) 0) (let ()
(_return #f)
) (void))
(define i 0)
(let/ec _break (let loop ()
...
(define (effusion_ratio m1 m2)
  (let/ec _return (begin
(if (not (validate (vector m1 m2))) (let ()
(displayln (to-string "ValueError: Molar mass values must greater than 0."))
(_return 0.0)
) (void))
...
(define (first_effusion_rate rate m1 m2)
  (let/ec _return (begin
(if (not (validate (vector rate m1 m2))) (let ()
(displayln (to-string "ValueError: Molar mass and effusion rate values must greater than 0."))
(_return 0.0)
) (void))
...
(define (first_molar_mass mass r1 r2)
  (let/ec _return (begin
(if (not (validate (vector mass r1 r2))) (let ()
(displayln (to-string "ValueError: Molar mass and effusion rate values must greater than 0."))
(_return 0.0)
) (void))
(define ratio (/ r1 r2))
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (effusion_ratio 2.016 4.002)))
(displayln (to-string (first_effusion_rate 1.0 2.016 4.002)))
(displayln (to-string (second_effusion_rate 1.0 2.016 4.002)))
(displayln (to-string (first_molar_mass 2.0 1.408943 0.709752)))
(displayln (to-string (second_molar_mass 2.0 1.408943 0.709752)))
    (void)
  ))
