...
(define _data_dir "tests/github/TheAlgorithms/Mochi/data_structures/suffix_tree/example")
(define (now)
  (if nowSeed
...
(_return (let ([h (make-hash)]) (hash-set! h "text" text) h))
))
)
(define (search tree pattern)
  (let/ec _return (begin
(define n (cond [(not (if tree (hash-ref tree "text" #f) #f)) 0] [(string? (if tree (hash-ref tree "text" #f) #f)) (string-length (if tree (hash-ref tree "text" #f) #f))] [(vector? (if tree (hash-ref tree "text" #f) #f)) (vector-length (if tree (hash-ref tree "text" #f) #f))] [(hash? (if tree (hash-ref tree "text" #f) #f)) (hash-count (if tree (hash-ref tree "text" #f) #f))] [else (length (if tree (hash-ref tree "text" #f) #f))]))
(define m (cond [(not pattern) 0] [(string? pattern) (string-length pattern)] [(vector? pattern) (vector-length pattern)] [(hash? pattern) (hash-count pattern)] [else (length pattern)]))
(if (equal? m 0) (let ()
(_return #t)
) (void))
(if (let ([__l m] [__r n]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))])) (let ()
(_return #f)
) (void))
(define i 0)
...
(define patterns (vector "ana" "ban" "na" "xyz" "mon"))
(define i 0)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not patterns) 0] [(string? patterns) (string-length patterns)] [(vector? patterns) (vector-length patterns)] [(hash? patterns) (hash-count patterns)] [else (length patterns)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define pattern (if patterns (if (hash? patterns) (hash-ref patterns i #f) (safe-index patterns (int i))) #f))
      (define found (search suffix_tree pattern))
      (displayln (to-string (string-append (string-append (string-append "Pattern '" pattern) "' found: ") (to-string found))))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
))
