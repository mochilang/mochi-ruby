...
(_return (let ([h (make-hash)]) (hash-set! h "children" children) (hash-set! h "is_end_of_string" is_end_of_string) (hash-set! h "start" start) (hash-set! h "end" end) (hash-set! h "suffix_link" suffix_link) h))
))
)
(define (empty_suffix_tree_node)
  (let/ec _return (begin
(_return (new_suffix_tree_node (let ([h (make-hash)]) h) #f (- 0 1) (- 0 1) (- 0 1)))
))
)
(define (has_key m k)
  (let/ec _return (begin
...
(define root (new_suffix_tree_node (let ([h (make-hash)]) (hash-set! h "a" 1) h) #f (- 0 1) (- 0 1) (- 0 1)))
(define leaf (new_suffix_tree_node (let ([h (make-hash)]) h) #t 0 2 0))
(define nodes (vector root leaf))
(define root_check (if nodes (if (hash? nodes) (hash-ref nodes 0 #f) (safe-index nodes (int 0))) #f))
(define leaf_check (if nodes (if (hash? nodes) (hash-ref nodes 1 #f) (safe-index nodes (int 1))) #f))
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (to-string (has_key (if root_check (hash-ref root_check "children" #f) #f) "a"))))
(displayln (to-string (to-string (if leaf_check (hash-ref leaf_check "is_end_of_string" #f) #f))))
(displayln (to-string (to-string (if leaf_check (hash-ref leaf_check "start" #f) #f))))
(displayln (to-string (to-string (if leaf_check (hash-ref leaf_check "end" #f) #f))))
(displayln (to-string (to-string (if leaf_check (hash-ref leaf_check "suffix_link" #f) #f))))
    (void)
  ))
