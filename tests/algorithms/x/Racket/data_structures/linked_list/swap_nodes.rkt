...
(set! res (appendv res (if list_ (hash-ref list_ "data" #f) #f)))
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not (if list_ (hash-ref list_ "data" #f) #f)) 0] [(string? (if list_ (hash-ref list_ "data" #f) #f)) (string-length (if list_ (hash-ref list_ "data" #f) #f))] [(vector? (if list_ (hash-ref list_ "data" #f) #f)) (vector-length (if list_ (hash-ref list_ "data" #f) #f))] [(hash? (if list_ (hash-ref list_ "data" #f) #f)) (hash-count (if list_ (hash-ref list_ "data" #f) #f))] [else (length (if list_ (hash-ref list_ "data" #f) #f))])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (and (equal? (if (if list_ (hash-ref list_ "data" #f) #f) (if (hash? (if list_ (hash-ref list_ "data" #f) #f)) (hash-ref (if list_ (hash-ref list_ "data" #f) #f) i #f) (safe-index (if list_ (hash-ref list_ "data" #f) #f) (int i))) #f) v1) (equal? idx1 (- 0 1))) (let ()
(set! idx1 i)
) (void))
      (if (and (equal? (if (if list_ (hash-ref list_ "data" #f) #f) (if (hash? (if list_ (hash-ref list_ "data" #f) #f)) (hash-ref (if list_ (hash-ref list_ "data" #f) #f) i #f) (safe-index (if list_ (hash-ref list_ "data" #f) #f) (int i))) #f) v2) (equal? idx2 (- 0 1))) (let ()
(set! idx2 i)
) (void))
...
(define temp (if res (if (hash? res) (hash-ref res idx1 #f) (safe-index res (int idx1))) #f))
(vector-set! res (int idx1) (if res (if (hash? res) (hash-ref res idx2 #f) (safe-index res (int idx2))) #f))
(vector-set! res (int idx2) temp)
(_return (let ([h (make-hash)]) (hash-set! h "data" res) h))
))
)
(define (to_string list_)
  (let/ec _return (begin
(_return (to-string (if list_ (hash-ref list_ "data" #f) #f)))
))
)
(define (main)
  (let/ec _return (begin
...
(displayln (to-string (string-append "Modified Linked List: " (to_string ll))))
(displayln (to-string "After swapping the nodes whose data is 1 and 4."))
))
)
