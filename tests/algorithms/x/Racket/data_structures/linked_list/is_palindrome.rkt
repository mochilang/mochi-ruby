...
(define fast 0)
(define slow 0)
(define n (cond [(not values) 0] [(string? values) (string-length values)] [(vector? values) (vector-length values)] [(hash? values) (hash-count values)] [else (length values)]))
(let/ec _break (let loop ()
  (if (and (let ([__l fast] [__r n]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ([__l (let ([__l fast] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))] [__r n]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))]))) (begin
    (let/ec _cont
      (set! stack (appendv (or stack (vector)) (vector (if values (if (hash? values) (hash-ref values slow #f) (safe-index values (int slow))) #f))))
...
(define i (- (cond [(not stack) 0] [(string? stack) (string-length stack)] [(vector? stack) (vector-length stack)] [(hash? stack) (hash-count stack)] [else (length stack)]) 1))
(let/ec _break (let loop ()
  (if (let ([__l slow] [__r n]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (not (equal? (if stack (if (hash? stack) (hash-ref stack i #f) (safe-index stack (int i))) #f) (if values (if (hash? values) (hash-ref values slow #f) (safe-index values (int slow))) #f))) (let ()
(_return #f)
) (void))
      (set! i (- i 1))
      (set! slow (let ([__l slow] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
...
(define (main)
  (let/ec _return (begin
(displayln (to-string (is_palindrome (vector))))
(displayln (to-string (is_palindrome (vector 1))))
(displayln (to-string (is_palindrome (vector 1 2))))
(displayln (to-string (is_palindrome (vector 1 2 1))))
(displayln (to-string (is_palindrome (vector 1 2 2 1))))
))
)
