...
(if (equal? (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)]) 0) (let ()
(_return "")
) (void))
(define s (to-string (if xs (if (hash? xs) (hash-ref xs 0 #f) (safe-index xs (int 0))) #f)))
(define i 1)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not xs) 0] [(string? xs) (string-length xs)] [(vector? xs) (vector-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (set! s (string-append (string-append s "->") (to-string (if xs (if (hash? xs) (hash-ref xs i #f) (safe-index xs (int i))) #f))))
...
(define (insert_node xs data)
  (let/ec _return (begin
(_return (appendv (or xs (vector)) (vector data)))
))
)
...
(define j 0)
(let/ec _break (let loop ()
  (if (let ([__l j] [__r split]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
...
(displayln (to-string (string-append "Original list: " (list_to_string head))))
(define places 3)
(define new_head (rotate_to_the_right head places))
(displayln (to-string (string-append (string-append (string-append "After " (to-string places)) " iterations: ") (list_to_string new_head))))
))
)
