...
(if (equal? (cond [(not elements) 0] [(string? elements) (string-length elements)] [(vector? elements) (vector-length elements)] [(hash? elements) (hash-count elements)] [else (length elements)]) 0) (let ()
(panic "The Elements List is empty")
) (void))
(set! nodes (vector))
(set! nodes (appendv (or nodes (vector)) (let ([h (make-hash)]) (hash-set! h "data" (if elements (if (hash? elements) (hash-ref elements 0 #f) (safe-index elements (int 0))) #f)) (hash-set! h "next" NIL) h)))
(define head 0)
(define current head)
(define i 1)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not elements) 0] [(string? elements) (string-length elements)] [(vector? elements) (vector-length elements)] [(hash? elements) (hash-count elements)] [else (length elements)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (set! nodes (appendv (or nodes (vector)) (let ([h (make-hash)]) (hash-set! h "data" (if elements (if (hash? elements) (hash-ref elements i #f) (safe-index elements (int i))) #f)) (hash-set! h "next" NIL) h)))
      (hash-set! (safe-index nodes current) "next" (- (cond [(not nodes) 0] [(string? nodes) (string-length nodes)] [(vector? nodes) (vector-length nodes)] [(hash? nodes) (hash-count nodes)] [else (length nodes)]) 1))
      (set! current (- (cond [(not nodes) 0] [(string? nodes) (string-length nodes)] [(vector? nodes) (vector-length nodes)] [(hash? nodes) (hash-count nodes)] [else (length nodes)]) 1))
...
(define index head)
(let/ec _break (let loop ()
  (if (not (equal? index NIL)) (begin
    (let/ec _cont
      (define node (if nodes (if (hash? nodes) (hash-ref nodes index #f) (safe-index nodes (int index))) #f))
      (set! s (string-append (string-append (string-append s "<") (to-string (if node (hash-ref node "data" #f) #f))) "> ---> "))
...
(define (main)
  (let/ec _return (begin
(define list_data (vector 1 3 5 32 44 12 43))
(displayln (to-string (string-append "List: " (to-string list_data))))
(displayln (to-string "Creating Linked List from List."))
(define head (make_linked_list list_data))
(displayln (to-string "Linked List:"))
(displayln (to-string (node_to_string head)))
))
)
