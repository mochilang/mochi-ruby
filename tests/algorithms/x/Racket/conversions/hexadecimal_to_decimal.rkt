...
(if (or (string=? c "a") (string=? c "A")) (let ()
(_return 10)
) (void))
(if (or (string=? c "b") (string=? c "B")) (let ()
(_return 11)
) (void))
(if (or (string=? c "c") (string=? c "C")) (let ()
(_return 12)
) (void))
(if (or (string=? c "d") (string=? c "D")) (let ()
(_return 13)
) (void))
(if (or (string=? c "e") (string=? c "E")) (let ()
(_return 14)
) (void))
(if (or (string=? c "f") (string=? c "F")) (let ()
(_return 15)
) (void))
(displayln "Non-hexadecimal value was passed to the function")
(_return 0)
))
)
(define (hex_to_decimal hex_string)
  (let/ec _return (begin
(define s (strip hex_string))
(if (equal? (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)]) 0) (let ()
(displayln "Empty string was passed to the function")
(_return 0)
) (void))
...
(let/ec _break (let loop ()
  (if (< i (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)])) (let/ec _cont
    ...
    (define value (hex_digit_value c))
    (set! decimal_number (let ([__l (* 16 decimal_number)] [__r value]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
...
(define (main)
  (let/ec _return (begin
(displayln (format "~a" (hex_to_decimal "a")))
(displayln (format "~a" (hex_to_decimal "12f")))
(displayln (format "~a" (hex_to_decimal "   12f   ")))
(displayln (format "~a" (hex_to_decimal "FfFf")))
(displayln (format "~a" (hex_to_decimal "-Ff")))
))
)
