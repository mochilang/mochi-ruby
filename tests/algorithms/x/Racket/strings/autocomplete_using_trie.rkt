...
(define words (vector "depart" "detergent" "daring" "dog" "deer" "deal"))
(define (autocomplete_using_trie prefix)
  (let/ec _return (begin
(define result (vector))
(define i 0)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not words) 0] [(string? words) (string-length words)] [(vector? words) (vector-length words)] [(hash? words) (hash-count words)] [else (length words)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (define w (if words (if (hash? words) (hash-ref words i #f) (safe-index words (int i))) #f))
      (if (string=? (slice w 0 (cond [(not prefix) 0] [(string? prefix) (string-length prefix)] [(vector? prefix) (vector-length prefix)] [(hash? prefix) (hash-count prefix)] [else (length prefix)])) prefix) (let ()
(set! result (appendv (or result (vector)) (vector (string-append w " "))))
) (void))
