...
(define (search_all text keywords)
  (let/ec _return (begin
(define result (let ([h (make-hash)]) h))
(let ([__seq keywords]) (let/ec _break (for ([word (if (hash? __seq) (in-hash-keys __seq) __seq)])
  (let/ec _cont
(define positions (vector))
(define m (cond [(not word) 0] [(string? word) (string-length word)] [(vector? word) (vector-length word)] [(hash? word) (hash-count word)] [else (length word)]))
(define i 0)
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (- (cond [(not text) 0] [(string? text) (string-length text)] [(vector? text) (vector-length text)] [(hash? text) (hash-count text)] [else (length text)]) m)]) (cond [(and (number? __l) (number? __r)) (<= __l __r)] [else (<= (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (string=? (slice text i (let ([__l i] [__r m]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))) word) (let ()
(set! positions (appendv (or positions (vector)) (vector i)))
) (void))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
    (loop)) (void))))
(if (let ([__l (cond [(not positions) 0] [(string? positions) (string-length positions)] [(vector? positions) (vector-length positions)] [(hash? positions) (hash-count positions)] [else (length positions)])] [__r 0]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))])) (let ()
(if (hash? result) (hash-set! result word positions) (begin (set! result (make-hash)) (hash-set! result word positions)))
) (void))
  ))))
(_return result)
))
)
(define text "whatever, err ... , wherever")
(define keywords (vector "what" "hat" "ver" "er"))
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (search_all text keywords))
    (void)
  ))
