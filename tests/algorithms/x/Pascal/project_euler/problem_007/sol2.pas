...
var
  solution_primes: array of int64;
  solution_num: int64;
begin
  if solution_nth <= 0 then begin
  error('Parameter nth must be greater than or equal to one.');
end;
  solution_primes := [];
  solution_num := 2;
  while Length(solution_primes) < solution_nth do begin
  if is_prime(solution_num) then begin
  solution_primes := concat(solution_primes, IntArray([solution_num]));
end;
  solution_num := solution_num + 1;
end;
  exit(solution_primes[Length(solution_primes) - 1]);
end;
begin
  init_now();
  bench_mem_0 := _mem();
  ...
  ans := solution(10001);
  writeln('solution(10001) = ' + IntToStr(ans));
