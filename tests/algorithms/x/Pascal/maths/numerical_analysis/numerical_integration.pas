...
begin
  ...
  trapezoidal_area_fx1 := f_var(x_start);
  ...
  trapezoidal_area_fx2 := f_var(trapezoidal_area_x2);
...
begin
  ...
  writeln('f(x) = x^3');
  writeln('The area between the curve, x = -10, x = 10 and the x axis is:');
  i := 10;
  while i <= 100000 do begin
  area := trapezoidal_area(f_var, -5, 5, i);
  writeln((('with ' + IntToStr(i)) + ' steps: ') + FloatToStr(area));
  i := i * 10;
end;
