...
fn main() {
    ...
    (*state).nodes = { let mut _v = state.nodes.clone().clone(); _v.push(Node {data: value, left: -1, right: -1}); _v };
    ...
    result = { let mut _v = result.clone(); _v.push(node.data); _v };
    ...
    fn mochi_main() {
    let mut state: TreeState = make_tree();
