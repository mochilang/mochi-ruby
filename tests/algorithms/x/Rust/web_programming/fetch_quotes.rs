...
fn main() {
        let _start: i64 = _now();
    fn quote_of_the_day() -> Vec<Quote> {
    return _fetch("https://zenquotes.io/api/today")
};
    fn random_quotes() -> Vec<Quote> {
    return _fetch("https://zenquotes.io/api/random")
};
    let mut response: Vec<Quote> = random_quotes();
    println!("{}", { let tmp = response; tmp.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(" ") });
    let _end: i64 = _now();
