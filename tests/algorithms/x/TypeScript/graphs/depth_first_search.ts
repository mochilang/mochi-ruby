// Generated by Mochi v0.10.66 on 2025-08-15 10:28:14 GMT+7
...
function contains(lst: string[], v: string): boolean {
  let i: number = 0;
  while ((i < _len(lst))) {
    if ((lst[(()=>{const _mochi_idx = i; return _mochi_idx < 0 ? lst.length + _mochi_idx : _mochi_idx;})()] === v)) {
      return true;
    }
    i = (i + 1);
  }
...
function depth_first_search(graph: Record<string, string[]>, start: string): string[] {
  ...
  while ((_len(stack) > 0)) {
    ...
    let v: string = stack[(()=>{const _mochi_idx = idx; return _mochi_idx < 0 ? stack.length + _mochi_idx : _mochi_idx;})()];
    stack = stack.slice(0, idx);
    let neighbors: string[] = graph[v];
    let i: number = (_len(neighbors) - 1);
    while ((i >= 0)) {
      let adj: string = neighbors[(()=>{const _mochi_idx = i; return _mochi_idx < 0 ? neighbors.length + _mochi_idx : _mochi_idx;})()];
