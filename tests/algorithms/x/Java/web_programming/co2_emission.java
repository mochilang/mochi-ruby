public class Main {
    static class Intensity {
        ...
        int actual;
        String index;
        Intensity(int forecast, int actual, String index) {
        ...
        @Override public String toString() {
            return String.format("{'forecast': %s, 'actual': %s, 'index': '%s'}", String.valueOf(forecast), String.valueOf(actual), String.valueOf(index));
        }
    }
    ...
    static class Entry {
        ...
        String to;
        Intensity intensity;
        Entry(String from, String to, Intensity intensity) {
            this.from = from;
            this.to = to;
        ...
        Entry() {}
        @Override public String toString() {
            return String.format("{'from': '%s', 'to': '%s', 'intensity': %s}", String.valueOf(from), String.valueOf(to), String.valueOf(intensity));
        }
    }
    ...
    static int fetch_last_half_hour() {
        Response resp = _fetch_Response(BASE_URL);
        Entry entry = resp.data[0];
        return entry.intensity.actual;
    }

    static Entry[] fetch_from_to(String start, String end) {
        String url = BASE_URL + "/" + start + "/" + end;
        Response resp_1 = _fetch_Response(url);
        return resp_1.data;
    }

    static void main() {
        Entry[] entries = ((Entry[])(fetch_from_to("2020-10-01", "2020-10-03")));
        int i = 0;
        while (i < entries.length) {
            Entry e = entries[i];
            System.out.println("from" + " " + e.from + " " + "to" + " " + e.to + " " + ":" + " " + String.valueOf(e.intensity.actual));
            i = i + 1;
        }
        int last = fetch_last_half_hour();
        System.out.println("fetch_last_half_hour() =" + " " + String.valueOf(last));
    }
