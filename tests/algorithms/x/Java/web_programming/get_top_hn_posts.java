public class Main {
    ...
    static Story get_hackernews_story(int story_id) {
        String url = "https://hacker-news.firebaseio.com/v0/item/" + _p(story_id) + ".json?print=pretty";
        Story story = ((Story)((_fetch(url))));
        if ((story.url.equals(""))) {
story.url = "https://news.ycombinator.com/item?id=" + _p(story_id);
        }
        return story;
    }

    static Story[] hackernews_top_stories(int max_stories) {
        String url_1 = "https://hacker-news.firebaseio.com/v0/topstories.json?print=pretty";
        int[] ids = ((int[])(((int[])((_fetch(url_1))))));
        ids = ((int[])(java.util.Arrays.copyOfRange(ids, 0, max_stories)));
        Story[] stories = ((Story[])(new Story[]{}));
        int i = 0;
        while (i < ids.length) {
            stories = ((Story[])(java.util.stream.Stream.concat(java.util.Arrays.stream(stories), java.util.stream.Stream.of(get_hackernews_story(ids[i]))).toArray(Story[]::new)));
            i = i + 1;
        }
        return stories;
    }

    static String hackernews_top_stories_as_markdown(int max_stories) {
        Story[] stories_1 = ((Story[])(hackernews_top_stories(max_stories)));
        String output = "";
        int i_1 = 0;
        while (i_1 < stories_1.length) {
            Story s = stories_1[i_1];
    ...
    public static void main(String[] args) {
        System.out.println(hackernews_top_stories_as_markdown(5));
    }
