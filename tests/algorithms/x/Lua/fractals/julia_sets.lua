  ...
  function prepare_grid(window_size, nb_pixels)
    ...
    while (i < nb_pixels) do
      ...
      while (j < nb_pixels) do
        local real = ((-window_size) + (((2.0 * window_size) * i) / (nb_pixels - 1)))
        local imag = ((-window_size) + (((2.0 * window_size) * j) / (nb_pixels - 1)))
...
function julia_demo()
  local grid = prepare_grid(1.0, 5)
  local c_poly = {re = (-0.4), im = 0.6}
  local c_exp = {re = (-2.0), im = 0.0}
  local poly_result = {}
  local exp_result = {}
  local y = 0
  while (y < (function(v)
        ...
        elseif type(v) == 'table' and (v[1] == nil or v[0] ~= nil) then
          ...
          elseif type(v) == 'string' then
            ...
            elseif type(v) == 'table' then
              ...
              local z_poly = iterate_function(eval_quadratic, c_poly, 20, z0, 4.0)
              local z_exp = iterate_function(eval_exponential, c_exp, 10, z0, 1e+10)
              row_poly = (function(lst, item)
              lst = lst or {}
              table.insert(lst, item)
              return lst
            end)(row_poly, (((complex_abs(z_poly) < 2.0)) and (1) or (0)))
            row_exp = (function(lst, item)
            lst = lst or {}
            table.insert(lst, item)
            return lst
          end)(row_exp, (((complex_abs(z_exp) < 10000.0)) and (1) or (0)))
          x = (x + 1)
        end
