...
do
  ...
  function test_distance()
    local p1 = {x = 2.0, y = (-1.0), z = 7.0}
    local p2 = {x = 1.0, y = (-3.0), z = 5.0}
    local d = distance(p1, p2)
    if (absf((d - 3.0)) > 0.0001) then
      _panic("distance test failed")
    end
    print((((type(((((("Distance from " .. point_to_string(p1)) .. " to ") .. point_to_string(p2)) .. " is ") .. _str(d))) == "table")) and (
    (function(v)
    ...
    if type(x) == "table" then
      ...
      elseif type(x) == "number" then
        ...
        return encode(v)
      end)(((((("Distance from " .. point_to_string(p1)) .. " to ") .. point_to_string(p2)) .. " is ") .. _str(d)))) or (((((("Distance from " .. point_to_string(p1)) .. " to ") .. point_to_string(p2)) .. " is ") .. _str(d)))))
    end
    function main()
      test_distance()
    end
