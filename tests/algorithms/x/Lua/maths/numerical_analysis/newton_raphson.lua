...
do
  ...
  function fail(msg)
    print((((type(("error: " .. msg)) == "table")) and (
    (function(v)
  ...
  function newton_raphson(f, x0, max_iter, step, max_error, log_steps)
    ...
    while (i < max_iter) do
      if log_steps then
        steps = (function(lst, item)
        local res = {table.unpack(lst or {})}
        table.insert(res, item)
        return res
      end)(steps, a)
    end
    local err = abs_float(f(a))
    if (err < max_error) then
      return {root = a, error = err, steps = steps}
    end
    local der = calc_derivative(f, a, step)
    if (der == 0) then
...
print((((type(((("root = " .. tostring(result.root)) .. ", error = ") .. tostring(result.error))) == "table")) and (
(function(v)
local function encode(x)
if type(x) == "table" then
  if x.__name and x.__order then
  ...
  return encode(v)
end)(((("root = " .. tostring(result.root)) .. ", error = ") .. tostring(result.error)))) or (((("root = " .. tostring(result.root)) .. ", error = ") .. tostring(result.error)))))
