    ...
    elseif type(v) == 'boolean' then
      ...
      do
        ...
        function gauss_easter(year)
          ...
          local leap_day_reinstall_number = (leap_day_inhibits / 4.0)
          local secular_moon_shift = ((((15.0 - lunar_orbit_correction) + leap_day_inhibits) - leap_day_reinstall_number) % 30.0)
          local century_starting_point = (((4.0 + leap_day_inhibits) - leap_day_reinstall_number) % 7.0)
          local days_to_add = (((19.0 * metonic_cycle) + secular_moon_shift) % 30.0)
          local days_from_phm_to_sunday = (((((2.0 * julian_leap_year) + (4.0 * non_leap_year)) + (6.0 * days_to_add)) + century_starting_point) % 7.0)
          if ((days_to_add == 29.0) and (days_from_phm_to_sunday == 6.0)) then
            return {month = 4, day = 19}
          end
          if ((days_to_add == 28.0) and (days_from_phm_to_sunday == 6.0)) then
            return {month = 4, day = 18}
          end
          local offset = (function(v) if v >= 0 then return math.floor(v) else return math.ceil(v) end end)((tonumber((days_to_add + days_from_phm_to_sunday)) or 0))
