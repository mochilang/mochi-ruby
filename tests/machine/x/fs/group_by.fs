// Generated by Mochi compiler v0.10.27 on 2025-07-17T07:46:53Z
open System
...
let stats: Anon2 list = [ for g in _group_by [ for person in people do yield person ] (fun person -> person.city) do
    yield { city = g.key; count = g.size; avg_age = (float (List.sum [ for p in g do yield p.age ]) / float (List.length [ for p in g do yield p.age ])) } ]
