-- Generated by Mochi compiler v0.10.27 on 2025-07-17T08:54:01Z
...
result = map snd (List.sortOn fst [(gKey (g), Map.fromList [("cat", gKey (g)), ("share", VDouble ((sum [if _asBool (fromMaybe (error "missing") (Map.lookup "flag" (x))) then fromMaybe (error "missing") (Map.lookup "val" (x)) else 0 | x <- g] / sum [fromMaybe (error "missing") (Map.lookup "val" (x)) | x <- g])))]) | g <- _group_by [(i) | i <- items] (\(i) -> fromMaybe (error "missing") (Map.lookup "cat" i))])
