# Generated by Mochi compiler v0.10.28 on 2025-07-18T07:03:44Z
defmodule Main do
  def main do
    # items :: list(map())
    ...
    grouped =
      (fn ->
         ...
         items = groups
         items = Enum.sort_by(items, fn g -> -Enum.sum(for x <- g.items, do: x.val) end)
         Enum.map(items, fn g -> %{cat: g.key, total: Enum.sum(for x <- g.items, do: x.val)} end)
       end).()
