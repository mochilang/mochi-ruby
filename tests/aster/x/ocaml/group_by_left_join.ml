...
let () =
  ...
  List.iter (fun (g_key, g_items) ->
    ...
    __res0 := [("name", g_key); ("count", List.length (List.filter_map (fun r -> if (List.assoc "o" r) then Some (r) else None) g))] :: !__res0
  ) !__groups0;
  List.rev !__res0) in
  print_endline (String.concat " " (List.filter (fun s -> s <> "") ["--- Group Left Join ---"]));
  (try List.iter (fun s ->
    try
