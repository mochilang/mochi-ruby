// Generated by Mochi 0.10.32 on 2025-08-01 18:40 +0700
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <malloc.h>
#include <math.h>


static char* str_concat(const char *a, const char *b) {
    size_t len1 = strlen(a);
    size_t len2 = strlen(b);
    char *res = malloc(len1 + len2 + 1);
    memcpy(res, a, len1);
    memcpy(res + len1, b, len2);
    res[len1 + len2] = 0;
    return res;
}

static char* str_float(double v) {
    char buf[64];
    snprintf(buf, sizeof(buf), "%g", v);
    return strdup(buf);
}

#include <time.h>
#include <stdlib.h>
static int seeded_now = 0;
static long long now_seed = 0;
static long long _now(void) {
    if (!seeded_now) {
        const char *s = getenv("MOCHI_NOW_SEED");
        if (s && *s) {
            now_seed = atoll(s);
            seeded_now = 1;
        }
    }
    if (seeded_now) {
        now_seed = (now_seed * 1664525 + 1013904223) % 2147483647;
        return now_seed;
    }
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    return (long long)(ts.tv_sec * 1000000000LL + ts.tv_nsec);
}

static long long _mem(void) {
    struct mallinfo mi = mallinfo();
    return (long long)mi.uordblks;
}

typedef struct Vector Vector;

struct Vector {
    double x;
    double y;
    double z;
};

Vector add(Vector a, Vector b);
Vector sub(Vector a, Vector b);
Vector mul(Vector v, double s);
double dot(Vector a, Vector b);
Vector intersectPoint(Vector rv, Vector rp, Vector pn, Vector pp);
int user_main();
int main(void);

Vector add(Vector a, Vector b) {
    return (Vector){.x = a.x + b.x, .y = a.y + b.y, .z = a.z + b.z};
}

Vector sub(Vector a, Vector b) {
    return (Vector){.x = a.x - b.x, .y = a.y - b.y, .z = a.z - b.z};
}

Vector mul(Vector v, double s) {
    return (Vector){.x = v.x * s, .y = v.y * s, .z = v.z * s};
}

double dot(Vector a, Vector b) {
    return ((a.x * b.x) + (a.y * b.y)) + (a.z * b.z);
}

Vector intersectPoint(Vector rv, Vector rp, Vector pn, Vector pp) {
    Vector diff = sub(rp, pp);
    double prod1 = dot(diff, pn);
    double prod2 = dot(rv, pn);
    double prod3 = prod1 / prod2;
    return sub(rp, mul(rv, prod3));
}

int user_main() {
    Vector rv = (Vector){.x = 0, .y = -(1), .z = -(1)};
    Vector rp = (Vector){.x = 0, .y = 0, .z = 10};
    Vector pn = (Vector){.x = 0, .y = 0, .z = 1};
    Vector pp = (Vector){.x = 0, .y = 0, .z = 5};
    Vector ip = intersectPoint(rv, rp, pn, pp);
    puts(str_concat(str_concat(str_concat(str_concat(str_concat(str_concat("The ray intersects the plane at (", str_float(ip.x)), ", "), str_float(ip.y)), ", "), str_float(ip.z)), ")"));
}

int main(void) {
    {
        long long __start = _now();
        long long __mem_start = _mem();
        user_main();
        long long __end = _now();
        long long __mem_end = _mem();
        long long __dur_us = (__end - __start) / 1000;
        long long __mem_bytes = __mem_end - __mem_start;
        printf("{\n  \"duration_us\": %-lld,\n  \"memory_bytes\": %-lld,\n  \"name\": \"main\"\n}\n", __dur_us, __mem_bytes);
    }
    return 0;
}
