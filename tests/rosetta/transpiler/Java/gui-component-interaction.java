public class Main {
    static int value;
    static boolean done;

    static java.util.Scanner _scanner = new java.util.Scanner(System.in);

    static int parseInt(String str) {
        int i = 0;
        boolean neg = false;
        if (_runeLen(str) > 0 && (str.substring(0, 1).equals("-"))) {
            neg = true;
            i = 1;
        }
        int n = 0;
        java.util.Map<String,Integer> digits = ((java.util.Map<String,Integer>)(new java.util.LinkedHashMap<String, Integer>(java.util.Map.ofEntries(java.util.Map.entry("0", 0), java.util.Map.entry("1", 1), java.util.Map.entry("2", 2), java.util.Map.entry("3", 3), java.util.Map.entry("4", 4), java.util.Map.entry("5", 5), java.util.Map.entry("6", 6), java.util.Map.entry("7", 7), java.util.Map.entry("8", 8), java.util.Map.entry("9", 9)))));
        while (i < _runeLen(str)) {
            n = n * 10 + (int)(((int)(digits).getOrDefault(str.substring(i, i + 1), 0)));
            i = i + 1;
        }
        if (neg) {
            n = -n;
        }
        return n;
    }

    static int rand10000() {
        return Math.floorMod(_now(), 10000);
    }
    public static void main(String[] args) {
        {
            long _benchStart = _now();
            long _benchMem = _mem();
            value = 0;
            System.out.println("Value:" + " " + String.valueOf(value));
            done = false;
            while (!done) {
                System.out.println("i=increment, r=random, s num=set, q=quit:");
                String line = (_scanner.hasNextLine() ? _scanner.nextLine() : "");
                if ((line.equals("i"))) {
                    value = value + 1;
                    System.out.println("Value:" + " " + String.valueOf(value));
                } else                 if ((line.equals("r"))) {
                    System.out.println("Set random value? (y/n)");
                    String ans = (_scanner.hasNextLine() ? _scanner.nextLine() : "");
                    if ((ans.equals("y"))) {
                        value = rand10000();
                        System.out.println("Value:" + " " + String.valueOf(value));
                    }
                } else                 if (_runeLen(line) > 2 && (line.substring(0, 2).equals("s "))) {
                    value = parseInt(line.substring(2, _runeLen(line)));
                    System.out.println("Value:" + " " + String.valueOf(value));
                } else                 if ((line.equals("q"))) {
                    done = true;
                } else {
                    System.out.println("Unknown command");
                }
            }
            long _benchDuration = _now() - _benchStart;
            long _benchMemory = _mem() - _benchMem;
            System.out.println("{");
            System.out.println("  \"duration_us\": " + _benchDuration + ",");
            System.out.println("  \"memory_bytes\": " + _benchMemory + ",");
            System.out.println("  \"name\": \"main\"");
            System.out.println("}");
            return;
        }
    }

    static boolean _nowSeeded = false;
    static int _nowSeed;
    static int _now() {
        if (!_nowSeeded) {
            String s = System.getenv("MOCHI_NOW_SEED");
            if (s != null && !s.isEmpty()) {
                try { _nowSeed = Integer.parseInt(s); _nowSeeded = true; } catch (Exception e) {}
            }
        }
        if (_nowSeeded) {
            _nowSeed = (int)((_nowSeed * 1664525L + 1013904223) % 2147483647);
            return _nowSeed;
        }
        return (int)(System.nanoTime() / 1000);
    }

    static long _mem() {
        Runtime rt = Runtime.getRuntime();
        rt.gc();
        return rt.totalMemory() - rt.freeMemory();
    }

    static int _runeLen(String s) {
        return s.codePointCount(0, s.length());
    }
}
