...
$__start = _now();
  ...
  $m = _intdiv($last, 2);
  $n5 = [];
  $n5 = array_merge($n5, [qsel(array_slice($a, 0, $m - 0), 0)]);
  $n5 = array_merge($n5, [qsel(array_slice($a, 0, $m - 0), count($a) / 4)]);
  $n5 = array_merge($n5, [qsel($a, $m)]);
  $arr2 = array_slice($a, $m);
  $q3 = $last - $m - count($a) / 4;
  $n5 = array_merge($n5, [qsel($arr2, $q3)]);
  $arr2 = array_slice($arr2, $q3);
  $n5 = array_merge($n5, [qsel($arr2, count($arr2) - 1)]);
  return $n5;
};
