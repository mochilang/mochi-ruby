...
$__start = _now();
  ...
  global $e, $f, $g, $h, $faces, $names, $idx2, $per;
  foreach ($xs as $x) {
  if ($x == $v) {
  return true;
}
  ...
  global $f, $g, $h, $faces, $names, $idx2, $per;
  $le = count($face);
  if ($le == 0) {
  return null;
}
...
Edge to perimeter format translations:'), PHP_EOL;
  ...
  $names = ['E', 'F', 'G', 'H'];
  $idx2 = 0;
  while ($idx2 < count($faces)) {
  $per = faceToPerim($faces[$idx2]);
  if ($per == null) {
  echo rtrim('  ' . $names[$idx2] . ' => Invalid edge format'), PHP_EOL;
} else {
  echo rtrim('  ' . $names[$idx2] . ' => ' . listStr($per)), PHP_EOL;
}
  $idx2 = $idx2 + 1;
}
$__end = _now();
