...
(let ((start11 (now)
 ...
 (letrec ((loop5 (lambda ()
 (if (< d base)
 (begin (let ((val (_add (* prefix base)
 d)
)
)
 (begin (if (isPrime val)
 (begin (if (> val b)
 (begin (set! b val)
)
 (quote ()
 ...
 (if (_lt (+ depth 1)
 limit)
 (begin (set! b (search base val (+ depth 1)
 limit b)
)
)
 (quote ()
 ...
 (letrec ((loop9 (lambda ()
 (if (<= b 17)
 (begin (_display (to-str (string-append (string-append (to-str b)
 ": ")
 (to-str (largest b)
)
)
