// Generated by Mochi transpiler v0.10.57 on 2025-08-04 22:45:19 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
void showTokens(int64_t tokens);
int main();
...
void showTokens(int64_t tokens) {
    std::cout << (std::string("Tokens remaining ") + ([&]{ std::ostringstream ss; ss << std::boolalpha << tokens; return ss.str(); }()));
    std::cout << std::endl;
}
...
int main() {
    {
        ...
        int64_t tokens = int64_t(12);
        bool done = false;
        while (!done) {
            showTokens(tokens);
            std::cout << std::string("");
            std::cout << std::endl;
            std::cout << std::string("How many tokens 1, 2 or 3?");
            std::cout << std::endl;
            std::string line = ([&]{ std::string __line; std::getline(std::cin, __line); return __line; }());
            int64_t t = int64_t(0);
            if ((line.size() > int64_t(0))) {
                t = _parse_int_str(line, int64_t(10));
            }
            if (((t < int64_t(1)) || (t > int64_t(3)))) {
                std::cout << std::string("\nMust be a number between 1 and 3, try again.\n");
                std::cout << std::endl;
            } else {
                int64_t ct = (int64_t(4) - t);
                std::string s = std::string("s");
                if ((ct == int64_t(1))) {
                    s = std::string("");
                }
                std::cout << ((((std::string("  Computer takes ") + ([&]{ std::ostringstream ss; ss << std::boolalpha << ct; return ss.str(); }())) + std::string(" token")) + s) + std::string("\n\n"));
                std::cout << std::endl;
                tokens = (tokens - int64_t(4));
            }
            if ((tokens == int64_t(0))) {
                showTokens(int64_t(0));
                std::cout << std::string("  Computer wins!");
                std::cout << std::endl;
                done = true;
            }
        }
