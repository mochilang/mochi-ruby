// Generated by Mochi transpiler v0.10.57 on 2025-08-04 22:37:30 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
int main() {
    {
        ...
        std::cout << std::endl;
        while (!done) {
            std::cout << std::string("i=increment, r=random, s num=set, q=quit:");
            std::cout << std::endl;
            ...
            if ((line == std::string("i"))) {
                ...
                std::cout << value;
                std::cout << std::endl;
            } else             if ((line == std::string("r"))) {
                std::cout << std::string("Set random value? (y/n)");
                std::cout << std::endl;
                std::string ans = ([&]{ std::string __line; std::getline(std::cin, __line); return __line; }());
                if ((ans == std::string("y"))) {
                    value = rand10000();
                    std::cout << std::string("Value:");
                    std::cout << " ";
            ...
            } else             if (((line.size() > int64_t(2)) && (([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(int64_t(0), int64_t(2) - int64_t(0)); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+int64_t(0), __v.begin()+int64_t(2)); })(line) == std::string("s ")))) {
                value = parseInt(([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(int64_t(2), line.size() - int64_t(2)); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+int64_t(2), __v.begin()+line.size()); })(line));
                std::cout << std::string("Value:");
                std::cout << " ";
                std::cout << value;
                std::cout << std::endl;
            } else             if ((line == std::string("q"))) {
                done = true;
            } else {
                std::cout << std::string("Unknown command");
                std::cout << std::endl;
            }
