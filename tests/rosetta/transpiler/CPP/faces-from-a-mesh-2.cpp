// Generated by Mochi transpiler v0.10.57 on 2025-08-04 16:14:30 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <any>
...
int main() {
    ...
    while ((idx < faces.size())) {
        std::any per = faceToPerim(faces[idx]);
        if ((!per.has_value())) {
            std::cout << ((std::string("  ") + names[idx]) + std::string(" => Invalid edge format"));
            std::cout << std::endl;
        } else {
            std::cout << (((std::string("  ") + names[idx]) + std::string(" => ")) + listStr(std::any_cast<std::vector<int64_t>>(per)));
            std::cout << std::endl;
        }
        idx = (idx + int64_t(1));
    }
