// Generated by Mochi transpiler v0.10.57 on 2025-08-04 21:23:01 GMT+7
#include <iostream>
#include <string>
#include <map>
#include <vector>
...
std::string strReplace(std::string s, std::string old, std::string _new);
std::string g2l(std::string pieces);
int64_t spid(std::string pieces);
int main();

std::string glyphs = std::string("♜♞♝♛♚♖♘♗♕♔");
...
std::string strReplace(std::string s, std::string old, std::string _new) {
    ...
    int64_t i = int64_t(0);
    while ((i < s.size())) {
        if ((((old.size() > int64_t(0)) && ((i + old.size()) <= s.size())) && (s.substr(i, (i + old.size()) - i) == old))) {
            res = (res + _new);
            i = (i + old.size());
        } else {
...
std::string g2l(std::string pieces) {
    std::string lets = std::string("");
    int64_t i = int64_t(0);
    while ((i < pieces.size())) {
        std::string ch = pieces.substr(i, (i + int64_t(1)) - i);
        lets = (lets + ([&](const auto& __m){ auto __it = __m.find(ch); return __it != __m.end() ? __it->second : ""; })(g2lMap));
        i = (i + int64_t(1));
    }
    return lets;
}
...
int64_t spid(std::string pieces) {
    ...
    for (std::string one : std::vector<std::string>{std::string("K"), std::string("Q")}) {
        ...
        int64_t i = int64_t(0);
        while ((i < pieces.size())) {
            if ((pieces.substr(i, (i + int64_t(1)) - i) == one)) {
    ...
    for (std::string two : std::vector<std::string>{std::string("R"), std::string("N"), std::string("B")}) {
        ...
        int64_t i = int64_t(0);
        while ((i < pieces.size())) {
            if ((pieces.substr(i, (i + int64_t(1)) - i) == two)) {
                count = (count + int64_t(1));
            }
            i = (i + int64_t(1));
        }
    ...
    std::string piecesN = strReplace(pieces, std::string("Q"), std::string(""));
    piecesN = strReplace(piecesN, std::string("B"), std::string(""));
    ...
    std::string np = (([&]{ std::ostringstream ss; ss << std::boolalpha << n1; return ss.str(); }()) + ([&]{ std::ostringstream ss; any_to_stream(ss, n2);
 return ss.str(); }()));
    int64_t N = ([&](const auto& __m){ auto __it = __m.find(np); return __it != __m.end() ? __it->second : 0; })(ntable);
    std::string piecesQ = strReplace(pieces, std::string("B"), std::string(""));
