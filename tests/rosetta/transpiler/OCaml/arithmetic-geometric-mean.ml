...
let rec agm a g =
  ...
  a := (Obj.magic !a : float);
  g := (Obj.magic !g : float);
  let eps = 0.00000000000001 in
  (try while (abs_float (!a -. !g) > (abs_float !a *. eps)) do
    try
  let newA = ((!a +. !g) /. 2.0) in
  let newG = sqrtApprox ((!a *. !g)) in
  a := newA;
  g := newG;
    with Continue -> ()
  done with Break -> ());
...
let rec main () =
  ...
  (try
  print_endline ((string_of_float (agm (1.0) ((1.0 /. sqrtApprox (2.0))))));
    !__ret
  with Return -> !__ret)
