...
let () =
  ...
  let idx2 = ref (0) in
  (try while (!idx2 < List.length (!faces)) do
    try
  let per = faceToPerim (List.nth (!faces) (!idx2)) in
  if (per = nil) then (
  print_endline ((("  " ^ List.nth (!names) (!idx2)) ^ " => Invalid edge format"));
  ) else (
  print_endline (((("  " ^ List.nth (!names) (!idx2)) ^ " => ") ^ listStr ((Obj.magic per : int list))));
  );
  idx2 := (!idx2 + 1);
    with Continue -> ()
  done with Break -> ());
