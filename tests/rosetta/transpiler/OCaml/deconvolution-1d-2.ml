...
and listToString1 xs =
  ...
  (try while (!i < List.length (xs)) do
    try
  s := (!s ^ fmt1 (Obj.repr (List.nth (xs) (!i))));
...
and deconv g f =
  ...
  let i = ref (0) in
  (try while (!i <= (List.length (g) - List.length (f))) do
    try
  let sum = ref (List.nth (g) (!i)) in
  let j = ref (1) in
  (try while (!j < List.length (f)) do
    try
  if (!j <= !i) then (
  sum := (!sum -. (List.nth (!out) ((!i - !j)) *. List.nth (f) (!j)));
  );
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  out := (List.append (!out) [(Obj.magic ((!sum /. List.nth (f) (0))) : float)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
and main () =
  ...
  print_endline (listToString1 (!h));
  print_endline (listToString1 (deconv (!g) (!f)));
  print_endline (listToString1 (!f));
  print_endline (listToString1 (deconv (!g) (!h)));
    !__ret
  with Return -> !__ret)
