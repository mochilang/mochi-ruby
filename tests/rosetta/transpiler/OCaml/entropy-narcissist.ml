...
and entropy data =
  ...
  (try while (!i < String.length (data)) do
    try
  let ch = String.sub (data) !i ((!i + 1) - !i) in
  ...
  let e = ref (0.0) in
  let l = float_of_int (String.length (data)) in
  (try List.iter (fun (ch, _) ->
    try
  let px = (float_of_int ((try List.assoc (ch) (!counts) with Not_found -> 0)) /. l) in
  if (px > 0.0) then (
  e := (!e -. (px *. log2 (Obj.repr (px))));
  );
    with Continue -> ()) (!counts) with Break -> ());
  __ret := (Obj.magic (!e) : float); raise Return
  with Return -> !__ret)
...
let rec main () =
  ...
  (try
  print_endline (("Source file entropy: " ^ (Printf.sprintf "%.16g" (entropy (source)))));
    !__ret
  with Return -> !__ret)
