...
let () =
  ...
  poss := (List.append (!poss) [(Obj.magic ([!x; !y]) : int list)]);
  );
  x := (!x + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  print_endline (((string_of_int (List.length (!poss))) ^ " possible points"));
  (try while (!r < span) do
    try
  ...
  (try while (!n < nPts) do
    try
  let i = ref ((_now () mod List.length (!poss))) in
  let x = List.nth (List.nth (!poss) (!i)) (0) in
  let yy = List.nth (List.nth (!poss) (!i)) (1) in
  let row = (yy + rMax) in
  let col = ((x + rMax) * 2) in
  ...
  (try while (!i2 < span) do
    try
  let line = ref ("") in
  let j = ref (0) in
  (try while (!j < (span * 2)) do
    try
  line := (!line ^ List.nth (List.nth (!rows) (!i2)) (!j));
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
