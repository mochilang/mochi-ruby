...
and g2l pieces =
  ...
  res := (!res ^ (try List.assoc (ch) !g2lMap with Not_found -> (Obj.magic 0)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  __ret := (Obj.magic (!res) : string); raise Return
...
and spid pieces =
  ...
  let piecesN = ref (removeChar (removeChar (!pieces) ("Q")) ("B")) in
  ...
  let _n = (try List.assoc (((string_of_int (n1)) ^ (string_of_int (n2)))) !ntable with Not_found -> (Obj.magic 0)) in
  let piecesQ = ref (removeChar (!pieces) ("B")) in
