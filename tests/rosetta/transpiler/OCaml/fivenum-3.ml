...
and fivenum a =
  let __ret = ref ([] : (float) list) in
  (try
  let arr = ref (sortFloat (a)) in
  let n = List.length (!arr) in
  let half = ((n + 3) - ((n + 3) mod 2)) in
  let n4 = (float_of_int ((half / 2)) /. 2.0) in
  let nf = float_of_int (n) in
  let d = ref ([1.0; n4; ((nf +. 1.0) /. 2.0); ((nf +. 1.0) -. n4); nf]) in
  let result = ref (([] : (float) list)) in
  let idx = ref (0) in
  (try while (!idx < List.length (!d)) do
    try
  let de = List.nth (!d) (!idx) in
  let fl = int_of_float ((de -. 1.0)) in
  let cl = ceilf (Obj.repr ((de -. 1.0))) in
  result := (List.append (!result) [(Obj.magic ((0.5 *. (List.nth (!arr) (fl) +. List.nth (!arr) (cl)))) : float)]);
  idx := (!idx + 1);
    with Continue -> ()
  done with Break -> ());
