...
let rec nthRoot x n =
  ...
  __ret := (Obj.magic (!low) : float); raise Return
  with Return -> !__ret)
...
let rec main () =
  ...
  let count = 10.0 in
  let a = (!sum /. count) in
  let g = nthRoot (!prod) (10) in
  let h = (count /. !sumRecip) in
  print_endline (((((("A: " ^ (string_of_float (a))) ^ " G: ") ^ (string_of_float (g))) ^ " H: ") ^ (string_of_float (h))));
  print_endline (("A >= G >= H: " ^ __show (((a >= g) && (g >= h)))));
    !__ret
  with Return -> !__ret)
