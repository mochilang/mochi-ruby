...
let rec listToStringInts xs =
  ...
  (try while (!i < List.length (xs)) do
    try
  s := (!s ^ (string_of_int ((Obj.magic List.nth (xs) (!i) : int))));
...
and main () =
  ...
  print_endline (listToStringInts (!h));
  print_endline (listToStringInts (deconv (!g) (!f)));
  print_endline (listToStringInts (!f));
  print_endline (listToStringInts (deconv (!g) (!h)));
    !__ret
  with Return -> !__ret)
