...
fn main() {
    ...
    return arr
        ...
        unsafe fn median(mut s: Vec<f64>) -> f64 {
    let mut n = (s.len() as i64);
    if ((n % 2) == 1) {
        return s[(n / 2) as usize]
    }
    return ((s[((n / 2) - 1) as usize] + s[(n / 2) as usize]) / 2.0)
};
        unsafe fn fivenum(mut xs: Vec<f64>) -> Vec<f64> {
    let mut s: Vec<f64> = sortFloat(xs.clone());
    let mut n = (s.len() as i64);
    let mut q1: f64 = s[((n - 1) / 4) as usize];
    let mut med: f64 = median(s.clone());
    let mut q3: f64 = s[((3 * (n - 1)) / 4) as usize];
    return vec![s[0 as usize], q1, med, q3, s[(n - 1) as usize]]
};
        static mut g_x1: Vec<f64> = Vec::new();;
        static mut g_x2: Vec<f64> = Vec::new();;
        static mut g_x3: Vec<f64> = Vec::new();;
