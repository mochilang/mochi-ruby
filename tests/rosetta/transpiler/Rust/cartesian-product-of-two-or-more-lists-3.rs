...
fn main() {
    ...
    for v in a.iter().cloned() {
        out = { let mut _v = out.clone(); _v.push(v); _v };
    }
    for v in b.iter().cloned() {
        out = { let mut _v = out.clone(); _v.push(v); _v };
    }
    return out
};
    ...
    let mut rest: Vec<Vec<i64>> = cartN(a.chars().skip(1 as usize).take(((a.len() as i64) - 1) as usize).collect::<String>());
    for x in a.chars().nth(0 as usize).unwrap() {
        for p in rest.iter() {
            out = { let mut _v = out.clone(); _v.push(concat(vec![x], p.clone())); _v };
        }
    }
    return out
};
