...
fn main() {
    ...
    let mut up: String = String::from("ABCDEFGHIJKLMNOPQRSTUVWXYZ").clone();
    let mut low: String = String::from("abcdefghijklmnopqrstuvwxyz").clone();
    let mut i: i64 = 0;
    while (i < (up.len() as i64)) {
        if (ch == { let tmp = &up; tmp.chars().skip(i as usize).take(((i + 1) - i) as usize).collect::<String>() }) {
            return { let tmp = &low; tmp.chars().skip(i as usize).take(((i + 1) - i) as usize).collect::<String>() }
        }
        i = (i + 1);
    }
