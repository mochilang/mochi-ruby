...
fn main() {
    ...
    while true {
        ...
        unsafe fn bezier2(b: &HashMap<String, f64>, mut x1: i64, mut y1: i64, mut x2: i64, mut y2: i64, mut x3: i64, mut y3: i64, p: &Pixel) {
    ...
    i = 0;
    while (i <= g_b2Seg) {
        let c: f64 = ((i as f64) / (g_b2Seg as f64));
        let mut a: f64 = (1.0 - c);
        let mut a2: f64 = (a * a);
        let mut b2: f64 = ((2.0 * c) * a);
        let mut c2: f64 = (c * c);
        px[i as usize] = ((((a2 * fx1) + (b2 * fx2)) + (c2 * fx3)) as i64);
        py[i as usize] = ((((a2 * fy1) + (b2 * fy2)) + (c2 * fy3)) as i64);
        i = (i + 1);
    }
    ...
    while (i <= g_b2Seg) {
        ...
        static g_b: LazyLock<Mutex<HashMap<&str, i64>>> = LazyLock::new(|| Mutex::new(HashMap::new()));;
        fillRgb(&g_b, 14614575);
        bezier2(&g_b, 20, 150, 500, -100, 300, 280, &pixelFromRgb(4165615));
        let _end: i64 = _now();
