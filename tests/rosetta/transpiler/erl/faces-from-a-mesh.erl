...
-export([main/1, contains/2, sliceequal/2, reverse/1, copyints/1, perimequal/2, sortedges/1, concat/2, facetoperim/1, liststr/1]).
...
reverse(Xs_2) ->
    try
        I_3 = 0,
        J = (length(Xs_2) - 1),
        Fun_3 = fun Fun_3_loop(I_3, J, Xs_2) ->
    case (I_3 < J) of
        true ->
            T = (case erlang:is_map(Xs_2) of true -> maps:get(I_3, Xs_2, nil); _ -> lists:nth(I_3 + 1, Xs_2) end),
            Xs_3 = lists:sublist(Xs_2, I_3) ++ [(case erlang:is_map(Xs_2) of true -> maps:get(J, Xs_2, nil); _ -> lists:nth(J + 1, Xs_2) end)] ++ lists:nthtail(I_3 + 1, Xs_2),
            Xs_4 = lists:sublist(Xs_3, J) ++ [T] ++ lists:nthtail(J + 1, Xs_3),
            I_4 = (I_3 + 1),
            J_2 = (J - 1),
            Fun_3_loop(I_4, J_2, Xs_4);
        _ -> {I_3, J, Xs_2}
    end
end,
{I_4, J_2, Xs_4} = Fun_3(I_3, J, Xs_2),
        {nil, Xs_4}
    catch {return, Ret} -> Ret end.
...
copyints(Xs_5) ->
    try
        Out = [],
        Fun_4 = fun Fun_4_loop(List, Out, Xs_5) ->
    case List of
        [] -> {Out, Xs_5};
        [V_2|V_2_rest] ->
            Out_2 = lists:append(Out, [V_2]),
            Fun_4_loop(V_2_rest, Out_2, Xs_5)
    end
end,
{Out_2, Xs_5} = Fun_4(Xs_5, Out, Xs_5),
        Out_2
    catch {return, Ret} -> Ret end.
...
perimequal(P1, P2) ->
    ...
    case List of
        [] -> {P1, P2};
        [V_3|V_3_rest] ->
            case mochi_not(mochi_member(V_3, P2)) of
        true -> throw({return, false});
        _ -> ok
    end,
            Fun_5_loop(V_3_rest, P1, P2)
    end
end,
...
concat(A_3, B_3) ->
    ...
    case List of
        [] -> {A_3, B_3, Out_3};
        [X_2|X_2_rest] ->
            Out_4 = lists:append(Out_3, [X_2]),
            Fun_11_loop(X_2_rest, A_3, B_3, Out_4)
    end
end,
{A_3, B_3, Out_4} = Fun_11(A_3, A_3, B_3, Out_3),
    ...
    case List of
        [] -> {A_3, B_3, Out_4};
        [X_3|X_3_rest] ->
            Out_5 = lists:append(Out_4, [X_3]),
            Fun_12_loop(X_3_rest, A_3, B_3, Out_5)
    end
end,
...
main(_) ->
    ...
    erlang:put('e', [#{"a" => 7, "b" => 11}, #{"a" => 1, "b" => 11}, #{"a" => 1, "b" => 7}]),
    erlang:put('f', [#{"a" => 11, "b" => 23}, #{"a" => 1, "b" => 17}, #{"a" => 17, "b" => 23}, #{"a" => 1, "b" => 11}]),
    erlang:put('g', [#{"a" => 8, "b" => 14}, #{"a" => 17, "b" => 19}, #{"a" => 10, "b" => 12}, #{"a" => 10, "b" => 14}, #{"a" => 12, "b" => 17}, #{"a" => 8, "b" => 18}, #{"a" => 18, "b" => 19}]),
    erlang:put('h', [#{"a" => 1, "b" => 3}, #{"a" => 9, "b" => 11}, #{"a" => 3, "b" => 11}, #{"a" => 1, "b" => 11}]),
    io:format("~ts~n", ["\nEdge to perimeter format translations:"]),
    erlang:put('faces', [erlang:get('e'), erlang:get('f'), erlang:get('g'), erlang:get('h')]),
    erlang:put('names', ["E", "F", "G", "H"]),
    erlang:put('idx2', 0),
    Fun_17 = fun Fun_17_loop() ->
    case (erlang:get('idx2') < length(erlang:get('faces'))) of
        true ->
            Per = facetoperim((case erlang:is_map(erlang:get('faces')) of true -> maps:get(erlang:get('idx2'), erlang:get('faces'), nil); _ -> lists:nth(erlang:get('idx2') + 1, erlang:get('faces')) end)),
            case (Per == nil) of
        true -> io:format("~ts~n", [(("  " ++ (case erlang:is_map(erlang:get('names')) of true -> maps:get(erlang:get('idx2'), erlang:get('names'), nil); _ -> lists:nth(erlang:get('idx2') + 1, erlang:get('names')) end)) ++ " => Invalid edge format")]);
        _ -> io:format("~ts~n", [((("  " ++ (case erlang:is_map(erlang:get('names')) of true -> maps:get(erlang:get('idx2'), erlang:get('names'), nil); _ -> lists:nth(erlang:get('idx2') + 1, erlang:get('names')) end)) ++ " => ") ++ liststr(Per))])
    end,
            erlang:put('idx2', (erlang:get('idx2') + 1)),
            Fun_17_loop();
        _ -> {}
    end
end,
