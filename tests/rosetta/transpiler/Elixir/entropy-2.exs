# Code generated by Mochi transpiler 2025-08-04 16:53 +0700
defmodule Main do
  defp _now() do
  ...
  def main() do
    try do
      ...
      l = 0.0
      i = 0
      while_fun_4 = fn while_fun_4, counts, i, l ->
        if i < _len(s) do
          ch = _slice(s, i, i + 1 - (i))
          counts = Map.put(counts, ch, ((if Map.has_key?(counts, ch), do: counts[ch] + 1, else: 1)))
          l = l + 1.0
          i = i + 1
          while_fun_4.(while_fun_4, counts, i, l)
        else
          {counts, i, l}
        end
      end
      {counts, i, l} = try do
          ...
          {:break, {counts, i, l}} -> {counts, i, l}
        end

      hm = 0.0
      {hm} = Enum.reduce(Map.keys(counts), {hm}, fn ch, {hm} ->
        c = counts[ch]
        hm = hm + c * log2(c)
        {hm}
      end)
      IO.puts(Kernel.inspect(log2(l) - hm / l))
    catch
      {:return, val} -> val
    end
  end
