...
defmodule Main do
  ...
  def splitPath(p) do
    try do
      ...
      while_fun = fn while_fun, cur, i, parts ->
        if i < _len(p) do
          {cur, parts} = if _slice(p, i, i + 1 - (i)) == "/" do
            {cur, parts} = if cur != "" do
              parts = (parts ++ [cur])
              cur = ""
              {cur, parts}
  ...
  def commonPrefix(paths) do
    try do
      if _len(paths) == 0 do
        throw {:return, ""}
      end
      base = splitPath(Enum.at(paths, 0))
      i = 0
      prefix = []
      while_fun_3 = fn while_fun_3, i, prefix ->
        if i < _len(base) do
          comp = Enum.at(base, i)
          ok = true
          {ok} = Enum.reduce(paths, {ok}, fn p, {ok} ->
            try do
              parts = splitPath(p)
              {ok} = if i >= _len(parts) || Enum.at(parts, i) != comp do
                ok = false
                throw {:break, {i, prefix}}
                {ok}
              else
                {ok}
              end
          ...
          i = i + 1
          while_fun_3.(while_fun_3, i, prefix)
        else
          {i, prefix}
        end
      end
      {i, prefix} = try do
          ...
          {:break, {i, prefix}} -> {i, prefix}
        end

      throw {:return, joinPath(prefix)}
    catch
      {:return, val} -> val
    end
  end
  def main() do
    try do
      ...
      if c == "" do
        IO.puts("No common path")
      else
        IO.puts(("Common path: " <> c))
      end
    catch
      {:return, val} -> val
    end
