...
defmodule Main do
  ...
  def median(s) do
    try do
      n = _len(s)
      if rem(n, 2) == 1 do
        throw {:return, Enum.at(s, div(n, 2))}
      end
      throw {:return, (Enum.at(s, div(n, 2) - 1) + Enum.at(s, div(n, 2))) / 2.0}
    catch
      {:return, val} -> val
    end
  end
  def fivenum(xs) do
    try do
      s = sortFloat(xs)
      n = _len(s)
      q1 = Enum.at(s, div((n - 1), 4))
      med = median(s)
      q3 = Enum.at(s, div((3 * (n - 1)), 4))
      throw {:return, [Enum.at(s, 0), q1, med, q3, Enum.at(s, n - 1)]}
    catch
      {:return, val} -> val
    end
  end
