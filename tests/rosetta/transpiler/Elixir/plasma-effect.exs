...
defmodule Main do
  ...
  def main() do
    ...
    while_fun_3 = fn while_fun_3 ->
      if Process.get(:f) <= Process.get(:nframes) do
        y = 0
        while_fun_4 = fn while_fun_4, y ->
          if y < Process.get(:h) do
            x = 0
            while_fun_5 = fn while_fun_5, x ->
              if x < Process.get(:w) do
                fx = :erlang.float(x)
                fy = :erlang.float(y)
                ...
                value = value + 4.0
                value = value / 8.0
                rem_ = frac(value + (:erlang.float(Process.get(:f))) / (:erlang.float(Process.get(:nframes))))
                ci = (fn v -> if is_binary(v), do: String.to_integer(v), else: trunc(v) end).(((:erlang.float(Process.get(:nframes))) * rem_)) + 1
                Process.put(:total, Process.get(:total) + ci)
                x = x + 1
