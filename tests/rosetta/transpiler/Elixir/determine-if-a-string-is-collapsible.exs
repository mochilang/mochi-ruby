...
defmodule Main do
  ...
  def collapse(s) do
    try do
      ...
      res = ""
      orig = _len(s)
      while_fun = fn while_fun, i, prev, res ->
        if i < _len(s) do
          ch = _slice(s, i, i + 1 - (i))
          {prev, res} = if ch != prev do
            res = (res <> ch)
            prev = ch
            {prev, res}
          else
          ...
          i = i + 1
          while_fun.(while_fun, i, prev, res)
        else
          {i, prev, res}
        end
      end
      {i, prev, res} = try do
          ...
          {:break, {i, prev, res}} -> {i, prev, res}
        end

      throw {:return, [res, orig, _len(res)]}
    catch
      {:return, val} -> val
    end
  end
  def main() do
    try do
      ...
      while_fun_2 = fn while_fun_2, idx ->
        if idx < _len(strings) do
          s = Enum.at(strings, idx)
          r = collapse(s)
          cs = Enum.at(r, 0)
          olen = Enum.at(r, 1)
          clen = Enum.at(r, 2)
          IO.puts((((("original : length = " <> Kernel.inspect(olen)) <> ", string = «««") <> s) <> "»»»"))
          IO.puts((((("collapsed: length = " <> Kernel.inspect(clen)) <> ", string = «««") <> cs) <> "»»»\n"))
          idx = idx + 1
