    ...
    (let {
        loop = do
            if (deref i) < exp then do
                writeIORef result $! ((deref result) * base)
                writeIORef i $! ((deref i) + 1)
                loop
            else return ((deref result))
    } in loop)


ackermann2 m n = if m == 0 then n + 1 else if m == 1 then n + 2 else if m == 2 then 2 * n + 3 else if m == 3 then 8 * pow 2 n - 3 else if n == 0 then ackermann2 (m - 1) 1 else ackermann2 (m - 1) (ackermann2 m (n - 1))

mainEntry = do
    putStrLn ("A(0, 0) = " ++ show (ackermann2 0 0))
    putStrLn ("A(1, 2) = " ++ show (ackermann2 1 2))
    putStrLn ("A(2, 4) = " ++ show (ackermann2 2 4))
    putStrLn ("A(3, 4) = " ++ show (ackermann2 3 4))
