...
function cz(yr, animal, yinYang, element, sc, bc)
  ...
  local stem = (y % 10)
  local branch = (y % 12)
  local sb = (sc[stem + 1] .. bc[branch + 1])
  return {animal = animal[branch + 1], yinYang = yinYang[(stem % 2) + 1], element = element[math.floor((stem // 2)) + 1], stemBranch = sb, cycle = ((y % 60) + 1)}
end;

for _, yr in ipairs({1935, 1938, 1968, 1972, 1976}) do
  r = cz(yr, animal, yinYang, element, stemChArr, branchChArr)
  print(((((((((((tostring(yr) .. ": ") .. r.element) .. " ") .. r.animal) .. ", ") .. r.yinYang) .. ", Cycle year ") .. tostring(r.cycle)) .. " ") .. r.stemBranch))
end;
