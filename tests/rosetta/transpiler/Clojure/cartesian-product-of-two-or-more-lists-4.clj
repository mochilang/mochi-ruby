...
(def ^:dynamic cartN_last nil)

(def ^:dynamic cartN_left nil)

(def ^:dynamic cartN_out nil)

(def ^:dynamic cartN_row nil)

(def ^:dynamic copy_out nil)
...
(defn copy [copy_xs]
  (binding [copy_out nil] (try (do (set! copy_out []) (doseq [v copy_xs] (set! copy_out (conj copy_out v))) (throw (ex-info "return" {:v copy_out}))) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e))))))

(defn cartN [cartN_lists]
  (binding [cartN_a nil cartN_last nil cartN_left nil cartN_out nil cartN_row nil] (try (do (when (= cartN_lists nil) (throw (ex-info "return" {:v []}))) (set! cartN_a cartN_lists) (when (= (count cartN_a) 0) (throw (ex-info "return" {:v [[]]}))) (set! cartN_out []) (set! cartN_last (- (count cartN_a) 1)) (set! cartN_left (cartN (subvec cartN_a 0 cartN_last))) (doseq [p cartN_left] (doseq [x (nth cartN_a cartN_last)] (do (set! cartN_row (copy p)) (set! cartN_row (conj cartN_row x)) (set! cartN_out (conj cartN_out cartN_row))))) (throw (ex-info "return" {:v cartN_out}))) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e))))))
