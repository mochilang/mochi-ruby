(ns main (:refer-clojure :exclude [pixelFromRgb newBitmap setPx fill fillRgb line bezier3]))
...
(declare pixelFromRgb newBitmap setPx fill fillRgb line bezier3)
...
(defn bezier3 [b x1 y1 x2 y2 x3 y3 x4 y4 p]
  (do (def px []) (def py []) (def i 0) (while (<= i b3Seg) (do (def px (conj px 0)) (def py (conj py 0)) (def i (+ i 1)))) (def fx1 (double x1)) (def fy1 (double y1)) (def fx2 (double x2)) (def fy2 (double y2)) (def fx3 (double x3)) (def fy3 (double y3)) (def fx4 (double x4)) (def fy4 (double y4)) (def i 0) (while (<= i b3Seg) (do (def d (/ (double i) (double b3Seg))) (def a (- 1.0 d)) (def bcoef (* a a)) (def ccoef (* d d)) (def a2 (* a bcoef)) (def b2 (* (* 3.0 bcoef) d)) (def c2 (* (* 3.0 a) ccoef)) (def d2 (* ccoef d)) (def px (assoc px i (int (+ (+ (+ (* a2 fx1) (* b2 fx2)) (* c2 fx3)) (* d2 fx4))))) (def py (assoc py i (int (+ (+ (+ (* a2 fy1) (* b2 fy2)) (* c2 fy3)) (* d2 fy4))))) (def i (+ i 1)))) (def x0 (nth px 0)) (def y0 (nth py 0)) (def i 1) (while (<= i b3Seg) (do (def x (nth px i)) (def y (nth py i)) (line b x0 y0 x y p) (def x0 x) (def y0 y) (def i (+ i 1))))))
