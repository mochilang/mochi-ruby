// Generated by Mochi v0.10.56 on 2025-08-04 00:19:59 GMT+7
...
def fivenum(a: ArrayBuffer[Double]): ArrayBuffer[Double] = {
  ...
  val n: Int = (arr).size
  val half: BigInt = n + BigInt(3).toInt - (n + BigInt(3).toInt) % BigInt(2)
  ...
  while (idx < BigInt(((d).size).toInt)) {
    val de: Double = d((idx.toInt).toInt)
    val fl: BigInt = (de - 1.0).toInt
    val cl: BigInt = ceilf(de - 1.0)
    result = result :+ 0.5 * (arr((fl.toInt).toInt) + arr((cl.toInt).toInt))
    idx = idx + BigInt(1)
  }
  return result
}
