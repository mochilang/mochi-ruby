// Generated by Mochi v0.10.52 on 2025-08-02 02:06:57 GMT+7
...
def main(): Any = {
  ...
  while (idx < (faces).size) {
    val per = faceToPerim(faces((idx).toInt))
    if (per == null) {
      println("  " + names((idx).toInt) + " => Invalid edge format")
    } else {
      println("  " + names((idx).toInt) + " => " + listStr(per.asInstanceOf[ArrayBuffer[BigInt]]))
    }
    idx = idx + BigInt(1)
  }
}
