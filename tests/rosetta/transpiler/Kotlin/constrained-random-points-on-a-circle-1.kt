...
fun main() {
    run {
        ...
        while (n < nPts) {
            var x: BigInteger = ((_now()).toBigInteger().remainder((span))).subtract((rMax).toBigInteger())
            var y: BigInteger = ((_now()).toBigInteger().remainder((span))).subtract((rMax).toBigInteger())
            var rs: BigInteger = (x.multiply((x))).add((y.multiply((y))))
            if ((rs.compareTo((min2)) < 0) || (rs.compareTo((max2)) > 0)) {
                continue
            }
            n = n + 1
            var row: BigInteger = y.add((rMax).toBigInteger())
            var col: BigInteger = (x.add((rMax).toBigInteger())).multiply((2).toBigInteger())
            (rows[(row).toInt()]!!)[(col).toInt()] = "*"
            var key: String = (row.toString() + ",") + col.toString()
            if (!(((seen)[key]) as? Boolean ?: false)) {
                (seen)[key] = true
                u = u + 1
            }
        }
        while ((i).toBigInteger().compareTo((span)) < 0) {
            var line: String = ""
            var j: Int = 0
            while ((j).toBigInteger().compareTo((span.multiply((2).toBigInteger()))) < 0) {
                line = line + (((rows[i]!!) as MutableList<String>))[j]!!
                j = j + 1
            }
            println(line)
            i = i + 1
