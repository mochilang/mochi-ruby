...
fun H(data: String): Double {
    ...
    var i: Int = 0
    while (i < data.length) {
        val ch: String = data.substring(i, i + 1)
        if (ch in counts) {
    ...
    val l: Double = data.length.toDouble()
    for (ch in counts.keys) {
        val px: Double = ((counts)[ch] as Int).toDouble() / l
        if (px > 0.0) {
