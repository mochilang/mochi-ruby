...
(_return (if s (list-ref s (int (quotient n 2))) #f))
) (void))
(_return (/ (let ([__l (if s (list-ref s (int (- (quotient n 2) 1))) #f)] [__r (if s (list-ref s (int (quotient n 2))) #f)]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))) 2.0))
))
)
(define (fivenum xs)
  (let/ec _return (begin
(define s (sortFloat xs))
(define n (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)]))
(define q1 (if s (list-ref s (int (quotient (- n 1) 4))) #f))
(define med (median s))
(define q3 (if s (list-ref s (int (quotient (* 3 (- n 1)) 4))) #f))
(_return (list (if s (list-ref s (int 0)) #f) q1 med q3 (if s (list-ref s (int (- n 1))) #f)))
))
)
