...
(let/ec _break (let loop ()
  (if (< r 2) (let ()
    (define i 0)
    (let/ec _break (let loop ()
  (if (< i (cond [(string? p1) (string-length p1)] [(hash? p1) (hash-count p1)] [else (length p1)])) (let ()
...
(define idx2 0)
(let/ec _break (let loop ()
  (if (< idx2 (cond [(string? faces) (string-length faces)] [(hash? faces) (hash-count faces)] [else (length faces)])) (let ()
    (define per (faceToPerim (if faces (list-ref faces (int idx2)) #f)))
    (if (equal? per #f) (let ()
(displayln (string-append (string-append "  " (if names (list-ref names (int idx2)) #f)) " => Invalid edge format"))
) (let ()
(displayln (string-append (string-append (string-append "  " (if names (list-ref names (int idx2)) #f)) " => ") (listStr per)))
))
    (set! idx2 (let ([__l idx2] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
  ))
