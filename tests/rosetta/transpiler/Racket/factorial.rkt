...
(let/ec _break (let ([i 0])
  (let loop ()
    (when (< i 11)
(displayln (string-append (string-append (format "~a" i) " ") (format "~a" (factorial i))))
      (set! i (+ i 1))
      (loop)))
))
(displayln (string-append "100 " (format "~a" (factorial 100))))
(displayln (string-append "800 " (format "~a" (factorial 800))))
  ))
