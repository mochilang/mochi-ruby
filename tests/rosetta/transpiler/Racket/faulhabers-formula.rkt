...
(define (coeff p j)
  (let/ec _return (begin
(define base (/ 1 (let ([__l p] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
(define c base)
(if (equal? (modulo j 2) 1) (let ()
(set! c (- c))
) (void))
...
(let/ec _break (let loop ()
  (if (< p 10) (let ()
    (define line (string-append (format "~a" p) " :"))
    (define j 0)
    (let/ec _break (let loop ()
  (if (<= j p) (let ()
    (define c (coeff p j))
    (if (not (string=? (format "~a" c) "0/1")) (let ()
(set! line (string-append (string-append (string-append line " ") (format "~a" c)) "×n"))
(define exp (- (let ([__l p] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))) j))
(if (> exp 1) (let ()
(set! line (string-append (string-append line "^") (format "~a" exp)))
) (void))
) (void))
