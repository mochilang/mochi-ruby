...
(let/ec _break (let loop ()
  (if (< i (cond [(string? pieces) (string-length pieces)] [(hash? pieces) (hash-count pieces)] [else (length pieces)])) (let ()
    ...
    (set! res (string-append res (if g2lMap (hash-ref g2lMap ch #f) #f)))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(_return res)
))
...
(let/ec _break (for ([one (list "K" "Q")])
  (let/ec _cont
(if (not (equal? (countChar pieces one) 1)) (let ()
(_return (- 1))
) (void))
  )))
(let/ec _break (for ([two (list "R" "N" "B")])
  (let/ec _cont
(if (not (equal? (countChar pieces two) 2)) (let ()
(_return (- 1))
) (void))
  )))
...
(define piecesN (removeChar (removeChar pieces "Q") "B"))
...
(define N (if ntable (hash-ref ntable (string-append (format "~a" n1) (format "~a" n2)) #f) #f))
(define piecesQ (removeChar pieces "B"))
