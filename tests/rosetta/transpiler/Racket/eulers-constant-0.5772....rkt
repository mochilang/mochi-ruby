...
(let/ec _break (let loop ()
  (if (<= i n) (let ()
    (set! sum (let ([__l sum] [__r (/ 1.0 (exact->inexact i))]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
...
(define (main)
  (let/ec _return (begin
(define n 100000)
(define gamma (- (harmonic n) (ln (exact->inexact n))))
(displayln (format "~a" gamma))
))
)
(main)
  ))
