...
auto getBins(auto limits, auto data) {
  auto n = limits.size();
  auto bins = std::vector<int>{};
  auto i = 0;
  while ((i < (n + 1))) {
    bins.push_back(0);
    i = (i + 1);
...
auto printBins(auto limits, auto bins) {
  auto n = limits.size();
  std::cout << (((std::string("           < ") + padLeft(limits[0], 3)) +
                 std::string(" = ")) +
                padLeft(bins[0], 2))
            << std::endl;
  auto i = 1;
  while ((i < n)) {
    std::cout << (((((std::string(">= ") + padLeft(limits[(i - 1)], 3)) +
                     std::string(" and < ")) +
                    padLeft(limits[i], 3)) +
                   std::string(" = ")) +
                  padLeft(bins[i], 2))
  ...
  std::cout << (((std::string(">= ") + padLeft(limits[(n - 1)], 3)) +
                 std::string("           = ")) +
                padLeft(bins[n], 2))
            << std::endl;
...
auto main() {
  auto limitsList =
      std::vector<decltype(std::vector<int>{23, 37, 43, 53, 67, 83})>{
          std::vector<int>{23, 37, 43, 53, 67, 83},
          std::vector<int>{14, 18, 249, 312, 389, 392, 513, 591, 634, 720}};
  auto dataList = std::vector<decltype(std::vector<int>{
      ...
      std::vector<int>{95, 21, 94, 12, 99, 4,  70, 75, 83, 93, 52, 80, 57,
  ...
  while ((i < limitsList.size())) {
    std::cout << ((std::string("Example ") + std::to_string((i + 1))) +
                  std::string("\n"))
              << std::endl;
    auto bins = getBins(limitsList[i], dataList[i]);
