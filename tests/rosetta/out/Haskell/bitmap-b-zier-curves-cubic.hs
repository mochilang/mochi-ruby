...
bezier3 :: Map.Map String () -> Int -> Int -> Int -> Int -> Int -> Int -> Int -> Int -> Pixel -> ()
bezier3 b x1 y1 x2 y2 x3 y3 x4 y4 p =
  fromMaybe (()) $
    (let px = [] in (let py = [] in (let i = 0 in case whileLoop (\() -> (_asInt (i) <= b3Seg)) (\() -> (let px = (px ++ [0]) in (let py = (py ++ [0]) in (let i = (_asInt (i) + 1) in Nothing)))) of Just v -> Just v; Nothing -> (let fx1 = (read x1 :: Double) in (let fy1 = (read y1 :: Double) in (let fx2 = (read x2 :: Double) in (let fy2 = (read y2 :: Double) in (let fx3 = (read x3 :: Double) in (let fy3 = (read y3 :: Double) in (let fx4 = (read x4 :: Double) in (let fy4 = (read y4 :: Double) in (let i = 0 in case whileLoop (\() -> (_asInt (i) <= b3Seg)) (\() -> (let d = (((read i :: Double)) / ((read b3Seg :: Double))) in (let a = (1.0 - _asDouble (d)) in (let bcoef = (a * a) in (let ccoef = (d * d) in (let a2 = (a * bcoef) in (let b2 = ((3.0 * _asDouble (bcoef)) * _asDouble (d)) in (let c2 = ((3.0 * _asDouble (a)) * _asDouble (ccoef)) in (let d2 = (ccoef * d) in (let px = Map.insert i (read ((((((((a2 * fx1) + b2) * fx2) + c2) * fx3) + d2) * fx4)) :: Int) px in (let py = Map.insert i (read ((((((((a2 * fy1) + b2) * fy2) + c2) * fy3) + d2) * fy4)) :: Int) py in (let i = (_asInt (i) + 1) in Nothing)))))))))))) of Just v -> Just v; Nothing -> (let x0 = (px !! 0) in (let y0 = (py !! 0) in (let i = 1 in whileLoop (\() -> (_asInt (i) <= b3Seg)) (\() -> (let x = (px !! i) in (let y = (py !! i) in case (let _ = line b x0 y0 x y p in Nothing) of Just v -> Just v; Nothing -> (let x0 = x in (let y0 = y in (let i = (_asInt (i) + 1) in Nothing)))))))))))))))))))))
