...
median a =
  fromMaybe (0.0) $
    (let arr = sortFloat a in (let half = (read ((div length arr 2)) :: Int) in (let m = (arr !! half) in case if ((length arr `mod` 2) == 0) then (let m = (_asDouble (((m + (arr !! (_asInt (half) - 1))))) / 2.0) in Nothing) else Nothing of Just v -> Just v; Nothing -> Just (m))))
