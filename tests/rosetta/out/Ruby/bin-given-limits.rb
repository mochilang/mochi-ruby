...
def printBins(limits, bins)
	n = (limits).length
	puts(((("           < " + padLeft(limits[0], 3)) + " = ") + padLeft(bins[0], 2)))
	i = 1
	while (i < n)
		puts((((((">= " + padLeft(limits[(i - 1)], 3)) + " and < ") + padLeft(limits[i], 3)) + " = ") + padLeft(bins[i], 2)))
		i = (i + 1)
	end
	puts((((">= " + padLeft(limits[(n - 1)], 3)) + "           = ") + padLeft(bins[n], 2)))
	puts("")
end
