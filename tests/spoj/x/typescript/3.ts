...
function contains(a: string, b: string): boolean {
  ...
  let i: number = 0;
  while ((i <= (_len(a) - _len(b)))) {
    if ((a.slice(i, (i + _len(b))) === b)) {
      return true;
    }
    i = (i + 1);
  }
...
function main() {
  ...
  while ((line !== "")) {
    let pos: number = 0;
    while (((pos < _len(line)) && (line.slice(pos, (pos + 1)) !== " "))) {
      pos = (pos + 1);
    }
    let a: string = line.slice(0, pos);
    let b: string = line.slice((pos + 1), _len(line));
    if (contains(a, b)) {
      console.log(_str(1));
    } else {
      console.log(_str(0));
