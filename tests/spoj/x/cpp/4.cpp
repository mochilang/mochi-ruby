// Generated by Mochi transpiler v0.10.74 on 2025-08-26 14:34:31 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::string popTop(const std::vector<std::string>& stack);
std::vector<std::string> popStack(const std::vector<std::string>& stack);
...
std::string popTop(const std::vector<std::string>& stack) {
    return _index(stack, (stack.size() - int64_t(1)));
}

std::vector<std::string> popStack(const std::vector<std::string>& stack) {
    std::vector<std::string> newStack = {};
    int64_t i = int64_t(0);
    while ((i < (stack.size() - int64_t(1)))) {
        newStack = ([&]{ auto __tmp = newStack; __tmp.push_back(_index(stack, i)); return __tmp; }());
        i = (i + int64_t(1));
    }
    return newStack;
}
...
std::string toRPN(std::string expr) {
    ...
    int64_t i = int64_t(0);
    while ((i < expr.size())) {
        std::string ch = ([&](const auto& __s){ auto __i = i; if (__i < 0) __i += __s.size(); return std::string(1, __s[static_cast<size_t>(__i)]); })(expr);
        if (((ch >= std::string("a")) && (ch <= std::string("z")))) {
            ...
            int64_t prec = precedence(ch);
            while ((stack.size() > int64_t(0))) {
                std::string top = popTop(stack);
                if ((top == std::string("("))) {
                    break;
                }
                int64_t topPrec = precedence(top);
                if (((topPrec > prec) || ((topPrec == prec) && (ch != std::string("^"))))) {
                    out = (out + top);
                    stack = popStack(stack);
                } else {
                    break;
...
int main() {
    {
        ...
        int64_t t = std::stoll(([&]{ std::string __line; std::getline(std::cin, __line); return __line; }()));
        int64_t i = int64_t(0);
        while ((i < t)) {
