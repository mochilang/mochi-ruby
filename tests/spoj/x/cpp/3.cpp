// Generated by Mochi transpiler v0.10.74 on 2025-08-26 14:34:28 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
int64_t is_substring(std::string a, std::string b);
std::vector<int64_t> solve(const std::vector<std::string>& lines);
int main();

std::vector<std::string> sample_input = std::vector<std::string>{std::string("1010110010 10110"), std::string("1110111011 10011")};
...
int64_t is_substring(std::string a, std::string b) {
    ...
    int64_t i = int64_t(0);
    while (((i + lb) <= la)) {
        if ((([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(i, (i + lb) - i); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+i, __v.begin()+(i + lb)); })(a) == b)) {
            return int64_t(1);
        }
        i = (i + int64_t(1));
    }
...
std::vector<int64_t> solve(const std::vector<std::string>& lines) {
    ...
    for (std::string line : lines) {
        ...
        int64_t i = int64_t(0);
        while ((i < line.size())) {
            std::string ch = ([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(i, (i + int64_t(1)) - i); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+i, __v.begin()+(i + int64_t(1))); })(line);
            if ((ch == std::string(" "))) {
        ...
        parts = ([&]{ auto __tmp = parts; __tmp.push_back(cur); return __tmp; }());
        std::string a = _index(parts, int64_t(0));
        std::string b = _index(parts, int64_t(1));
        res = ([&]{ auto __tmp = res; __tmp.push_back(is_substring(a, b)); return __tmp; }());
    }
    return res;
}
...
int main() {
    {
        ...
        for (int64_t r : solve(sample_input)) {
            std::cout << r;
            std::cout << std::endl;
        }
    }
