// Generated by Mochi transpiler v0.10.74 on 2025-08-26 14:34:24 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::vector<int64_t> precompute(int64_t limit);
int main();

std::vector<int64_t> primes = std::vector<int64_t>{};
int64_t t = 0;
int64_t case_idx = int64_t(0);
...
std::vector<int64_t> precompute(int64_t limit) {
    ...
    for (int64_t i = int64_t(0); i < (limit + int64_t(1)); i++ ) {
        sieve = ([&]{ auto __tmp = sieve; __tmp.push_back(true); return __tmp; }());
    }
    _index(sieve, int64_t(0)) = false;
    _index(sieve, int64_t(1)) = false;
    int64_t p = int64_t(2);
    while (((p * p) <= limit)) {
        if (_index(sieve, p)) {
            int64_t j = (p * p);
            while ((j <= limit)) {
                _index(sieve, j) = false;
                j = (j + p);
            }
        }
        p = (p + int64_t(1));
    ...
    for (int64_t i = int64_t(2); i < (limit + int64_t(1)); i++ ) {
        if (_index(sieve, i)) {
...
int main() {
    {
        ...
        primes = precompute(int64_t(32000));
        t = std::stoll(([&]{ std::string __line; std::getline(std::cin, __line); return __line; }()));
        while ((case_idx < t)) {
            std::string line = ([&]{ std::string __line; std::getline(std::cin, __line); return __line; }());
            std::vector<std::string> parts = _split(line, std::string(" "));
            int64_t m = std::stoll(_index(parts, int64_t(0)));
            int64_t n = std::stoll(_index(parts, int64_t(1)));
            int64_t size = ((n - m) + int64_t(1));
            std::vector<bool> segment = {};
            for (int64_t i = int64_t(0); i < size; i++ ) {
                segment = ([&]{ auto __tmp = segment; __tmp.push_back(true); return __tmp; }());
            }
            for (int64_t p : primes) {
                ...
                int64_t j = start;
                while ((j <= n)) {
                    _index(segment, (j - m)) = false;
                    j = (j + p);
                }
            }
            if ((m == int64_t(1))) {
                _index(segment, int64_t(0)) = false;
            }
            int64_t i = int64_t(0);
            while ((i < size)) {
                if (_index(segment, i)) {
            ...
            if ((case_idx < (t - int64_t(1)))) {
                std::cout << std::string("");
                std::cout << std::endl;
            }
            case_idx = (case_idx + int64_t(1));
        }
    }
    return 0;
}
