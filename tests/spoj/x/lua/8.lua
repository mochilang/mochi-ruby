...
function main()
  ...
  local current = seq
  while (_len(current) > 1) do
    local next = {}
    local i = 0
    while ((i + 1) < _len(current)) do
      next = (function(lst, item)
      local res = {table.unpack(lst or {})}
      res[#res+1] = item
      return res
    end)(next, (current[(i + 1) + 1] - current[i + 1]))
    i = (i + 1)
  end
  levels = (function(lst, item)
  local res = {table.unpack(lst or {})}
...
local depth = (_len(levels) - 1)
local step = 0
local res = {}
while (step < c) do
  local bottom = levels[depth + 1]
  bottom = (function(lst, item)
  local res = {table.unpack(lst or {})}
  res[#res+1] = item
  return res
end)(bottom, bottom[(_len(bottom) - 1) + 1])
levels[depth + 1] = bottom
local level = (depth - 1)
while (level >= 0) do
  local arr = levels[level + 1]
  local arrBelow = levels[(level + 1) + 1]
  local nextVal = (arr[(_len(arr) - 1) + 1] + arrBelow[(_len(arrBelow) - 1) + 1])
...
res[#res+1] = item
return res
end)(res, levels[0 + 1][(_len(levels[0 + 1]) - 1) + 1])
step = (step + 1)
end
local out = ""
local i2 = 0
while (i2 < _len(res)) do
  if (i2 > 0) then
    out = (out .. " ")
  end
  out = (out .. _str(res[i2 + 1]))
  i2 = (i2 + 1)
end
