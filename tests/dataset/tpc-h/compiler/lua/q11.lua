...
function test_Q11_returns_high_value_partkeys_from_GERMANY()
    if not ((result == {{["ps_partkey"]=1000, ["value"]=2000.0}, {["ps_partkey"]=2000, ["value"]=50.0}})) then error('expect failed') end
end
...
filtered = (function()
    local _src = partsupp
    return __query(_src, {
        { items = supplier, on = function(ps, s) return (s.s_suppkey == ps.ps_suppkey) end },
        { items = nation, on = function(ps, s, n) return (n.n_nationkey == s.s_nationkey) end }
    }, { selectFn = function(ps, s, n) return {["ps_partkey"]=ps.ps_partkey, ["value"]=(ps.ps_supplycost * ps.ps_availqty)} end, where = function(ps, s, n) return ((n.n_name == target_nation)) end })
end)()
grouped = (function()
