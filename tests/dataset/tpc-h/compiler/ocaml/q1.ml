...
let result : record4 list = (let (__groups0 : (record2 * record1 list) list ref) = ref [] in
  ...
  List.iter (fun ((gKey : record2), gItems) ->
    ...
    __res0 := { returnflag = g.key.returnflag; linestatus = g.key.linestatus; sum_qty = (float_of_int (sum (let __res1 = ref [] in
  List.iter (fun x ->
...
List.rev !__res5)
)); avg_price = (List.fold_left (+.) 0.0 (let __res6 = ref [] in
  List.iter (fun x ->
...
List.rev !__res6)
)); avg_disc = (List.fold_left (+.) 0.0 (let __res7 = ref [] in
  List.iter (fun x ->
