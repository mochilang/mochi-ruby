; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:09Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q9_computes_profit_for_green_parts_by_nation_and_year)
  (define revenue (* 1000.0 0.9))
  (define cost (* 5 10.0))
  (when (not (equal? result (list (list (cons 'nation "BRAZIL") (cons 'o_year "1995") (cons 'profit (- revenue cost)))))) (error "expect failed"))
)

(define nation (list (list (cons 'n_nationkey 1) (cons 'n_name "BRAZIL")) (list (cons 'n_nationkey 2) (cons 'n_name "CANADA"))))
(define supplier (list (list (cons 's_suppkey 100) (cons 's_nationkey 1)) (list (cons 's_suppkey 200) (cons 's_nationkey 2))))
(define part (list (list (cons 'p_partkey 1000) (cons 'p_name "green metal box")) (list (cons 'p_partkey 2000) (cons 'p_name "red plastic crate"))))
(define partsupp (list (list (cons 'ps_partkey 1000) (cons 'ps_suppkey 100) (cons 'ps_supplycost 10.0)) (list (cons 'ps_partkey 1000) (cons 'ps_suppkey 200) (cons 'ps_supplycost 8.0))))
(define orders (list (list (cons 'o_orderkey 1) (cons 'o_orderdate "1995-02-10")) (list (cons 'o_orderkey 2) (cons 'o_orderdate "1997-01-01"))))
(define lineitem (list (list (cons 'l_orderkey 1) (cons 'l_partkey 1000) (cons 'l_suppkey 100) (cons 'l_quantity 5) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.1)) (list (cons 'l_orderkey 2) (cons 'l_partkey 1000) (cons 'l_suppkey 200) (cons 'l_quantity 10) (cons 'l_extendedprice 800.0) (cons 'l_discount 0.05))))
(define prefix "green")
...
(define result (let ((_tmp '()))
      ...
      (when (equal? (map-get p 'p_partkey) (map-get l 'l_partkey))
        (for-each (lambda (s)
          (when (equal? (map-get s 's_suppkey) (map-get l 'l_suppkey))
            (for-each (lambda (ps)
              (when (and (equal? (map-get ps 'ps_partkey) (map-get l 'l_partkey)) (equal? (map-get ps 'ps_suppkey) (map-get l 'l_suppkey)))
                (for-each (lambda (o)
                  (when (equal? (map-get o 'o_orderkey) (map-get l 'l_orderkey))
                    (for-each (lambda (n)
                      (when (equal? (map-get n 'n_nationkey) (map-get s 's_nationkey))
                        (when (and (and (equal? (substring (map-get p 'p_name) 0 (string-length prefix)) prefix) (_ge (map-get o 'o_orderdate) start_date)) (_le (map-get o 'o_orderdate) end_date))
  ...
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 'nation (map-get (map-get g 'key) 'nation)) (cons 'o_year (let ((s (open-output-string))) (write (map-get (map-get g 'key) 'o_year) s) (get-output-string s))) (cons 'profit (_sum (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (- (* (map-get (map-get x 'l) 'l_extendedprice) (- 1 (map-get (map-get x 'l) 'l_discount))) (* (map-get (map-get x 'ps) 'ps_supplycost) (map-get (map-get x 'l) 'l_quantity))))))
  ) (if (string? g) (string->list g) g))
  _res)))))))
    ) (_group_by _tmp (lambda (l) (list (cons 'nation (map-get n 'n_name)) (cons 'o_year (inexact->exact (substring (map-get o 'o_orderdate) 0 4)))))))
    (set! _res (_sort (map (lambda (x) (cons x (list (map-get (map-get g 'key) 'nation) (- (map-get (map-get g 'key) 'o_year))))) _res)))
    (set! _res (map car _res))
    _res)))
(_json result)
(run-test "Q9 computes profit for green parts by nation and year" test_Q9_computes_profit_for_green_parts_by_nation_and_year)
