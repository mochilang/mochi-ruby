; Generated by Mochi compiler v0.10.25 on 2025-07-13T16:37:26Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q3_returns_revenue_per_order_with_correct_priority)
  (when (not (equal? order_line_join (list (list (cons 'l_orderkey 100) (cons 'revenue (+ (* 1000.0 0.95) 500.0)) (cons 'o_orderdate "1995-03-14") (cons 'o_shippriority 1))))) (error "expect failed"))
)

(define customer (list (list (cons 'c_custkey 1) (cons 'c_mktsegment "BUILDING")) (list (cons 'c_custkey 2) (cons 'c_mktsegment "AUTOMOBILE"))))
(define orders (list (list (cons 'o_orderkey 100) (cons 'o_custkey 1) (cons 'o_orderdate "1995-03-14") (cons 'o_shippriority 1)) (list (cons 'o_orderkey 200) (cons 'o_custkey 2) (cons 'o_orderdate "1995-03-10") (cons 'o_shippriority 2))))
(define lineitem (list (list (cons 'l_orderkey 100) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.05) (cons 'l_shipdate "1995-03-16")) (list (cons 'l_orderkey 100) (cons 'l_extendedprice 500.0) (cons 'l_discount 0.0) (cons 'l_shipdate "1995-03-20")) (list (cons 'l_orderkey 200) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.1) (cons 'l_shipdate "1995-03-14"))))
(define cutoff "1995-03-15")
(define segment "BUILDING")
(define building_customers (let ((_res '()))
  (for-each (lambda (c)
    (when (equal? (map-get c 'c_mktsegment) segment)
      (set! _res (append _res (list c)))
    )
  ) (if (string? customer) (string->list customer) customer))
  _res))
(define valid_orders (let ((_res '()))
  (for-each (lambda (o)
    (for-each (lambda (c)
      (when (equal? (map-get o 'o_custkey) (map-get c 'c_custkey))
        (when (_lt (map-get o 'o_orderdate) cutoff)
          (set! _res (append _res (list o)))
        )
      )) (if (string? building_customers) (string->list building_customers) building_customers))
  ) (if (string? orders) (string->list orders) orders))
  _res))
(define valid_lineitems (let ((_res '()))
  (for-each (lambda (l)
    (when (_gt (map-get l 'l_shipdate) cutoff)
      (set! _res (append _res (list l)))
    )
  ) (if (string? lineitem) (string->list lineitem) lineitem))
  _res))
(define order_line_join (let ((_tmp '()))
  (for-each (lambda (o)
    (for-each (lambda (l)
      (when (equal? (map-get l 'l_orderkey) (map-get o 'o_orderkey))
        (set! _tmp (append _tmp (list o)))
      )) (if (string? valid_lineitems) (string->list valid_lineitems) valid_lineitems))
  ) (if (string? valid_orders) (string->list valid_orders) valid_orders))
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 'l_orderkey (map-get (map-get g 'key) 'o_orderkey)) (cons 'revenue (_sum (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (* (map-get (map-get r 'l) 'l_extendedprice) (- 1 (map-get (map-get r 'l) 'l_discount))))))
  ) (if (string? g) (string->list g) g))
  _res))) (cons 'o_orderdate (map-get (map-get g 'key) 'o_orderdate)) (cons 'o_shippriority (map-get (map-get g 'key) 'o_shippriority))))))
    ) (_group_by _tmp (lambda (o) (list (cons 'o_orderkey (map-get o 'o_orderkey)) (cons 'o_orderdate (map-get o 'o_orderdate)) (cons 'o_shippriority (map-get o 'o_shippriority))))))
    (set! _res (_sort (map (lambda (x) (cons x (list (- (_sum (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (* (map-get (map-get r 'l) 'l_extendedprice) (- 1 (map-get (map-get r 'l) 'l_discount))))))
  ) (if (string? g) (string->list g) g))
  _res))) (map-get (map-get g 'key) 'o_orderdate)))) _res)))
    (set! _res (map car _res))
    _res)))
(_json order_line_join)
(run-test "Q3 returns revenue per order with correct priority" test_Q3_returns_revenue_per_order_with_correct_priority)
