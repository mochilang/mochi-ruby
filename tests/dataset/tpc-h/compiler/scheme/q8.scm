; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:06Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q8_returns_correct_market_share_for_BRAZIL_in_1995)
  (define numerator (* 1000.0 0.9))
  (define denominator (+ numerator (* 500.0 0.95)))
  (define share (/ numerator denominator))
  (when (not (equal? result (list (list (cons 'o_year "1995") (cons 'mkt_share share))))) (error "expect failed"))
)

(define region (list (list (cons 'r_regionkey 0) (cons 'r_name "AMERICA"))))
(define nation (list (list (cons 'n_nationkey 10) (cons 'n_regionkey 0) (cons 'n_name "BRAZIL")) (list (cons 'n_nationkey 20) (cons 'n_regionkey 0) (cons 'n_name "CANADA"))))
(define customer (list (list (cons 'c_custkey 1) (cons 'c_nationkey 10)) (list (cons 'c_custkey 2) (cons 'c_nationkey 20))))
(define orders (list (list (cons 'o_orderkey 100) (cons 'o_custkey 1) (cons 'o_orderdate "1995-04-10")) (list (cons 'o_orderkey 200) (cons 'o_custkey 2) (cons 'o_orderdate "1995-07-15"))))
(define lineitem (list (list (cons 'l_orderkey 100) (cons 'l_suppkey 1000) (cons 'l_partkey 5000) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.1)) (list (cons 'l_orderkey 200) (cons 'l_suppkey 2000) (cons 'l_partkey 5000) (cons 'l_extendedprice 500.0) (cons 'l_discount 0.05))))
(define supplier (list (list (cons 's_suppkey 1000)) (list (cons 's_suppkey 2000))))
(define part (list (list (cons 'p_partkey 5000) (cons 'p_type "ECONOMY ANODIZED STEEL")) (list (cons 'p_partkey 6000) (cons 'p_type "SMALL BRASS"))))
...
(define result (let ((_tmp '()))
                  ...
                  (when (equal? (map-get c 'c_custkey) (map-get o 'o_custkey))
                    (for-each (lambda (n)
                      (when (equal? (map-get n 'n_nationkey) (map-get c 'c_nationkey))
                        (for-each (lambda (r)
                          (when (equal? (map-get r 'r_regionkey) (map-get n 'n_regionkey))
                            (when (and (and (and (equal? (map-get p 'p_type) target_type) (_ge (map-get o 'o_orderdate) start_date)) (_le (map-get o 'o_orderdate) end_date)) (equal? (map-get r 'r_name) "AMERICA"))
  ...
  (let ((_res '()))
    (for-each (lambda (year)
      (set! _res (append _res (list (list (cons 'o_year (map-get year 'key)) (cons 'mkt_share (/ (_sum (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (let ((_t (equal? (map-get (map-get x 'n) 'n_name) target_nation))) (cond ((equal? _t #t) (* (map-get (map-get x 'l) 'l_extendedprice) (- 1 (map-get (map-get x 'l) 'l_discount)))) (else 0))))))
  ) (if (string? year) (string->list year) year))
  _res)) (_sum (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (* (map-get (map-get x 'l) 'l_extendedprice) (- 1 (map-get (map-get x 'l) 'l_discount))))))
  ) (if (string? year) (string->list year) year))
  _res))))))))
    ) (_group_by _tmp (lambda (l) (substring (map-get o 'o_orderdate) 0 4))))
    (set! _res (_sort (map (lambda (x) (cons x (map-get year 'key))) _res)))
    (set! _res (map car _res))
    _res)))
(_json result)
(run-test "Q8 returns correct market share for BRAZIL in 1995" test_Q8_returns_correct_market_share_for_BRAZIL_in_1995)
