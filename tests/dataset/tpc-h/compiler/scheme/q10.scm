; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:12Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q10_returns_customer_revenue_from_returned_items)
  (when (not (equal? result (list (list (cons 'c_custkey 1) (cons 'c_name "Alice") (cons 'revenue (* 1000.0 0.9)) (cons 'c_acctbal 100.0) (cons 'n_name "BRAZIL") (cons 'c_address "123 St") (cons 'c_phone "123-456") (cons 'c_comment "Loyal"))))) (error "expect failed"))
)
...
(define result (let ((_tmp '()))
  ...
  _res))))) _res)))
    ...
    _res)))
(_json result)
(run-test "Q10 returns customer revenue from returned items" test_Q10_returns_customer_revenue_from_returned_items)
