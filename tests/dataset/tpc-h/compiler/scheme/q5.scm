; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:03:58Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q5_returns_revenue_per_nation_in_ASIA_with_local_suppliers)
  (when (not (equal? result (list (list (cons 'n_name "JAPAN") (cons 'revenue 950)) (list (cons 'n_name "INDIA") (cons 'revenue 720))))) (error "expect failed"))
)

(define region (list (list (cons 'r_regionkey 0) (cons 'r_name "ASIA")) (list (cons 'r_regionkey 1) (cons 'r_name "EUROPE"))))
(define nation (list (list (cons 'n_nationkey 10) (cons 'n_regionkey 0) (cons 'n_name "JAPAN")) (list (cons 'n_nationkey 20) (cons 'n_regionkey 0) (cons 'n_name "INDIA")) (list (cons 'n_nationkey 30) (cons 'n_regionkey 1) (cons 'n_name "FRANCE"))))
(define customer (list (list (cons 'c_custkey 1) (cons 'c_nationkey 10)) (list (cons 'c_custkey 2) (cons 'c_nationkey 20))))
(define supplier (list (list (cons 's_suppkey 100) (cons 's_nationkey 10)) (list (cons 's_suppkey 200) (cons 's_nationkey 20))))
(define orders (list (list (cons 'o_orderkey 1000) (cons 'o_custkey 1) (cons 'o_orderdate "1994-03-15")) (list (cons 'o_orderkey 2000) (cons 'o_custkey 2) (cons 'o_orderdate "1994-06-10")) (list (cons 'o_orderkey 3000) (cons 'o_custkey 2) (cons 'o_orderdate "1995-01-01"))))
(define lineitem (list (list (cons 'l_orderkey 1000) (cons 'l_suppkey 100) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.05)) (list (cons 'l_orderkey 2000) (cons 'l_suppkey 200) (cons 'l_extendedprice 800.0) (cons 'l_discount 0.1)) (list (cons 'l_orderkey 3000) (cons 'l_suppkey 200) (cons 'l_extendedprice 900.0) (cons 'l_discount 0.05))))
(define asia_nations (let ((_res '()))
  (for-each (lambda (r)
    (for-each (lambda (n)
      (when (equal? (map-get n 'n_regionkey) (map-get r 'r_regionkey))
        (when (equal? (map-get r 'r_name) "ASIA")
          (set! _res (append _res (list n)))
        )
      )) (if (string? nation) (string->list nation) nation))
  ) (if (string? region) (string->list region) region))
  _res))
(define local_customer_supplier_orders (let ((_res '()))
  (for-each (lambda (c)
    (for-each (lambda (n)
      (when (equal? (map-get c 'c_nationkey) (map-get n 'n_nationkey))
        (for-each (lambda (o)
              ...
              (when (equal? (map-get l 'l_orderkey) (map-get o 'o_orderkey))
                (for-each (lambda (s)
                  (when (equal? (map-get s 's_suppkey) (map-get l 'l_suppkey))
                    (when (and (and (_ge (map-get o 'o_orderdate) "1994-01-01") (_lt (map-get o 'o_orderdate) "1995-01-01")) (equal? (map-get s 's_nationkey) (map-get c 'c_nationkey)))
                      (set! _res (append _res (list (list (cons 'nation (map-get n 'n_name)) (cons 'revenue (* (map-get l 'l_extendedprice) (- 1 (map-get l 'l_discount))))))))
                    )
      ...
      )) (if (string? asia_nations) (string->list asia_nations) asia_nations))
  ) (if (string? customer) (string->list customer) customer))
  _res))
(define result (let ((_tmp '()))
  (for-each (lambda (r)
    (set! _tmp (append _tmp (list r)))
  ) (if (string? local_customer_supplier_orders) (string->list local_customer_supplier_orders) local_customer_supplier_orders))
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 'n_name (map-get g 'key)) (cons 'revenue (_sum (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (map-get x 'revenue))))
  ) (if (string? g) (string->list g) g))
  _res)))))))
    ) (_group_by _tmp (lambda (r) (map-get r 'nation))))
  ...
  _res))))) _res)))
    ...
    _res)))
(_json result)
(run-test "Q5 returns revenue per nation in ASIA with local suppliers" test_Q5_returns_revenue_per_nation_in_ASIA_with_local_suppliers)
