; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:35Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q18_returns_large_volume_customers_with_total_quantity___200)
  (when (not (equal? result (list (list (cons 'c_name "Alice") (cons 'c_custkey 1) (cons 'revenue 1700.0) (cons 'c_acctbal 1000.0) (cons 'n_name "GERMANY") (cons 'c_address "123 Market St") (cons 'c_phone "123-456") (cons 'c_comment "Premium client"))))) (error "expect failed"))
)

(define nation (list (list (cons 'n_nationkey 1) (cons 'n_name "GERMANY"))))
(define customer (list (list (cons 'c_custkey 1) (cons 'c_name "Alice") (cons 'c_acctbal 1000.0) (cons 'c_nationkey 1) (cons 'c_address "123 Market St") (cons 'c_phone "123-456") (cons 'c_comment "Premium client")) (list (cons 'c_custkey 2) (cons 'c_name "Bob") (cons 'c_acctbal 200.0) (cons 'c_nationkey 1) (cons 'c_address "456 Side St") (cons 'c_phone "987-654") (cons 'c_comment "Frequent returns"))))
(define orders (list (list (cons 'o_orderkey 100) (cons 'o_custkey 1)) (list (cons 'o_orderkey 200) (cons 'o_custkey 1)) (list (cons 'o_orderkey 300) (cons 'o_custkey 2))))
(define lineitem (list (list (cons 'l_orderkey 100) (cons 'l_quantity 150) (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.1)) (list (cons 'l_orderkey 200) (cons 'l_quantity 100) (cons 'l_extendedprice 800.0) (cons 'l_discount 0.0)) (list (cons 'l_orderkey 300) (cons 'l_quantity 30) (cons 'l_extendedprice 300.0) (cons 'l_discount 0.05))))
...
(define result (let ((_tmp '()))
  ...
  (let ((_res '()))
    (for-each (lambda (g)
      (when (_gt (_sum (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (map-get (map-get x 'l) 'l_quantity))))
  ) (if (string? g) (string->list g) g))
  _res)) threshold) (set! _res (append _res (list (list (cons 'c_name (map-get (map-get g 'key) 'c_name)) (cons 'c_custkey (map-get (map-get g 'key) 'c_custkey)) (cons 'revenue (_sum (let ((_res '()))
  (for-each (lambda (x)
  ...
  _res))) (cons 'c_acctbal (map-get (map-get g 'key) 'c_acctbal)) (cons 'n_name (map-get (map-get g 'key) 'n_name)) (cons 'c_address (map-get (map-get g 'key) 'c_address)) (cons 'c_phone (map-get (map-get g 'key) 'c_phone)) (cons 'c_comment (map-get (map-get g 'key) 'c_comment)))))))
    ) (_group_by _tmp (lambda (c) (list (cons 'c_name (map-get c 'c_name)) (cons 'c_custkey (map-get c 'c_custkey)) (cons 'c_acctbal (map-get c 'c_acctbal)) (cons 'c_address (map-get c 'c_address)) (cons 'c_phone (map-get c 'c_phone)) (cons 'c_comment (map-get c 'c_comment)) (cons 'n_name (map-get n 'n_name))))))
  ...
  _res))))) _res)))
    ...
    _res)))
(_json result)
(run-test "Q18 returns large-volume customers with total quantity > 200" test_Q18_returns_large_volume_customers_with_total_quantity___200)
