; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:46Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q22_returns_wealthy_inactive_customers_by_phone_prefix)
  (when (not (equal? result (list (list (cons 'cntrycode "13") (cons 'numcust 1) (cons 'totacctbal 600.0)) (list (cons 'cntrycode "30") (cons 'numcust 1) (cons 'totacctbal 700.0))))) (error "expect failed"))
)

(define customer (list (list (cons 'c_custkey 1) (cons 'c_phone "13-123-4567") (cons 'c_acctbal 600.0)) (list (cons 'c_custkey 2) (cons 'c_phone "31-456-7890") (cons 'c_acctbal 100.0)) (list (cons 'c_custkey 3) (cons 'c_phone "30-000-0000") (cons 'c_acctbal 700.0))))
(define orders (list (list (cons 'o_orderkey 10) (cons 'o_custkey 2))))
(define valid_codes (list "13" "31" "23" "29" "30" "18" "17"))
(define avg_balance (_avg (let ((_res '()))
  (for-each (lambda (c)
    (when (and (_gt (map-get c 'c_acctbal) 0.0) (if (member (substring (map-get c 'c_phone) 0 2) valid_codes) #t #f))
      (set! _res (append _res (list (map-get c 'c_acctbal))))
    )
  ) (if (string? customer) (string->list customer) customer))
  _res)))
(define eligible_customers (let ((_res '()))
  (for-each (lambda (c)
    (when (and (and (if (member (substring (map-get c 'c_phone) 0 2) valid_codes) #t #f) (_gt (map-get c 'c_acctbal) avg_balance)) (not (_exists (let ((_res '()))
  (for-each (lambda (o)
  ...
  _res))))
      (set! _res (append _res (list (list (cons 'cntrycode (substring (map-get c 'c_phone) 0 2)) (cons 'c_acctbal (map-get c 'c_acctbal))))))
    )
  ) (if (string? customer) (string->list customer) customer))
  _res))
(define groups (let ((_tmp '()))
  ...
  ) (if (string? eligible_customers) (string->list eligible_customers) eligible_customers))
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list g)))
    ) (_group_by _tmp (lambda (c) (map-get c 'cntrycode))))
    _res)))
(define tmp (list ))
(let loop ((g_idx 0))
  ...
  (for-each (lambda (x)
    (set! _res (append _res (list (map-get x 'c_acctbal))))
  ) (if (string? (map-get g 'items)) (string->list (map-get g 'items)) (map-get g 'items)))
  _res)))
        (define row (list (cons 'cntrycode (map-get g 'key)) (cons 'numcust (_count g)) (cons 'totacctbal total)))
...
(define result (let ((_res '()) (_tmp '()))
  (for-each (lambda (r)
    (set! _tmp (append _tmp (list (cons r (map-get r 'cntrycode)))))
  ) (if (string? tmp) (string->list tmp) tmp))
  (set! _res (_sort _tmp))
  (set! _res (map car _res))
  _res))
(_json result)
(run-test "Q22 returns wealthy inactive customers by phone prefix" test_Q22_returns_wealthy_inactive_customers_by_phone_prefix)
