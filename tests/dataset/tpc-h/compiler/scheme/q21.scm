; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:43Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q21_returns_Saudi_suppliers_who_caused_unique_delivery_delays)
  (when (not (equal? result (list (list (cons 's_name "Desert Trade") (cons 'numwait 1))))) (error "expect failed"))
)

(define nation (list (list (cons 'n_nationkey 1) (cons 'n_name "SAUDI ARABIA")) (list (cons 'n_nationkey 2) (cons 'n_name "FRANCE"))))
(define supplier (list (list (cons 's_suppkey 100) (cons 's_name "Desert Trade") (cons 's_nationkey 1)) (list (cons 's_suppkey 200) (cons 's_name "Euro Goods") (cons 's_nationkey 2))))
(define orders (list (list (cons 'o_orderkey 500) (cons 'o_orderstatus "F")) (list (cons 'o_orderkey 600) (cons 'o_orderstatus "O"))))
(define lineitem (list (list (cons 'l_orderkey 500) (cons 'l_suppkey 100) (cons 'l_receiptdate "1995-04-15") (cons 'l_commitdate "1995-04-10")) (list (cons 'l_orderkey 500) (cons 'l_suppkey 200) (cons 'l_receiptdate "1995-04-12") (cons 'l_commitdate "1995-04-12")) (list (cons 'l_orderkey 600) (cons 'l_suppkey 100) (cons 'l_receiptdate "1995-05-01") (cons 'l_commitdate "1995-04-25"))))
(define result (let ((_tmp '()))
  (for-each (lambda (s)
    (for-each (lambda (l1)
      (when (equal? (map-get s 's_suppkey) (map-get l1 'l_suppkey))
        (for-each (lambda (o)
          (when (equal? (map-get o 'o_orderkey) (map-get l1 'l_orderkey))
            (for-each (lambda (n)
              (when (equal? (map-get n 'n_nationkey) (map-get s 's_nationkey))
                (when (and (and (and (equal? (map-get o 'o_orderstatus) "F") (_gt (map-get l1 'l_receiptdate) (map-get l1 'l_commitdate))) (equal? (map-get n 'n_name) "SAUDI ARABIA")) (not (_exists (let ((_res '()))
  (for-each (lambda (x)
    (when (and (and (equal? (map-get x 'l_orderkey) (map-get l1 'l_orderkey)) (not (equal? (map-get x 'l_suppkey) (map-get l1 'l_suppkey)))) (_gt (map-get x 'l_receiptdate) (map-get x 'l_commitdate)))
  ...
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 's_name (map-get g 'key)) (cons 'numwait (_count g))))))
    ) (_group_by _tmp (lambda (s) (map-get s 's_name))))
    (set! _res (_sort (map (lambda (x) (cons x (list (- (_count g)) (map-get g 'key)))) _res)))
    (set! _res (map car _res))
    _res)))
(_json result)
(run-test "Q21 returns Saudi suppliers who caused unique delivery delays" test_Q21_returns_Saudi_suppliers_who_caused_unique_delivery_delays)
