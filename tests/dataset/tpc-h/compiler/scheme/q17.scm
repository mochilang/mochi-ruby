; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:34Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q17_returns_average_yearly_revenue_for_small_quantity_orders)
  (define expected (/ 100.0 7.0))
  (when (not (equal? result expected)) (error "expect failed"))
)

(define part (list (list (cons 'p_partkey 1) (cons 'p_brand "Brand#23") (cons 'p_container "MED BOX")) (list (cons 'p_partkey 2) (cons 'p_brand "Brand#77") (cons 'p_container "LG JAR"))))
(define lineitem (list (list (cons 'l_partkey 1) (cons 'l_quantity 1) (cons 'l_extendedprice 100.0)) (list (cons 'l_partkey 1) (cons 'l_quantity 10) (cons 'l_extendedprice 1000.0)) (list (cons 'l_partkey 1) (cons 'l_quantity 20) (cons 'l_extendedprice 2000.0)) (list (cons 'l_partkey 2) (cons 'l_quantity 5) (cons 'l_extendedprice 500.0))))
  ...
  (for-each (lambda (l)
    (for-each (lambda (p)
      (when (equal? (map-get p 'p_partkey) (map-get l 'l_partkey))
        (when (and (and (equal? (map-get p 'p_brand) brand) (equal? (map-get p 'p_container) container)) (_lt (map-get l 'l_quantity) (* 0.2 (_avg (let ((_res '()))
  (for-each (lambda (x)
    (when (equal? (map-get x 'l_partkey) (map-get p 'p_partkey))
      (set! _res (append _res (list (map-get x 'l_quantity))))
    )
  ) (if (string? lineitem) (string->list lineitem) lineitem))
  _res)))))
          (set! _res (append _res (list (map-get l 'l_extendedprice))))
        )
      )) (if (string? part) (string->list part) part))
  ) (if (string? lineitem) (string->list lineitem) lineitem))
  _res))
(define result (/ (_sum filtered) 7.0))
(_json result)
(run-test "Q17 returns average yearly revenue for small-quantity orders" test_Q17_returns_average_yearly_revenue_for_small_quantity_orders)
