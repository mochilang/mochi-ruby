; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:04:23Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q13_groups_customers_by_non_special_order_count)
  (when (not (equal? grouped (list (list (cons 'c_count 2) (cons 'custdist 1)) (list (cons 'c_count 0) (cons 'custdist 2))))) (error "expect failed"))
)

(define customer (list (list (cons 'c_custkey 1)) (list (cons 'c_custkey 2)) (list (cons 'c_custkey 3))))
(define orders (list (list (cons 'o_orderkey 100) (cons 'o_custkey 1) (cons 'o_comment "fast delivery")) (list (cons 'o_orderkey 101) (cons 'o_custkey 1) (cons 'o_comment "no comment")) (list (cons 'o_orderkey 102) (cons 'o_custkey 2) (cons 'o_comment "special requests only"))))
(define per_customer (let ((_res '()))
  (for-each (lambda (c)
    (set! _res (append _res (list (list (cons 'c_count (_count (let ((_res '()))
  (for-each (lambda (o)
    (when (and (and (equal? (map-get o 'o_custkey) (map-get c 'c_custkey)) (not (if (member "special" (map-get o 'o_comment)) #t #f))) (not (if (member "requests" (map-get o 'o_comment)) #t #f)))
...
(define grouped (let ((_tmp '()))
  ...
  ) (if (string? per_customer) (string->list per_customer) per_customer))
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 'c_count (map-get g 'key)) (cons 'custdist (_count g))))))
    ) (_group_by _tmp (lambda (x) (map-get x 'c_count))))
    (set! _res (_sort (map (lambda (x) (cons x (- (map-get g 'key)))) _res)))
    (set! _res (map car _res))
    _res)))
(_json grouped)
(run-test "Q13 groups customers by non-special order count" test_Q13_groups_customers_by_non_special_order_count)
