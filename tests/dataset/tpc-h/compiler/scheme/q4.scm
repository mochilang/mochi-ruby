; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:03:54Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q4_returns_count_of_orders_with_late_lineitems_in_range)
  (when (not (equal? result (list (list (cons 'o_orderpriority "1-URGENT") (cons 'order_count 1)) (list (cons 'o_orderpriority "2-HIGH") (cons 'order_count 1))))) (error "expect failed"))
)

(define orders (list (list (cons 'o_orderkey 1) (cons 'o_orderdate "1993-07-01") (cons 'o_orderpriority "1-URGENT")) (list (cons 'o_orderkey 2) (cons 'o_orderdate "1993-07-15") (cons 'o_orderpriority "2-HIGH")) (list (cons 'o_orderkey 3) (cons 'o_orderdate "1993-08-01") (cons 'o_orderpriority "3-NORMAL"))))
(define lineitem (list (list (cons 'l_orderkey 1) (cons 'l_commitdate "1993-07-10") (cons 'l_receiptdate "1993-07-12")) (list (cons 'l_orderkey 1) (cons 'l_commitdate "1993-07-12") (cons 'l_receiptdate "1993-07-10")) (list (cons 'l_orderkey 2) (cons 'l_commitdate "1993-07-20") (cons 'l_receiptdate "1993-07-25")) (list (cons 'l_orderkey 3) (cons 'l_commitdate "1993-08-02") (cons 'l_receiptdate "1993-08-01")) (list (cons 'l_orderkey 3) (cons 'l_commitdate "1993-08-05") (cons 'l_receiptdate "1993-08-10"))))
(define start_date "1993-07-01")
(define end_date "1993-08-01")
(define date_filtered_orders (let ((_res '()))
  (for-each (lambda (o)
    (when (and (_ge (map-get o 'o_orderdate) start_date) (_lt (map-get o 'o_orderdate) end_date))
      (set! _res (append _res (list o)))
    )
  ) (if (string? orders) (string->list orders) orders))
  _res))
(define late_orders (let ((_res '()))
  (for-each (lambda (o)
    (when (_exists (let ((_res '()))
  (for-each (lambda (l)
    (when (and (equal? (map-get l 'l_orderkey) (map-get o 'o_orderkey)) (_lt (map-get l 'l_commitdate) (map-get l 'l_receiptdate)))
  ...
  ) (if (string? date_filtered_orders) (string->list date_filtered_orders) date_filtered_orders))
  _res))
(define result (let ((_tmp '()))
  (for-each (lambda (o)
    (set! _tmp (append _tmp (list o)))
  ) (if (string? late_orders) (string->list late_orders) late_orders))
  (let ((_res '()))
    (for-each (lambda (g)
      (set! _res (append _res (list (list (cons 'o_orderpriority (map-get g 'key)) (cons 'order_count (_count g))))))
    ) (_group_by _tmp (lambda (o) (map-get o 'o_orderpriority))))
    (set! _res (_sort (map (lambda (x) (cons x (map-get g 'key))) _res)))
    (set! _res (map car _res))
    _res)))
(_json result)
(run-test "Q4 returns count of orders with late lineitems in range" test_Q4_returns_count_of_orders_with_late_lineitems_in_range)
