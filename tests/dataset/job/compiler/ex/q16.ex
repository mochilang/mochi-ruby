  ...
  def main do
    ...
    rows =
      for an <- aka_name,
          ...
          mc <- movie_companies,
          cn <- company_name,
          n.id == an.person_id && ci.person_id == n.id && t.id == ci.movie_id &&
            mk.movie_id == t.id && k.id == mk.keyword_id && mc.movie_id == t.id &&
            cn.id == mc.company_id &&
            (cn.country_code == "[us]" && k.keyword == "character-name-in-title" &&
               t.episode_nr >= 50 && t.episode_nr < 100),
          do: %{pseudonym: an.name, series: t.title}

    # result :: list(map())
    result = [
      %{
        cool_actor_pseudonym: _min(for r <- rows, do: r.pseudonym),
        series_named_after_char: _min(for r <- rows, do: r.series)
      }
    ]
