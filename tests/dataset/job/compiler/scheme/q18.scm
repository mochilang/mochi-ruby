; Generated by Mochi compiler v0.10.25 on 2025-07-13T13:08:37Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q18_finds_minimal_budget__votes_and_title_for_Tim_productions)
  (when (not (equal? result (list (cons 'movie_budget 90) (cons 'movie_votes 400) (cons 'movie_title "Alpha")))) (error "expect failed"))
)

(define info_type (list (list (cons 'id 1) (cons 'info "budget")) (list (cons 'id 2) (cons 'info "votes")) (list (cons 'id 3) (cons 'info "rating"))))
(define name (list (list (cons 'id 1) (cons 'name "Big Tim") (cons 'gender "m")) (list (cons 'id 2) (cons 'name "Slim Tim") (cons 'gender "m")) (list (cons 'id 3) (cons 'name "Alice") (cons 'gender "f"))))
(define title (list (list (cons 'id 10) (cons 'title "Alpha")) (list (cons 'id 20) (cons 'title "Beta")) (list (cons 'id 30) (cons 'title "Gamma"))))
(define cast_info (list (list (cons 'movie_id 10) (cons 'person_id 1) (cons 'note "(producer)")) (list (cons 'movie_id 20) (cons 'person_id 2) (cons 'note "(executive producer)")) (list (cons 'movie_id 30) (cons 'person_id 3) (cons 'note "(producer)"))))
(define movie_info (list (list (cons 'movie_id 10) (cons 'info_type_id 1) (cons 'info 90)) (list (cons 'movie_id 20) (cons 'info_type_id 1) (cons 'info 120)) (list (cons 'movie_id 30) (cons 'info_type_id 1) (cons 'info 110))))
(define movie_info_idx (list (list (cons 'movie_id 10) (cons 'info_type_id 2) (cons 'info 500)) (list (cons 'movie_id 20) (cons 'info_type_id 2) (cons 'info 400)) (list (cons 'movie_id 30) (cons 'info_type_id 2) (cons 'info 800))))
(define rows (let ((_res '()))
                      ...
                      (when (equal? (map-get it1 'id) (map-get mi 'info_type_id))
                        (for-each (lambda (it2)
                          (when (equal? (map-get it2 'id) (map-get mi_idx 'info_type_id))
                            (when (and (and (and (and (and (and (and (and (if (member (map-get ci 'note) (list "(producer)" "(executive producer)")) #t #f) (equal? (map-get it1 'info) "budget")) (equal? (map-get it2 'info) "votes")) (equal? (map-get n 'gender) "m")) (if (string-contains (map-get n 'name) "Tim") #t #f)) (equal? (map-get t 'id) (map-get ci 'movie_id))) (equal? (map-get ci 'movie_id) (map-get mi 'movie_id))) (equal? (map-get ci 'movie_id) (map-get mi_idx 'movie_id))) (equal? (map-get mi 'movie_id) (map-get mi_idx 'movie_id)))
                              (set! _res (append _res (list (list (cons 'budget (map-get mi 'info)) (cons 'votes (map-get mi_idx 'info)) (cons 'title (map-get t 'title))))))
                            )
  ...
  _res))
(define result (list (cons 'movie_budget (_min (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (map-get r 'budget))))
  ) (if (string? rows) (string->list rows) rows))
  _res))) (cons 'movie_votes (_min (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (map-get r 'votes))))
  ) (if (string? rows) (string->list rows) rows))
  ...
  _res)))))
(_json result)
(run-test "Q18 finds minimal budget, votes and title for Tim productions" test_Q18_finds_minimal_budget__votes_and_title_for_Tim_productions)
