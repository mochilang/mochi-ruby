; Generated by Mochi compiler v0.10.25 on 2025-07-13T13:08:33Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q16_finds_series_named_after_a_character_between_episodes_50_and_99)
  (when (not (equal? result (list (list (cons 'cool_actor_pseudonym "Alpha") (cons 'series_named_after_char "Hero Bob"))))) (error "expect failed"))
)

(define aka_name (list (list (cons 'person_id 1) (cons 'name "Alpha")) (list (cons 'person_id 2) (cons 'name "Beta"))))
(define cast_info (list (list (cons 'person_id 1) (cons 'movie_id 101)) (list (cons 'person_id 2) (cons 'movie_id 102))))
(define company_name (list (list (cons 'id 1) (cons 'country_code "[us]")) (list (cons 'id 2) (cons 'country_code "[de]"))))
(define keyword (list (list (cons 'id 1) (cons 'keyword "character-name-in-title")) (list (cons 'id 2) (cons 'keyword "other"))))
(define movie_companies (list (list (cons 'movie_id 101) (cons 'company_id 1)) (list (cons 'movie_id 102) (cons 'company_id 2))))
(define movie_keyword (list (list (cons 'movie_id 101) (cons 'keyword_id 1)) (list (cons 'movie_id 102) (cons 'keyword_id 2))))
(define name (list (list (cons 'id 1)) (list (cons 'id 2))))
(define title (list (list (cons 'id 101) (cons 'title "Hero Bob") (cons 'episode_nr 60)) (list (cons 'id 102) (cons 'title "Other Show") (cons 'episode_nr 40))))
(define rows (let ((_res '()))
                          ...
                          (when (equal? (map-get mc 'movie_id) (map-get t 'id))
                            (for-each (lambda (cn)
                              (when (equal? (map-get cn 'id) (map-get mc 'company_id))
                                (when (and (and (and (equal? (map-get cn 'country_code) "[us]") (equal? (map-get k 'keyword) "character-name-in-title")) (_ge (map-get t 'episode_nr) 50)) (_lt (map-get t 'episode_nr) 100))
                                  (set! _res (append _res (list (list (cons 'pseudonym (map-get an 'name)) (cons 'series (map-get t 'title))))))
                                )
  ...
  _res))
(define result (list (list (cons 'cool_actor_pseudonym (_min (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (map-get r 'pseudonym))))
  ) (if (string? rows) (string->list rows) rows))
  _res))) (cons 'series_named_after_char (_min (let ((_res '()))
  (for-each (lambda (r)
    (set! _res (append _res (list (map-get r 'series))))
  ) (if (string? rows) (string->list rows) rows))
  _res))))))
(_json result)
(run-test "Q16 finds series named after a character between episodes 50 and 99" test_Q16_finds_series_named_after_a_character_between_episodes_50_and_99)
