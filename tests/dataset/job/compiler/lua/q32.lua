...
function test_Q32_finds_movie_link_for_10_000_mile_club()
    if not (__eq(result, {["link_type"]="sequel", ["first_movie"]="Movie A", ["second_movie"]="Movie C"})) then error('expect failed') end
end

keyword = {{["id"]=1, ["keyword"]="10,000-mile-club"}, {["id"]=2, ["keyword"]="character-name-in-title"}}
link_type = {{["id"]=1, ["link"]="sequel"}, {["id"]=2, ["link"]="remake"}}
movie_keyword = {{["movie_id"]=100, ["keyword_id"]=1}, {["movie_id"]=200, ["keyword_id"]=2}}
movie_link = {{["movie_id"]=100, ["linked_movie_id"]=300, ["link_type_id"]=1}, {["movie_id"]=200, ["linked_movie_id"]=400, ["link_type_id"]=2}}
title = {{["id"]=100, ["title"]="Movie A"}, {["id"]=200, ["title"]="Movie B"}, {["id"]=300, ["title"]="Movie C"}, {["id"]=400, ["title"]="Movie D"}}
joined = (function()
    local _src = keyword
    return __query(_src, {
        { items = movie_keyword, on = function(k, mk) return __eq(mk.keyword_id, k.id) end },
        { items = title, on = function(k, mk, t1) return __eq(t1.id, mk.movie_id) end },
        { items = movie_link, on = function(k, mk, t1, ml) return __eq(ml.movie_id, t1.id) end },
        { items = title, on = function(k, mk, t1, ml, t2) return __eq(t2.id, ml.linked_movie_id) end },
        { items = link_type, on = function(k, mk, t1, ml, t2, lt) return __eq(lt.id, ml.link_type_id) end }
    }, { selectFn = function(k, mk, t1, ml, t2, lt) return {["link_type"]=lt.link, ["first_movie"]=t1.title, ["second_movie"]=t2.title} end, where = function(k, mk, t1, ml, t2, lt) return (__eq(k.keyword, "10,000-mile-club")) end })
end)()
result = {["link_type"]=__min((function()
    local _res = {}
...
__json({result})
local __tests = {
    {name="Q32 finds movie link for 10,000-mile-club", fn=test_Q32_finds_movie_link_for_10_000_mile_club},
}
