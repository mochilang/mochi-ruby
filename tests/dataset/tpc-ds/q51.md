...
This query compares cumulative web and store sales for each item over a one year window.
...
## Expected Output
The simplified example yields two rows where web sales exceed store sales.
```json
[
  {"item_sk": 1, "d_date": 1},
  {"item_sk": 1, "d_date": 2}
