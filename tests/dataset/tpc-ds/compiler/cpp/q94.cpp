...
std::vector<WebSale> web_sales = {WebSale{1, 1, 1, 1, 1, 5, 2},
                                  WebSale{1, 1, 2, 1, 1, 0, 0},
                                  WebSale{2, 1, 3, 1, 1, 3, 1}};
std::vector<std::string> web_returns =
    std::vector<decltype(std::unordered_map<std::string, decltype(2)>{
        {std::string("wr_order_number"), 2}})>{
        ...
        for (auto ca : customer_address) {
          ...
          for (auto w : web_site) {
            ...
            if (!(((((ca.ca_state == std::string("CA")) &&
                    ...
                    std::any_of(
                        web_sales.begin(), web_sales.end(),
                        [&](auto ws2) {
                          return ((ws.ws_order_number == ws2.ws_order_number) &&
                                  (ws.ws_warehouse_sk != ws2.ws_warehouse_sk));
                        })) &&
                   (std::any_of(
                        web_returns.begin(), web_returns.end(), [&](auto wr) {
                          return (wr.wr_order_number == ws.ws_order_number);
                        }) == false))))
              continue;
