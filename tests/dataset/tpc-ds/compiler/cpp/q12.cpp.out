...
struct DateDim {
  ...
  decltype(std::string("2001-01-20")) d_date;
};
struct Filtered {
...
int main() {
  std::vector<WebSale> web_sales = {WebSale{1, 1, 100}, WebSale{1, 2, 100},
                                    WebSale{2, 2, 200}, WebSale{3, 3, 50}};
  std::vector<Item> item = {
      Item{1, std::string("ITEM1"), std::string("Item One"), std::string("A"),
           std::string("C1"), 10},
      Item{2, std::string("ITEM2"), std::string("Item Two"), std::string("A"),
           std::string("C1"), 20},
      Item{3, std::string("ITEM3"), std::string("Item Three"), std::string("B"),
           std::string("C2"), 30}};
  std::vector<DateDim> date_dim = {DateDim{1, std::string("2001-01-20")},
                                   DateDim{2, std::string("2001-02-05")},
                                   DateDim{3, std::string("2001-03-05")}};
        ...
        for (auto d : date_dim) {
          ...
          if (!((((std::find(
                   ...
                   std::vector<std::string>{std::string("A"), std::string("B"),
                                            std::string("C")}
                       .end()) &&
                  (d.d_date >= std::string("2001-01-15"))) &&
                 (d.d_date <= std::string("2001-02-14")))))
            continue;
          ...
          for (auto &__g : __groups) {
            if (__g.key == __key) {
              __g.items.push_back(__struct5{ws, i, d});
              __found = true;
              break;
            }
          }
          if (!__found) {
            __groups.push_back(
                __struct6{__key, std::vector<__struct5>{__struct5{ws, i, d}}});
          }
        }
      }
    }
    ...
    for (auto &g : __groups) {
      __items.push_back(__struct7{
          ...
          ([&](auto v) {
            ...
            std::vector<decltype(std::declval<__struct5>().ws_ext_sales_price)>
                __items;
            for (auto x : g.items) {
              __items.push_back(x.ws_ext_sales_price);
            }
            return __items;
          })())});
    }
