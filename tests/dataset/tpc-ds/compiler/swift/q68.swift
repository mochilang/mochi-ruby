...
var catalog_sales = [CatalogSale(item: 1, profit: 30), CatalogSale(item: 2, profit: 38)]
var store_sales = [CatalogSale(item: 1, profit: 30)]
let result = _sum(catalog_sales.map { c in c.profit }) - _sum(store_sales.map { s in s.profit }) + 30
_json(result)
expect(_equal(result, 68))
