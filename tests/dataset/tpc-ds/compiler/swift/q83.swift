...
struct SrItem: Equatable {
    var qty: Int
}

var sr_items = [SrItem(qty: 10), SrItem(qty: 5)]
var cr_items = [SrItem(qty: 25), SrItem(qty: 20)]
var wr_items = [SrItem(qty: 10), SrItem(qty: 13)]
let result = _sum(sr_items.map { x in x.qty }) + _sum(cr_items.map { x in x.qty }) + _sum(wr_items.map { x in x.qty })
_json(result)
expect(_equal(result, 83))
