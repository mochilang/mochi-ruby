...
var web_sales = [WebSale(net: 30)]
var catalog_sales = [WebSale(net: 36)]
let result = _sum(web_sales.map { w in w.net }) + _sum(catalog_sales.map { c in c.net })
_json(result)
expect(_equal(result, 66))
