...
var store_sales = [StoreSale(item: 1, price: 10), StoreSale(item: 1, price: 20)]
var catalog_sales = [StoreSale(item: 1, price: 15)]
var web_sales = [StoreSale(item: 1, price: 15)]
let all_sales = ((store_sales + catalog_sales) + web_sales)
let result = _sum(all_sales.map { s in s.price })
_json(result)
expect(_equal(result, 60))
