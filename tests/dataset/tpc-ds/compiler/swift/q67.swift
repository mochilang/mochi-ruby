...
var store_sales = [StoreSale(price: 40, reason: 1), StoreSale(price: 27, reason: 2)]
var reason = [Reason(id: 1, name: "PROMO"), Reason(id: 2, name: "RETURN")]
let result = _sum(({
	var _res: [Int] = []
	for ss in store_sales {
		for r in reason {
			if !(ss.reason == r.id) { continue }
			if !(["PROMO", "RETURN"].contains(r.name) != nil) { continue }
			_res.append(ss.price)
		}
	}
	return _res
}()))
_json(result)
expect(_equal(result, 67))
