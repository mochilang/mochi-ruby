...
(struct WebSale (ws_item_sk ws_sold_date_sk ws_ext_sales_price) #:transparent #:mutable)
...
(define web_sales (list (hash 'ws_item_sk 1 'ws_sold_date_sk 1 'ws_ext_sales_price 100) (hash 'ws_item_sk 1 'ws_sold_date_sk 2 'ws_ext_sales_price 100) (hash 'ws_item_sk 2 'ws_sold_date_sk 2 'ws_ext_sales_price 200) (hash 'ws_item_sk 3 'ws_sold_date_sk 3 'ws_ext_sales_price 50)))
(define item (list (hash 'i_item_sk 1 'i_item_id "ITEM1" 'i_item_desc "Item One" 'i_category "A" 'i_class "C1" 'i_current_price 10) (hash 'i_item_sk 2 'i_item_id "ITEM2" 'i_item_desc "Item Two" 'i_category "A" 'i_class "C1" 'i_current_price 20) (hash 'i_item_sk 3 'i_item_id "ITEM3" 'i_item_desc "Item Three" 'i_category "B" 'i_class "C2" 'i_current_price 30)))
(define date_dim (list (hash 'd_date_sk 1 'd_date "2001-01-20") (hash 'd_date_sk 2 'd_date "2001-02-05") (hash 'd_date_sk 3 'd_date "2001-03-05")))
(define filtered (let ([groups (make-hash)])
  (for* ([ws web_sales] [i item] [d date_dim] #:when (and (equal? (hash-ref ws 'ws_item_sk) (hash-ref i 'i_item_sk)) (equal? (hash-ref ws 'ws_sold_date_sk) (hash-ref d 'd_date_sk)) (and (and (cond [(string? '("A" "B" "C")) (regexp-match? (regexp (hash-ref i 'i_category)) '("A" "B" "C"))] [(hash? '("A" "B" "C")) (hash-has-key? '("A" "B" "C") (hash-ref i 'i_category))] [else (member (hash-ref i 'i_category) '("A" "B" "C"))]) (string>=? (hash-ref d 'd_date) "2001-01-15")) (string<=? (hash-ref d 'd_date) "2001-02-14")))) (let* ([key (hash 'id (hash-ref i 'i_item_id) 'desc (hash-ref i 'i_item_desc) 'cat (hash-ref i 'i_category) 'class (hash-ref i 'i_class) 'price (hash-ref i 'i_current_price))] [bucket (hash-ref groups key '())]) (hash-set! groups key (cons (hash 'ws ws 'i i 'd d) bucket))))
  (define _groups (for/list ([k (hash-keys groups)]) (hash 'key k 'items (hash-ref groups k))))
  (for/list ([g _groups]) (hash 'i_item_id (hash-ref (hash-ref g 'key) 'id) 'i_item_desc (hash-ref (hash-ref g 'key) 'desc) 'i_category (hash-ref (hash-ref g 'key) 'cat) 'i_class (hash-ref (hash-ref g 'key) 'class) 'i_current_price (hash-ref (hash-ref g 'key) 'price) 'itemrevenue (apply + (for*/list ([v (for*/list ([x (hash-ref g 'items)]) (hash-ref x 'ws_ext_sales_price))]) v))))))
...
(when (equal? result (list (hash 'i_item_id "ITEM1" 'i_item_desc "Item One" 'i_category "A" 'i_class "C1" 'i_current_price 10 'itemrevenue 200 'revenueratio 50) (hash 'i_item_id "ITEM2" 'i_item_desc "Item Two" 'i_category "A" 'i_class "C1" 'i_current_price 20 'itemrevenue 200 'revenueratio 50))) (displayln "ok"))
