# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:36:16Z
defmodule Main do
  def main do
           ...
           _query(
             ...
             [
               ...
               %{
                 ...
                 on: fn ss, d, s, hd -> hd.hd_demo_sk == ss.ss_hdemo_sk end
               }
             ],
             %{
               select: fn ss, d, s, hd ->
                 Map.merge(Map.merge(Map.merge(Map.merge(ss, d), s), hd), %{
                   ss: ss,
                   d: d,
                   s: s,
                   hd: hd
               ...
               where: fn [ss, d, s, hd] ->
                 d.d_dom >= 1 && d.d_dom <= 2 &&
                   ...
                   (d.d_year == 1998 || d.d_year == 1999 || d.d_year == 2000) && s.s_county == "A"
               end
             }
           )

         groups =
           _group_by(rows, fn %{ss: ss, d: d, s: s, hd: hd} ->
             %{ticket: ss.ss_ticket_number, cust: ss.ss_customer_sk}
           end)

         items = groups
         Enum.map(items, fn g -> %{key: g.key, cnt: _count(g)} end)
       end).()
      ...
      Enum.map(
        Enum.sort_by(
          for(
            g <- groups,
            c <- customer,
            c.c_customer_sk == g.key.cust && (g.cnt >= 1 && g.cnt <= 5),
            do:
              {%{
