...
defmodule Main do
  def main do
    ...
    store_sales = [
      ...
      %{item: 2, sold_date: 1, price: 20},
      %{item: 3, sold_date: 1, price: 25}
    ]
    ...
    grouped =
      (fn ->
         ...
         rows =
           _query(
             ...
             [
               ...
               %{items: date_dim, on: fn ss, i, d -> ss.sold_date == d.d_date_sk end}
             ],
             %{
               select: fn ss, i, d ->
         ...
         groups = _group_by(rows, fn %{ss: ss, i: i, d: d} -> %{brand_id: i.i_brand_id} end)
         items = groups

         Enum.map(items, fn g ->
           %{brand_id: g.key.brand_id, ext_price: _sum(for x <- g.items, do: x.price)}
         end)
       end).()

    # result :: list(map())
    result = for g <- Enum.sort_by(grouped, fn g -> [-g.ext_price, g.brand_id] end), do: g
    _json(result)
  end
