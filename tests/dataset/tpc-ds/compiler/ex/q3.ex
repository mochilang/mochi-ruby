# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:33:45Z
defmodule Main do
  def main do
    # date_dim :: list(map())
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: store_sales, on: fn dt, ss -> dt.d_date_sk == ss.ss_sold_date_sk end},
               %{items: item, on: fn dt, ss, i -> ss.ss_item_sk == i.i_item_sk end}
             ],
             %{
               select: fn dt, ss, i ->
                 Map.merge(Map.merge(Map.merge(dt, ss), i), %{dt: dt, ss: ss, i: i})
               end,
               where: fn [dt, ss, i] -> i.i_manufact_id == 100 && dt.d_moy == 12 end
             }
           )

         groups =
           _group_by(rows, fn %{dt: dt, ss: ss, i: i} ->
         ...
         items =
           Enum.sort_by(items, fn g ->
             [g.key.d_year, -_sum(for x <- g.items, do: x.ss_ext_sales_price), g.key.brand_id]
           end)
         ...
         Enum.map(items, fn g ->
           %{
             ...
             brand: g.key.brand,
             sum_agg: _sum(for x <- g.items, do: x.ss_ext_sales_price)
           }
         end)
       end).()
