# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:33:58Z
defmodule Main do
  def main do
           ...
           _query(
             ...
             [
               %{
                 ...
                 on: fn ss, cd -> ss.ss_cdemo_sk == cd.cd_demo_sk end
               },
               %{items: date_dim, on: fn ss, cd, d -> ss.ss_sold_date_sk == d.d_date_sk end},
               %{items: item, on: fn ss, cd, d, i -> ss.ss_item_sk == i.i_item_sk end},
               %{items: promotion, on: fn ss, cd, d, i, p -> ss.ss_promo_sk == p.p_promo_sk end}
             ],
             %{
               select: fn ss, cd, d, i, p ->
                 Map.merge(Map.merge(Map.merge(Map.merge(Map.merge(ss, cd), d), i), p), %{
                   ss: ss,
                   cd: cd,
                   d: d,
                   i: i,
               ...
               where: fn [ss, cd, d, i, p] ->
                 cd.cd_gender == "M" && cd.cd_marital_status == "S" &&
                   cd.cd_education_status == "College" &&
                   (p.p_channel_email == "N" || p.p_channel_event == "N") && d.d_year == 1998
               end
             }
           )

         groups =
           _group_by(rows, fn %{ss: ss, cd: cd, d: d, i: i, p: p} -> %{i_item_id: i.i_item_id} end)

         items = groups
         items = Enum.sort_by(items, fn g -> g.key.i_item_id end)

         Enum.map(items, fn g ->
           %{
             i_item_id: g.key.i_item_id,
             agg1: _avg(for x <- g.items, do: x.ss.ss_quantity),
             agg2: _avg(for x <- g.items, do: x.ss.ss_list_price),
             agg3: _avg(for x <- g.items, do: x.ss.ss_coupon_amt),
             agg4: _avg(for x <- g.items, do: x.ss.ss_sales_price)
           }
         end)
       end).()
