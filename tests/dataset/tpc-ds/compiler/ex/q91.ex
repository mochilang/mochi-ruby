    ...
    result =
      _first(
        (fn ->
             ...
             _query(
               ...
               [
                 %{
                   ...
                   on: fn cc, cr -> cc.cc_call_center_sk == cr.cr_call_center_sk end
                 },
                 %{
                   items: date_dim,
                   on: fn cc, cr, d -> cr.cr_returned_date_sk == d.d_date_sk end
                 },
                 %{
                   items: customer,
                   on: fn cc, cr, d, c -> cr.cr_returning_customer_sk == c.c_customer_sk end
                 },
                 %{
                   items: customer_demographics,
                   on: fn cc, cr, d, c, cd -> c.c_current_cdemo_sk == cd.cd_demo_sk end
                 },
                 %{
                   items: household_demographics,
                   on: fn cc, cr, d, c, cd, hd -> c.c_current_hdemo_sk == hd.hd_demo_sk end
                 },
                 %{
                   items: customer_address,
                   on: fn cc, cr, d, c, cd, hd, ca -> c.c_current_addr_sk == ca.ca_address_sk end
                 }
               ],
               %{
                 select: fn cc, cr, d, c, cd, hd, ca ->
                   Map.merge(
                     Map.merge(
                       Map.merge(
                         Map.merge(Map.merge(Map.merge(Map.merge(cc, cr), d), c), cd),
                         hd
                       ),
                       ca
                     ),
                     %{cc: cc, cr: cr, d: d, c: c, cd: cd, hd: hd, ca: ca}
                   )
                 end,
                 where: fn [cc, cr, d, c, cd, hd, ca] ->
                   d.d_year == 2001 && d.d_moy == 5 && cd.cd_marital_status == "M" &&
                     cd.cd_education_status == "Unknown" && hd.hd_buy_potential == "1001-5000" &&
                     ca.ca_gmt_offset == -6
                 end
               }
             )
           ...
           groups =
             _group_by(rows, fn %{cc: cc, cr: cr, d: d, c: c, cd: cd, hd: hd, ca: ca} ->
           ...
           Enum.map(items, fn g ->
             %{
               ...
               Manager: g.key.mgr,
               Returns_Loss: _sum(for x <- g.items, do: x.cr_net_loss)
             }
           end)
         end).()
      )
