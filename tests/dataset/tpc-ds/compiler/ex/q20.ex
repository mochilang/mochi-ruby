# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:34:38Z
defmodule Main do
  def main do
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: item, on: fn cs, i -> cs.cs_item_sk == i.i_item_sk end},
               %{items: date_dim, on: fn cs, i, d -> cs.cs_sold_date_sk == d.d_date_sk end}
             ],
             %{
               select: fn cs, i, d ->
                 Map.merge(Map.merge(Map.merge(cs, i), d), %{cs: cs, i: i, d: d})
               end,
               where: fn [cs, i, d] ->
                 if(is_map(["A", "B", "C"]),
         ...
         groups =
           _group_by(rows, fn %{cs: cs, i: i, d: d} ->
             %{
               id: i.i_item_id,
               desc: i.i_item_desc,
               cat: i.i_category,
         ...
         Enum.map(items, fn g ->
           %{
             ...
             itemrevenue: _sum(for x <- g.items, do: x.cs_ext_sales_price)
           }
         end)
       end).()
