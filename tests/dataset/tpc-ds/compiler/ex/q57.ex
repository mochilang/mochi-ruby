...
defmodule Main do
  ...
  def main do
    ...
    catalog_sales = [
      %{item: 1, call: 1, date: 1, price: 20},
      %{item: 1, call: 1, date: 2, price: 20},
      %{item: 1, call: 1, date: 3, price: 40}
    ]
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: item, on: fn cs, i -> cs.item == i.i_item_sk end},
               %{items: date_dim, on: fn cs, i, d -> cs.date == d.d_date_sk end},
               %{items: call_center, on: fn cs, i, d, cc -> cs.call == cc.cc_call_center_sk end}
             ],
             %{
               select: fn cs, i, d, cc ->
                 Map.merge(Map.merge(Map.merge(Map.merge(cs, i), d), cc), %{
                   cs: cs,
                   i: i,
                   d: d,
                   cc: cc
         ...
         groups =
           _group_by(rows, fn %{cs: cs, i: i, d: d, cc: cc} ->
             %{cat: i.i_category, call: cc.cc_name, year: d.d_year}
           end)
         ...
         Enum.map(items, fn g ->
           %{
             ...
             call: g.key.call,
             year: g.key.year,
             sum_sales: _sum(for x <- g.items, do: x.price)
           }
         end)
       end).()

    # avg_by_year :: list(map())
    avg_by_year =
      Enum.map(_group_by(grouped, fn g -> %{cat: g.cat, call: g.call} end), fn gg ->
        %{cat: gg.key.cat, call: gg.key.call, avg_sales: _avg(for x <- gg.items, do: x.sum_sales)}
      end)
