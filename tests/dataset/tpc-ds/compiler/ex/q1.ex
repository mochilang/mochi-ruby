# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:33:38Z
defmodule Main do
  def main do
    ...
    # customer_total_return :: list(map())
      ...
      (fn ->
         ...
         rows =
           _query(
             ...
             [
               %{
                 items: date_dim,
                 on: fn sr, d -> sr.sr_returned_date_sk == d.d_date_sk && d.d_year == 1998 end
               }
             ],
         ...
         Enum.map(items, fn g ->
           %{
             ...
             ctr_total_return: _sum(for x <- g.items, do: x.sr_return_amt)
           }
         end)
       end).()
    ...
    result =
      Enum.map(
        Enum.sort_by(
          for(
            ctr1 <- customer_total_return,
            s <- store,
            c <- customer,
            ctr1.ctr_store_sk == s.s_store_sk && ctr1.ctr_customer_sk == c.c_customer_sk &&
              (ctr1.ctr_total_return >
                 _avg(
                   for ctr2 <- customer_total_return,
                       ctr1.ctr_store_sk == ctr2.ctr_store_sk,
                       do: ctr2.ctr_total_return
                 ) * 1.2 && s.s_state == "TN"),
            do: {%{c_customer_id: c.c_customer_id}, c.c_customer_id}
          ),
          fn {_, k} -> k end
        ),
        fn {v, _} -> v end
