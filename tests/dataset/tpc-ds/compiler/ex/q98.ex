...
defmodule Main do
  def main do
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: item, on: fn ss, i -> ss.ss_item_sk == i.i_item_sk end},
               %{items: date_dim, on: fn ss, i, d -> ss.ss_sold_date_sk == d.d_date_sk end}
             ],
             %{
               select: fn ss, i, d ->
         ...
         Enum.map(items, fn g ->
           %{
             ...
             itemrevenue: _sum(for x <- g.items, do: x.ss_ext_sales_price)
           }
         end)
       end).()
    ...
    # totals :: list(map())
    totals =
      Enum.map(_group_by(grouped, fn g -> g.i_class end), fn cg ->
        %{class: cg.key, total: _sum(for x <- cg.items, do: x.itemrevenue)}
      end)
