...
defmodule Main do
  ...
  ], %{select: fn inv, d, i -> [inv, d, i] end, where: fn inv, d, i -> ((d.d_month_seq >= 0) && (d.d_month_seq <= 11)) end })
  groups = _group_by(rows, fn inv, d, i -> %{product_name: i.i_product_name, brand: i.i_brand, class: i.i_class, category: i.i_category} end)
  groups = Enum.map(groups, fn g -> %{g | items: Enum.map(g.items, fn [inv, d, i] -> inv end)} end)
  items = groups
  Enum.map(items, fn g -> %{i_product_name: g.key.product_name, i_brand: g.key.brand, i_class: g.key.class, i_category: g.key.category, qoh: _avg(for x <- g.items, do: x.inv_quantity_on_hand)} end)
end).()
    _json(qoh)
  end
  defp _avg(v) do
