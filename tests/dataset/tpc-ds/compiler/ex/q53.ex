# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:35:23Z
defmodule Main do
  ...
  def main do
    ...
    store_sales = [
      %{item: 1, date: 1, price: 10},
      %{item: 1, date: 2, price: 10},
      %{item: 2, date: 1, price: 30},
      %{item: 2, date: 2, price: 23}
    ]
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: item, on: fn ss, i -> ss.item == i.i_item_sk end},
               %{items: date_dim, on: fn ss, i, d -> ss.date == d.d_date_sk end}
             ],
             %{
               select: fn ss, i, d ->
         ...
         groups = _group_by(rows, fn %{ss: ss, i: i, d: d} -> i.i_manufact_id end)
         items = groups
         ...
         Enum.map(items, fn g ->
           %{
             ...
             avg_sales: _avg(for x <- g.items, do: x.price)
           }
         end)
       end).()
    ...
    result =
      for g <- grouped,
          g.avg_sales > 0 && abs(g.sum_sales - g.avg_sales) / g.avg_sales > 0.1,
          do: %{i_manufact_id: g.manu, sum_sales: g.sum_sales}
