...
defmodule Main do
  ...
  ], %{select: fn inv, d -> [inv, d] end, where: fn inv, d -> (d.d_date < "2000-03-15") end })
  groups = _group_by(rows, fn inv, d -> %{w: inv.inv_warehouse_sk, i: inv.inv_item_sk} end)
  groups = Enum.map(groups, fn g -> %{g | items: Enum.map(g.items, fn [inv, d] -> inv end)} end)
  items = groups
  ...
  Enum.map(items, fn g -> %{w: g.key.w, i: g.key.i, qty: _sum(for x <- g.items, do: x.inv_quantity_on_hand)} end)
    ...
    joined = for b <- before, a <- after, w <- warehouse, it <- item, ((b.w == a.w) && (b.i == a.i)) && (w.w_warehouse_sk == b.w) && (it.i_item_sk == b.i), do: %{w_name: w.w_warehouse_name, i_id: it.i_item_id, before_qty: b.qty, after_qty: a.qty, ratio: (a.qty / b.qty)}
    result = for r <- Enum.sort_by(joined, fn r -> [r.w_name, r.i_id] end), ((r.ratio >= (2 / 3)) && (r.ratio <= (3 / 2))), do: %{w_warehouse_name: r.w_name, i_item_id: r.i_id, inv_before: r.before_qty, inv_after: r.after_qty}
    _json(result)
  end
  defmodule Group do
