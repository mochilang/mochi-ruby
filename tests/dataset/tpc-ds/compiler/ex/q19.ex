# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:34:35Z
defmodule Main do
  def main do
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: store_sales, on: fn d, ss -> ss.ss_sold_date_sk == d.d_date_sk end},
               %{
                 items: item,
                 on: fn d, ss, i -> ss.ss_item_sk == i.i_item_sk && i.i_manager_id == 10 end
               },
               %{items: customer, on: fn d, ss, i, c -> ss.ss_customer_sk == c.c_customer_sk end},
               %{
                 items: customer_address,
                 on: fn d, ss, i, c, ca -> c.c_current_addr_sk == ca.ca_address_sk end
               },
               %{
                 items: store,
                 on: fn d, ss, i, c, ca, s ->
                   ss.ss_store_sk == s.s_store_sk &&
                     _slice_string(ca.ca_zip, 0, 5) != _slice_string(s.s_zip, 0, 5)
                 end
               }
             ],
             %{
               select: fn d, ss, i, c, ca, s ->
                 Map.merge(
                   Map.merge(Map.merge(Map.merge(Map.merge(Map.merge(d, ss), i), c), ca), s),
                   %{d: d, ss: ss, i: i, c: c, ca: ca, s: s}
                 )
               end,
               where: fn [d, ss, i, c, ca, s] -> d.d_moy == 11 && d.d_year == 1999 end
             }
           )

         groups =
           _group_by(rows, fn %{d: d, ss: ss, i: i, c: c, ca: ca, s: s} ->
             %{
               brand: i.i_brand,
         ...
         items = groups
         items = Enum.sort_by(items, fn g -> [g.key.brand] end)
         ...
         Enum.map(items, fn g ->
           %{
             ...
             ext_price: _sum(for x <- g.items, do: x.ss_ext_sales_price)
           }
         end)
       end).()
