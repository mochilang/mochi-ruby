# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:34:02Z
defmodule Main do
  @zip_list ["12345"]
  def main do
    ...
    _reverse(_slice_string("zip", 0, 2))
    # result :: list(map())
    result =
      (fn ->
         src = store_sales
           ...
           _query(
             ...
             [
               %{
                 items: date_dim,
                 on: fn ss, d ->
                   ss.ss_sold_date_sk == d.d_date_sk && d.d_qoy == 1 && d.d_year == 1998
                 end
               },
               ...
               %{
                 ...
                 on: fn ss, d, s, ca ->
                   _slice_string(s.s_zip, 0, 2) == _slice_string(ca.ca_zip, 0, 2)
                 end
               },
               %{
                 items: customer,
                 on: fn ss, d, s, ca, c ->
                   ca.ca_address_sk == c.c_current_addr_sk && c.c_preferred_cust_flag == "Y"
                 end
               }
             ],
             %{
               select: fn ss, d, s, ca, c ->
                 Map.merge(Map.merge(Map.merge(Map.merge(Map.merge(ss, d), s), ca), c), %{
                   ss: ss,
                   d: d,
                   s: s,
                   ca: ca,
         ...
         groups = _group_by(rows, fn %{ss: ss, d: d, s: s, ca: ca, c: c} -> s.s_store_name end)
         items = groups
         items = Enum.sort_by(items, fn g -> g.key end)

         Enum.map(items, fn g ->
           %{s_store_name: g.key, net_profit: _sum(for x <- g.items, do: x.ss.ss_net_profit)}
         end)
       end).()
