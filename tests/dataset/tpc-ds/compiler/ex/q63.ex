# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:35:48Z
defmodule Main do
  def main do
    # sales :: list(map())
    ...
    # by_mgr :: list(map())
    by_mgr =
      Enum.map(_group_by(sales, fn s -> %{mgr: s.mgr} end), fn g ->
        %{mgr: g.key.mgr, sum_sales: _sum(for x <- g.items, do: x.amount)}
      end)

    # result :: float()
    result = _sum(for x <- by_mgr, do: x.sum_sales)
    _json(result)
  end
