# Generated by Mochi compiler v0.10.26 on 2025-07-16T17:35:00Z
defmodule Main do
      ...
      (fn ->
         ...
         rows =
           _query(
             src,
             [
               %{items: customer, on: fn ws, c -> ws.bill_customer_sk == c.c_customer_sk end},
               %{
                 items: customer_address,
                 on: fn ws, c, ca -> c.c_current_addr_sk == ca.ca_address_sk end
               },
               %{items: item, on: fn ws, c, ca, i -> ws.item_sk == i.i_item_sk end},
               %{items: date_dim, on: fn ws, c, ca, i, d -> ws.sold_date_sk == d.d_date_sk end}
             ],
             %{
               select: fn ws, c, ca, i, d ->
                 Map.merge(Map.merge(Map.merge(Map.merge(Map.merge(ws, c), ca), i), d), %{
                   ws: ws,
                   c: c,
                   ca: ca,
                   i: i,
               ...
               where: fn [ws, c, ca, i, d] ->
                 (if(is_map(@zip_list),
                    do: Map.has_key?(@zip_list, _slice_string(ca.ca_zip, 0, 5)),
                    else: Enum.member?(@zip_list, _slice_string(ca.ca_zip, 0, 5))
                  ) ||
                    if(is_map(@item_ids),
                      do: Map.has_key?(@item_ids, i.i_item_id),
                      else: Enum.member?(@item_ids, i.i_item_id)
                    )) && d.d_qoy == @qoy && d.d_year == @year
               end
             }
           )

         groups = _group_by(rows, fn %{ws: ws, c: c, ca: ca, i: i, d: d} -> ca.ca_zip end)
         items = groups

         Enum.map(items, fn g ->
           %{ca_zip: g.key, sum_ws_sales_price: _sum(for x <- g.items, do: x.ws.sales_price)}
         end)
       end).()
