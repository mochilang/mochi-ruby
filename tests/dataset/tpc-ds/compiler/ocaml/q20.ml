...
type record1 = { mutable cs_item_sk : int; mutable cs_sold_date_sk : int; mutable cs_ext_sales_price : float }
...
type catalogsale = { mutable cs_item_sk : int; mutable cs_sold_date_sk : int; mutable cs_ext_sales_price : float }
...
let catalog_sales : record1 list = [{ cs_item_sk = 1; cs_sold_date_sk = 1; cs_ext_sales_price = 100. };{ cs_item_sk = 1; cs_sold_date_sk = 1; cs_ext_sales_price = 200. };{ cs_item_sk = 2; cs_sold_date_sk = 1; cs_ext_sales_price = 150. };{ cs_item_sk = 1; cs_sold_date_sk = 2; cs_ext_sales_price = 300. };{ cs_item_sk = 2; cs_sold_date_sk = 2; cs_ext_sales_price = 150. };{ cs_item_sk = 3; cs_sold_date_sk = 1; cs_ext_sales_price = 50. }]
let item : record2 list = [{ i_item_sk = 1; i_item_id = "ITEM1"; i_item_desc = "Item One"; i_category = "A"; i_class = "X"; i_current_price = 10. };{ i_item_sk = 2; i_item_id = "ITEM2"; i_item_desc = "Item Two"; i_category = "A"; i_class = "X"; i_current_price = 20. };{ i_item_sk = 3; i_item_id = "ITEM3"; i_item_desc = "Item Three"; i_category = "D"; i_class = "Y"; i_current_price = 15. }]
let date_dim : record3 list = [{ d_date_sk = 1; d_date = "2000-02-10" };{ d_date_sk = 2; d_date = "2000-02-20" }]
let filtered : record6 list = (let (__groups0 : (record4 * record1 list) list ref) = ref [] in
  List.iter (fun (cs : record1) ->
      List.iter (fun (i : record2) ->
            List.iter (fun (d : record3) ->
                        if (cs.cs_item_sk = i.i_item_sk) && (cs.cs_sold_date_sk = d.d_date_sk) && (((List.mem i.i_category ["A";"B";"C"]) && (d.d_date >= "2000-02-01")) && (d.d_date <= "2000-03-02")) then (
  ...
  List.iter (fun x ->
      __res1 := x.cs_ext_sales_price :: !__res1;
  ) g.items;
List.rev !__res1)
) } :: !__res0
  ) !__groups0;
...
let () =
  json result;
  assert ((result = [{ i_item_id = "ITEM1"; i_item_desc = "Item One"; i_category = "A"; i_class = "X"; i_current_price = 10.; itemrevenue = 600.; revenueratio = 66.66666666666667 };{ i_item_id = "ITEM2"; i_item_desc = "Item Two"; i_category = "A"; i_class = "X"; i_current_price = 20.; itemrevenue = 300.; revenueratio = 33.333333333333336 }]))
