; Generated by Mochi compiler v0.10.25 on 2025-07-15T04:47:31Z
(ns main)
...
(declare web_sales web_returns date_dim customer_address web_site filtered result)

(defn WebSale [ws_order_number ws_ship_date_sk ws_warehouse_sk ws_ship_addr_sk ws_web_site_sk ws_net_profit ws_ext_ship_cost]
  {:__name "WebSale" :ws_order_number ws_order_number :ws_ship_date_sk ws_ship_date_sk :ws_warehouse_sk ws_warehouse_sk :ws_ship_addr_sk ws_ship_addr_sk :ws_web_site_sk ws_web_site_sk :ws_net_profit ws_net_profit :ws_ext_ship_cost ws_ext_ship_cost}
)
...
(def filtered (vec (->> (for [ws web_sales d date_dim :when (_equal (:ws_ship_date_sk ws) (:d_date_sk d)) ca customer_address :when (_equal (:ws_ship_addr_sk ws) (:ca_address_sk ca)) w web_site :when (_equal (:ws_web_site_sk ws) (:web_site_sk w)) :when (and (and (and (_equal (:ca_state ca) "CA") (_equal (:web_company_name w) "pri")) (boolean (seq (vec (->> (for [ws2 web_sales :when (and (_equal (:ws_order_number ws) (:ws_order_number ws2)) (not (_equal (:ws_warehouse_sk ws) (:ws_warehouse_sk ws2))))] ws2)))))) (_equal (boolean (seq (vec (->> (for [wr web_returns :when (_equal (:wr_order_number wr) (:ws_order_number ws))] wr))))) false))] ws)))) ;; list of
