...
WebSale = Struct.new(:ws_order_number, :ws_warehouse_sk, :ws_ship_date_sk, :ws_ship_addr_sk, :ws_web_site_sk, :ws_ext_ship_cost, :ws_net_profit, keyword_init: true)
...
$web_sales = [OpenStruct.new(ws_order_number: 1, ws_warehouse_sk: 1, ws_ship_date_sk: 1, ws_ship_addr_sk: 1, ws_web_site_sk: 1, ws_ext_ship_cost: 2.0, ws_net_profit: 5.0), OpenStruct.new(ws_order_number: 1, ws_warehouse_sk: 2, ws_ship_date_sk: 1, ws_ship_addr_sk: 1, ws_web_site_sk: 1, ws_ext_ship_cost: 0.0, ws_net_profit: 0.0)]
$web_returns = [OpenStruct.new(wr_order_number: 1)]
	...
	for ws1 in $web_sales
		for ws2 in $web_sales
			if ((ws1.ws_order_number == ws2.ws_order_number) && (ws1.ws_warehouse_sk != ws2.ws_warehouse_sk))
				_res << OpenStruct.new(ws_order_number: ws1.ws_order_number)
			end
		end
	end
	_res
					...
					if (ws.ws_ship_addr_sk == ca.ca_address_sk)
						for w in $web_site
							if (ws.ws_web_site_sk == w.web_site_sk)
								if ((((ca.ca_state == "CA") && (w.web_company_name == "pri")) && (((($ws_wh)).map { |x| x.ws_order_number }).include?(ws.ws_order_number))) && (((($web_returns)).map { |wr| wr.wr_order_number }).include?(ws.ws_order_number)))
									_res << ws
								end
							end
						end
